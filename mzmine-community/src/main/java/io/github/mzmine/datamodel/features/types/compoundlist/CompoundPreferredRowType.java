...
/**
 * Sub-column under {@link CompoundMembersType} carrying the compound row's preferred
 * (representative) row reference. The value is a live {@link ModularFeatureListRow} which may be
 * either a source row from the underlying {@link ModularFeatureList} or a
 * {@link ModularCompoundRow} (nested compound).
 * <p>
 * Persisted as {@code <datatype id="compound_preferred_row" id_ref="..." compound="true|false"/>}.
 * On load the row reference is resolved against the source feature list (when
 * {@code compound="false"}) or against the surrounding {@link CompoundList} (when
 * {@code compound="true"}), obtained via the in-construction {@link ModularCompoundRow} carried in
 * the {@code row} parameter.
 * <p>
...
public class CompoundPreferredRowType extends DataType<ModularFeatureListRow> {
  ...
  public Object loadFromXML(@NotNull final XMLStreamReader reader,
    ...
    if (isCompound) {
      ...
      if (resolved == null) {
        throw new MissingCompoundRowException("Cannot resolve compound preferred row reference", cl,
            idRef);
      }
      return resolved;
    }
    ...
    if (!(resolved instanceof ModularFeatureListRow mflr)) {
      throw new MissingFeatureListRowException("Cannot resolve compound preferred row reference",
          flist, idRef);
    }
    return mflr;
  }
