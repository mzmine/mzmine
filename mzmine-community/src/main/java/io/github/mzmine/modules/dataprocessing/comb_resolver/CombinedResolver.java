...
public class CombinedResolver extends AbstractResolver {
    ...
    protected CombinedResolver(@NotNull ParameterSet parameters,
    ...
    public void loadEmbeddedResolvers(CombinedResolverParameters params,
        ...
        ModuleOptionsEnumComboParameter<CombinedResolverEnum> firstResolverParameters =
                CombinedResolverParameters.firstResolverParameters;
        ModuleOptionsEnumComboParameter<CombinedResolverEnum> secondResolverParameters =
                CombinedResolverParameters.secondResolverParameters;
        ParameterSet firstEmbeddedParameters =
                firstResolverParameters.getEmbeddedParameters().cloneParameterSet();
        ParameterSet secondEmbeddedParameters =
                secondResolverParameters.getEmbeddedParameters().cloneParameterSet();
    ...
    private FilterOutput filterRanges(double[] time, double[] intensity,
        ...
        return new FilterOutput(onlyFirstResolverRanges, onlySecondResolverRanges,
                bothResolverRanges);
    }
    ...
    private double getVarianceRatio(double[] time, double[] intensity, double leftBound,
        ...
        double[] extendedRangeIntensityLeft =
                Arrays.copyOfRange(intensity, extendedLeftIndex, leftIndex);
    ...
    public double calculateZigZagIndex(double[] featureIntensity) {
        ...
        for (int i = 1; i < featureIntensity.length - 1; i++) {
            zigZagSum += Math.pow(
                    (2 * featureIntensity[i] - featureIntensity[i - 1] - featureIntensity[i + 1]),
                    2);
        }
    ...
    private boolean peakOnEdge(double[] featureIntensities) {
        double maxIntensity = Double.NEGATIVE_INFINITY;
        int maxIndex = 0;
        for (int i = 0; i < featureIntensities.length; i++) {
            if (maxIntensity < featureIntensities[i]) {
                maxIntensity = featureIntensities[i];
                maxIndex = i;
            }
        }
        if (maxIndex == 0 || maxIndex == featureIntensities.length - 1) {
            return true;
        }
        return false;
    }
    ...
    public @NotNull List<Range<Double>> resolve(double[] time, double[] intensity) {
        ...
        FilterOutput filteredRanges =
                filterRanges(time, intensity, firstResolverRanges, secondResolverRanges);
    ...
    private void processRanges(double[] time, double[] intensity, List<Range<Double>> ranges,
        ...
        for (Range<Double> range : ranges) {
            ...
            final double varianceRatio =
                    getVarianceRatio(time, intensity, range.lowerEndpoint(), range.upperEndpoint());
            final double sharpness = calculateSharpness(featureIntensities);
            final int signChanges = countSignChanges(featureIntensities);
            ...
            if (detectedBy == DetectedBy.SECOND) {
                ...
                if (checkEdgeTooClose(time, intensity, range.lowerEndpoint(),
                        range.upperEndpoint())) {
                    continue;
                }
                if (signChanges > 7) {
                ...
                if (peakOnEdge(featureIntensities)) {
                    continue;
                }
                if (zigZagIndex < 0.5) {
                    validRanges.add(range);
            ...
            if (added) {
                debug.add(new CombinedResolverResult(zigZagIndex, varianceRatio, 0d, sharpness,
                        signChanges, detectedBy));
            }
        }
    }
