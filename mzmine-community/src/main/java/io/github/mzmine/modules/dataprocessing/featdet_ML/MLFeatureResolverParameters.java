...
public class MLFeatureResolverParameters extends GeneralResolverParameters {
  ...
  public static final PercentParameter threshold = new PercentParameter(
      "Confidence level from 0 to 1", "description for ML resolver", 0.5d, 0d, 1d);
  ...
  public static final BooleanParameter correctRanges = new BooleanParameter(
      "Correct ranges (for debugging)",
      "extends ranges if slope to next data point is sufficiently high");
  ...
  public static final BooleanParameter withOffset = new BooleanParameter(
      "(For testing) Use model which uses offset for later predictions", "", false);

  public MLFeatureResolverParameters() {
    super(createParams(MLSetup.FULL), null);
  }

  public MLFeatureResolverParameters(MLSetup setup) {
    super(createParams(setup), null);
  }
  ...
  private static Parameter[] createParams(MLSetup setup) {
    return switch (setup) {
      ...
      case INTEGRATED ->
          new Parameter[]{dimension, threshold, MIN_NUMBER_OF_DATAPOINTS, withOffset};
    };
  }
