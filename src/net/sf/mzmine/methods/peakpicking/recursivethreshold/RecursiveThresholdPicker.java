...
public class RecursiveThresholdPicker implements Method, TaskListener,
    ...
    public void taskStarted(Task task) {
        logger.info("Running recursive threshold peak picker on "
                + ((RecursiveThresholdPickerTask) task).getDataFile());
    ...
    public void runMethod(OpenedRawDataFile[] dataFiles,
        ...
        Task tasks[] = new RecursiveThresholdPickerTask[dataFiles.length];
        for (int i = 0; i < dataFiles.length; i++) {
            tasks[i] = new RecursiveThresholdPickerTask(dataFiles[i],
                    parameters);
        }
