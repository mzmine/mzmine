 ...
 */
public class LocalPicker implements PeakPicker {
	 ...
	 */
	public LocalPickerParameters askParameters(MainWindow mainWin, LocalPickerParameters currentValues) {

		// Initialize parameters
		LocalPickerParameters myParameters;
		if (currentValues==null) {
			myParameters = new LocalPickerParameters();
		} else {
	...
	public PeakList findPeaks(NodeServer nodeServer, RawDataAtNode rawData, PeakPickerParameters _parameters) {
		...
		LocalPickerParameters parameters = (LocalPickerParameters)_parameters;
		...
		*/
		...
		for (int i=0; i<maxScanNum; i++) {
			...
			for (int j=1; j<(intensities.length-1); j++) {
	 ...
	 */
	private MatchScore calcScore(PeakConstruction ucPeak, OneDimPeak oneDimPeak, double maxScore, LocalPickerParameters parameters) {
	 ...
	 */
	private int[] cropRTShape(double[] intensities, LocalPickerParameters parameters) {
/*
		int leftInd=0;
