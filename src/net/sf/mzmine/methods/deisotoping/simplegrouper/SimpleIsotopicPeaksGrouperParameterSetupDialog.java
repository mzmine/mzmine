...
/**
 * This class represent a dialog for adjusting parameter values for simple
 * isotopic peaks grouper method
 * 
 * @version 30 March 2006
 */
class SimpleIsotopicPeaksGrouperParameterSetupDialog extends JDialog
     ...
     *            MZmine GeneralParameters. There are required for determining
     *            current peak measurement type
     * @param _parameters
     *            Current AlignmentResultExporter parameter settings
     * 
     */
    public SimpleIsotopicPeaksGrouperParameterSetupDialog(Frame owner,
            SimpleIsotopicPeaksGrouperParameters _parameters) {
    ...
    private void initComponents() {
        ...
        NumberFormat rtNumberFormat = NumberFormat.getNumberInstance();
        rtNumberFormat.setMinimumFractionDigits(1);
        lblRTTolerance = new JLabel("RT tolerance");
        txtRTTolerance = new JFormattedTextField(rtNumberFormat);

        NumberFormat mzNumberFormat = NumberFormat.getNumberInstance();
        mzNumberFormat.setMinimumFractionDigits(3);
    ...
    private void getSettingsToForm() {
        ...
        if (parameters.monotonicShape) {
            cbMonotonicShape.setSelected(true);
        } else {
            cbMonotonicShape.setSelected(false);
        }

        if (parameters.chargeStates.contains(SimpleIsotopicPeaksGrouperParameters.chargeOne)) {
            cbChargeStatesOne.setSelected(true);
        } else {
            cbChargeStatesOne.setSelected(false);
        }
        if (parameters.chargeStates.contains(SimpleIsotopicPeaksGrouperParameters.chargeTwo)) {
            cbChargeStatesTwo.setSelected(true);
        } else {
            cbChargeStatesTwo.setSelected(false);
        }
        if (parameters.chargeStates.contains(SimpleIsotopicPeaksGrouperParameters.chargeThree)) {
            cbChargeStatesThree.setSelected(true);
        } else {
            cbChargeStatesThree.setSelected(false);
        }
    ...
    private void setSettingsFromForm() {
        ...
        if (cbMonotonicShape.isSelected()) {
            parameters.monotonicShape = true;
        } else {
            parameters.monotonicShape = false;
        }

        parameters.chargeStates.clear();
        if (cbChargeStatesOne.isSelected()) {
            parameters.chargeStates.add(SimpleIsotopicPeaksGrouperParameters.chargeOne);
        }
        if (cbChargeStatesTwo.isSelected()) {
            parameters.chargeStates.add(SimpleIsotopicPeaksGrouperParameters.chargeTwo);
        }
        if (cbChargeStatesThree.isSelected()) {
            parameters.chargeStates.add(SimpleIsotopicPeaksGrouperParameters.chargeThree);
        }
