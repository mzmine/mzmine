		...
		for (int scani=0; scani<totalScans; scani++) {
			...
			if ((sc.getRetentionTime() >= parameters.minRT) &&
				...
				for (int ind=0; ind<originalMassValues.length; ind++) {
					if (originalMassValues[ind]>=parameters.minMZ) { break; }
					numSmallerThanMin++;
				}

				int numBiggerThanMax = 0;
				for (int ind=(originalMassValues.length-1); ind>=0; ind--) {
					if (originalMassValues[ind]<=parameters.maxMZ) { break; }
					numBiggerThanMax++;
				}
				...
				// If base peak M/Z of original scan is out of range, then we must manually search for a new base peak
