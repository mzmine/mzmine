...
public class ChromatographicMedianFilterTask implements Task {
     ...
     * @param parameters
     */
    ChromatographicMedianFilterTask(RawDataFile rawDataFile, ChromatographicMedianFilterParameters parameters) {
		...
		for (int scani=0; scani<(totalScans+parameters.oneSidedWindowLength); scani++) {
			...
			if (scani<totalScans) {
				try {
					...
					status = TaskStatus.ERROR;
					errorMessage = e.toString();
					try { filteredRawDataFile = rawDataFileWriter.finishWriting();	} catch (IOException e2) {}
					return;
				}
			} else {
				sc = null;
			...
			if (sc != null) {
				...
				for (int datapointIndex=0; datapointIndex<mzValues.length; datapointIndex++) {
					...
					for (int bufferIndex=0; bufferIndex<scanBuffer.length; bufferIndex++) {
						...
						if ( (bufferIndex!=parameters.oneSidedWindowLength) && (scanBuffer[bufferIndex]!=null) ) {
							Object[] res = findClosestDatapointIntensity(mzValue, scanBuffer[bufferIndex], dataPointIndices[bufferIndex].intValue(), parameters);
