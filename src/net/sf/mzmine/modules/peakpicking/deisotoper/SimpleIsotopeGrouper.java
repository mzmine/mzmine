...
public class SimpleIsotopeGrouper implements BatchStepPeakPicking, TaskListener, ActionListener {
    ...
    public void taskStarted(Task task) {
        logger.info("Running isotopic peak grouper on "
                + ((SimpleIsotopeGrouperTask) task).getDataFile());
    }
    ...
    public TaskGroup runModule(RawDataFile[] dataFiles,
        ...
        Task tasks[] = new SimpleIsotopeGrouperTask[dataFiles.length];
        for (int i = 0; i < dataFiles.length; i++) {
            ...
            tasks[i] = new SimpleIsotopeGrouperTask(dataFiles[i],
