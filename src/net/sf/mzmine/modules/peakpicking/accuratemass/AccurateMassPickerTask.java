...
class AccurateMassPickerTask implements Task {
	...
	AccurateMassPickerTask(RawDataFile dataFile,
		...
		suffix = (String) parameters
				.getParameterValue(AccurateMassPickerParameters.suffix);
		mzTolerance = (Float) parameters
				.getParameterValue(AccurateMassPickerParameters.mzTolerance);
		minDataPoints = (Integer) parameters
				.getParameterValue(AccurateMassPickerParameters.minDataPoints);
	...
	public void run() {
		...
		for (int scanIndex = 0; scanIndex < totalScans; scanIndex++) {
			...
			for (AccurateMassDataPoint mzPeak : scanMZPeaks) {
				...
				Iterator<AccurateMassPeak> peaksIterator = currentPeaks
						.iterator();
	...
	private AccurateMassDataPoint[] detectMZPeaksInOneScan(Scan scan) {
		...
		for (int i = 0; i < scanDataPoints.length; i++) {
			...
			boolean nextPointIsHigher = ((i == scanDataPoints.length - 1)
					|| (scanDataPoints[i].getIntensity() == 0) || (scanDataPoints[i]
					.getIntensity() < scanDataPoints[i + 1].getIntensity()));
			...
			if (!ascending && nextPointIsHigher) {
				...
				DataPoint currentPeakDataPoints[] = currentMZPeak
		...
		AccurateMassDataPoint detectedMZPeaksArray[] = detectedMZPeaks
				.toArray(new AccurateMassDataPoint[0]);
	 ...
	 * 
	 * @param candidate
	 *            A chromatographic peak candidate consisting of same mass data
	 *            points from successive scans
	...
	private AccurateMassPeak[] processPeakCandidate(
		...
		if (1 == 1) {
			...
			for (int i = 5; i < peakScans.length - 5; i++) {
				AccurateMassDataPoint dp = candidatePeak
						.getDataPoint(peakScans[i]);

				float intSum = 0;
				for (int j = i - 5; j < i + 6; j++) {
					intSum += candidatePeak.getDataPoint(peakScans[j])
							.getIntensity();
				}
				intSum /= 11f;
			...
			if (smoothed.getScanNumbers().length == 0)
				return new AccurateMassPeak[0];
		...
		// Don't know why but labeled nested loop make eclipse complain that following
		// loop
		// starting with (AccurateMassDataPoint localMaximum : goodLocalMaxima)
		// ... unreachable
		//
		boolean skipMaximaSearch;
		for (int i = 5; i < peakScans.length - 5; i++) {
			AccurateMassDataPoint maximaCandidate = candidatePeak
					.getDataPoint(peakScans[i]);
			skipMaximaSearch = false;
			for (int j = 1; j <= 5; j++) {
				DataPoint leftNeighbour = candidatePeak
						.getDataPoint(peakScans[i - j]);
				DataPoint rightNeighbour = candidatePeak
						.getDataPoint(peakScans[i + j]);
				if ((leftNeighbour.getIntensity() > maximaCandidate
						.getIntensity())
						|| (rightNeighbour.getIntensity() > maximaCandidate
								.getIntensity())) {
					skipMaximaSearch = true;
					break;
				}
			}

			// found a good maximum
			if (skipMaximaSearch == true) {
				continue;
			}
		...
		for (AccurateMassDataPoint localMaximum : goodLocalMaxima) {
			...
			for (int i = 0; i < peakScans.length; i++) {
				AccurateMassDataPoint point = candidatePeak
						.getDataPoint(peakScans[i]);
				// 10s range
				if ((point.getRT() >= localMaximum.getRT() - 10)
						&& ((point.getRT() <= localMaximum.getRT() + 10))) {
