...
public class PubChemSingleRowIdentificationTask implements Task {
	...
	public void run() {
		...
		try {
			...
			Range massRange = new Range(valueOfQuery - range, valueOfQuery
					+ range);

			boolean chargedOnly = false;
			if ((chargedMol) && (ionName.equals(IonizationType.NO_IONIZATION)))
				chargedOnly = true;

			int resultCIDs[] = PubChemGateway.findPubchemCID(massRange,
					numOfResults, chargedOnly);
			...
			for (int i = 0; i < numItems; i++) {
				...
				PubChemGateway.getSummary(compound, valueOfQuery);
				...
				IsotopePattern compoundIsotopePattern = analyzer
						.getIsotopePattern(compound.getCompoundFormula(), 0.01,
								charge, ionName.isPositiveCharge(), 0, true,
								true, ionName);
