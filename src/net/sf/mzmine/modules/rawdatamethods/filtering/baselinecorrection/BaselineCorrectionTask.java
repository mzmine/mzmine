...
public class BaselineCorrectionTask extends AbstractTask {
    ...
    private final String chromatogramType;
    ...
    public BaselineCorrectionTask(final RawDataFile dataFile,
                                  final BaselineCorrectionParameters parameters) {
        ...
        // Get parameters.
        suffix = (String) parameters.getParameterValue(BaselineCorrectionParameters.SUFFIX);
        removeOriginal = (Boolean) parameters.getParameterValue(BaselineCorrectionParameters.REMOVE_ORIGINAL);
        smoothing = (Double) parameters.getParameterValue(BaselineCorrectionParameters.SMOOTHING);
        asymmetry = (Double) parameters.getParameterValue(BaselineCorrectionParameters.ASYMMETRY);
        binWidth = (Double) parameters.getParameterValue(BaselineCorrectionParameters.MZ_BIN_WIDTH);
        useBins = (Boolean) parameters.getParameterValue(BaselineCorrectionParameters.USE_MZ_BINS);
        msLevel = (Integer) parameters.getParameterValue(BaselineCorrectionParameters.MS_LEVEL);
        chromatogramType = (String) parameters.getParameterValue(BaselineCorrectionParameters.CHROMOTAGRAM_TYPE);
    }
    ...
    public void run() {
        ...
        try {
            ...
            final boolean useTIC = BaselineCorrectionParameters.CHROMATOGRAM_TIC.equals(chromatogramType);
     ...
     */
    private void correctBasePeakBaselines(final RawDataFileWriter writer, final int level, final int numBins) throws IOException {
     ...
     * @throws IOException if there are i/o problems.
     */
    private void correctTICBaselines(final RawDataFileWriter writer, final int level, final int numBins) throws IOException {
    ...
    private double[] asymBaseline(final double[] chromatogram) {
        ...
        try {
            ...
            throw new IllegalStateException("Baseline correction requires R but it couldn't be loaded (" + t.getMessage() + ')');
        }
     ...
     * @param scanIndex  the current scan index that these datapoints come from.
     * @return the corrected data points.
     */
    private DataPoint[] subtractBasePeakBaselines(final DataPoint[] dataPoints, final double[][] baselines, final int numBins, final int scanIndex) {
     ...
     * @return the corrected data points.
     */
    private DataPoint[] subtractTICBaselines(final DataPoint[] dataPoints, final double[][] baselines, final int numBins, final int scanIndex) {
