...
class SpectraVisualizerWindow extends JInternalFrame implements ActionListener {
	...
	private NumberFormat intensityFormat = MZmineCore.getDesktop()
			.getIntensityFormat();
				...
				synchronized (dataFile) {
					...
					if ((currentScan.getMSLevel() > 1) && (parentNumber > 0)) {
						...
						String itemText = "Parent scan #"
								+ parentNumber
								+ ", RT: "
								+ rtFormat.format(dataFile
										.getScan(parentNumber)
										.getRetentionTime())
								+ ", precursor m/z: "
					...
					String subTitle = "MS"
							...
							+ " m/z ("
							+ intensityFormat.format(currentScan
	...
	public void actionPerformed(ActionEvent event) {
		...
		if (command.equals("SET_SAME_RANGE")) {
			...
			JInternalFrame spectraFrames[] = desktop
					.getVisibleFrames(SpectraVisualizerWindow.class);
