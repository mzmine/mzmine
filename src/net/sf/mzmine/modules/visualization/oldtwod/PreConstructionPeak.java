 ...
 * pre-peak information are m/z ratio and start and stop RT of the peak, which
 * are defined by the user when drawing the peak on 2d plot. Once the drawn
 * pre-peaks are finalized, datapoints of raw data file must be added using
 * methods of the ConstructionPeak class.
 * 
 */
public class PreConstructionPeak implements Peak, RawDataAcceptor {
	...
	public PreConstructionPeak(RawDataFile dataFile, float preMZ,
		...
		// TODO: Add m/z tolerance parameter to setup dialog, and pass the value
		// to here
	...
	/**
	 * Checks if the scan is within the range of this peak If yes, then adds the
	 * nearest data point (if any within tolerances) to this peak
	 */
	public void addScan(Scan scan, int index, int total) {
		...
		if ((scan.getRetentionTime() >= preStartRT)
			...
			// TODO: Change this: locate most intense datapoint within
			// tolerances (instead of nearest in m/z)
			...
			for (int i = 0; i < dataPoints.length; i++) {
				float currentMZDifference = Math.abs(dataPoints[i].getMZ()
						- preMZ);
			...
			// If nearest datapoint is within m/z tolerance, then add this
			// datapoint to this peak
			if (nearestMZDifference < mzTolerance)
				this.addDatapoint(scan.getScanNumber(), nearestDatapointMZ,
						scan.getRetentionTime(), nearestDatapointInt);
