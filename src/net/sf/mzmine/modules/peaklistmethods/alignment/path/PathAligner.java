 ...
 * 
 */
public class PathAligner implements BatchStep, ActionListener {

	private Logger logger = Logger.getLogger(this.getClass().getName());
	private PathAlignerParameters parameters;
	...
	public PathAligner() {
		...
		parameters = new PathAlignerParameters();

		desktop.addMenuItem(MZmineMenu.ALIGNMENT, "Path alignment",
				"Alignment of two or more data sets.", KeyEvent.VK_P, false, this, null);
	...
	public void taskFinished(Task task) {
		if (task.getStatus() == TaskStatus.FINISHED) {
			logger.info("Finished alignment on " + ((PathAlignerTask) task).getTaskDescription());
		}

		if (task.getStatus() == TaskStatus.ERROR) {

			String msg = "Error while alignment on .. " + ((PathAlignerTask) task).getErrorMessage();
			logger.severe(msg);
