...
class DuplicateFilterTask extends AbstractTask {
	...
	public void run() {
		...
		filteredPeakList = new SimplePeakList(peakList + " " + suffix,
		...
		for (int firstRowIndex = 0; firstRowIndex < peakListRows.length; firstRowIndex++) {
			...
			for (int secondRowIndex = (firstRowIndex + 1); secondRowIndex < peakListRows.length; secondRowIndex++) {
				...
				if (requireSameIdentification) {
					// TODO
					// Use preferred identifications if available instead of all
					// identifications
					...
					if ((firstIdentities.length == 0)
							&& (secondIdentities.length == 0))
						sameID = true;

					for (PeakIdentity firstIdentity : firstIdentities) {
						sameID = false;
						for (PeakIdentity secondIdentity : secondIdentities) {
							if (firstIdentity.getName().equals(
									secondIdentity.getName())) {
								sameID = true;
								break;
							}
						}
				...
				Range mzRange = mzDifferenceMax.getToleranceRange(firstMZ);
				if (mzRange.contains(secondMZ)) {
					sameMZ = true;
				}
				...
				Range rtRange = rtDifferenceMax.getToleranceRange(firstRT);
				if (rtRange.contains(secondRT)) {
					sameRT = true;
				}
		...
		filteredPeakList
				.addDescriptionOfAppliedTask(new SimplePeakListAppliedMethod(
						"Duplicate peak filter", parameters));
