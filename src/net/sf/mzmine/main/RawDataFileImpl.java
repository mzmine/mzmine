...
public class RawDataFileImpl implements RawDataFile, RawDataFileWriter {
	...
	RawDataFileImpl(File scanFileDir, String fileName,
		...
		if (preloadLevel != PreloadLevel.PRELOAD_ALL_SCANS) {
			...
			File scanFile;
			while (true) {
				scanFile = new File(scanFileDir, fileName + "." + offset
						+ ".scan");
				if (!scanFile.exists()) {
					break;
				}
			...
			writingScanDataFileName = scanFile.getName();
		...
		if (newScan.getParentScanNumber() > 0) {
			...
			if (parentScan != null) {
				if (parentScan instanceof StorableScan) {
					int fragmentScans[] = ((StorableScan) parentScan)
					...
					if (fragmentScans != null) {
						...
						fragmentScans = CollectionUtils
								.toIntArray(fragmentScansList);
						((StorableScan) parentScan)
								.setFragmentScanNumbers(fragmentScans);
					} else {
						((StorableScan) parentScan)
								.setFragmentScanNumbers(new int[] { newScan
										.getScanNumber() });
					}
				}
				if (parentScan instanceof SimpleScan) {
					int fragmentScans[] = ((SimpleScan) parentScan)
							.getFragmentScanNumbers();
					if (fragmentScans != null) {
						ArrayList<Integer> fragmentScansList = new ArrayList<Integer>();
						for (int fragmentScan : fragmentScans)
	...
	public RawDataFile finishWriting() throws IOException {
		...
		scanDataFile = new RandomAccessFile(
				new File(MZmineCore.getCurrentProject().getLocation(),
