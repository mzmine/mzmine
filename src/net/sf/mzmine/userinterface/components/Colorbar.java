...
public class Colorbar extends JPanel {
    ...
    public Colorbar(HeatMapColorPicker _colorMap, String[] _intensityLabels,
            int _numOfSlots) {
        colorMap = _colorMap;
    ...
    public void paint(Graphics g) {
        ...
        double diff_y_dat = intensityLevels[intensityLevels.length - 1]
                - intensityLevels[0];
        ...
        for (int i = 0; i < numOfSlots; i++) {
            ...
            g.fillRect(0, (int) java.lang.Math.round(currentY),
                    (int) java.lang.Math.round(0.4 * w),
                    (int) java.lang.Math.round(currentY + scrStep)
        ...
        for (int i = 0; i < intensityLabels.length; i++) {
            currentY = h - marginY - (intensityLevels[i] - intensityLevels[0])
                    * pixelsPerData;
            String s = intensityLabels[i];
            g.drawBytes(s.getBytes(), 0, s.length(),
                    (int) java.lang.Math.round(0.4 * w),
                    (int) (int) java.lang.Math.round(currentY) + 5);
