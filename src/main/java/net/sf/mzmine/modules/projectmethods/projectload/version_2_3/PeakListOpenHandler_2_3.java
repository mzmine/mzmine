...
public class PeakListOpenHandler_2_3 extends DefaultHandler implements
	PeakListOpenHandler {
    ...
    public PeakListOpenHandler_2_3(Hashtable<String, RawDataFile> dataFilesIDMap) {
	...
	if (qName
	    ...
	    preferred = Boolean.parseBoolean(attrs
		    .getValue(PeakListElementName_2_3.PREFERRED
			    .getElementName()));
	}

	// <IDENTITY_PROPERTY>
	if (qName.equals(PeakListElementName_2_3.IDPROPERTY.getElementName())) {
	    identityPropertyName = attrs.getValue(PeakListElementName_2_3.NAME
		    .getElementName());
	}
	...
	if (qName.equals(PeakListElementName_2_3.PEAK.getElementName())) {
	    ...
	    peakColumnID = attrs.getValue(PeakListElementName_2_3.COLUMN
		    .getElementName());
	    mass = Double.parseDouble(attrs.getValue(PeakListElementName_2_3.MZ
		    .getElementName()));
	    // Before MZmine 2.6 retention time was saved in seconds, but now we
	    // use minutes, so we need to divide by 60
	    rt = Double.parseDouble(attrs.getValue(PeakListElementName_2_3.RT
		    .getElementName())) / 60d;
	    ...
	    peakStatus = attrs.getValue(PeakListElementName_2_3.STATUS
		    .getElementName());
	    String chargeString = attrs.getValue(PeakListElementName_2_3.CHARGE
		    .getElementName());
	...
	if (qName.equals(PeakListElementName_2_3.MZPEAKS.getElementName())) {
	    numOfMZpeaks = Integer
		    .parseInt(attrs.getValue(PeakListElementName_2_3.QUANTITY
			    .getElementName()));
	}

	// <ISOTOPE_PATTERN>
	if (qName.equals(PeakListElementName_2_3.ISOTOPE_PATTERN
		.getElementName())) {
	    ...
	    currentIsotopePatternDescription = attrs
		    .getValue(PeakListElementName_2_3.DESCRIPTION
			    .getElementName());
	}
	...
	// <NAME>
	if (qName
		.equals(PeakListElementName_2_3.PEAKLIST_NAME.getElementName())) {
	...
	// <PEAKLIST_DATE>
	if (qName
		.equals(PeakListElementName_2_3.PEAKLIST_DATE.getElementName())) {
	...
	if (qName.equals(PeakListElementName_2_3.METHOD_PARAMETERS
		.getElementName())) {
