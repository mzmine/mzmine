...
public class SQLColumnSettingsParameter implements
	...
	for (int row = 0; row < value.getRowCount(); row++) {
	    String sqlColumn = (String) value.getValueAt(row, 0);
	    SQLExportDataType dataType = (SQLExportDataType) value.getValueAt(
		    row, 1);
		...
		errorMessages.add("Please set the column value for column "
			+ sqlColumn);
		return false;
	    }
	}
