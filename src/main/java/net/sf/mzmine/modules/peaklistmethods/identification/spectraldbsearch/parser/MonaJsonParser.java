...
public class MonaJsonParser implements SpectralDBParser {
  ...
  @Override
  public @Nonnull List<SpectralDBEntry> parse(File dataBaseFile) throws IOException {
    ...
    for (String l : lines) {
      ...
      if (error > 5 && list.isEmpty()) {
        logger.log(Level.WARNING, "This file was no MONA spectral json library");
        return list;
      }
    }
    logger.info(list.size() + " MONA library entries imported");
    return list;
  }
