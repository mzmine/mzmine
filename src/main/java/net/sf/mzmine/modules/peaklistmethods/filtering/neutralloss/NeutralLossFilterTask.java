...
public class NeutralLossFilterTask extends AbstractTask {
	...
	NeutralLossFilterTask(MZmineProject project, PeakList peakList, ParameterSet parameters) {
		...
		if (suffix.equals("auto")) {
			suffix = "_NL: " + dMassLoss + " RTtol: " + rtTolerance.getTolerance() + " minRating: " + minRating
					+ "_results";
		}
		...
		for (int i = 0; i < totalRows; i++) {
			...
			for (int k = 0; k < resultBuffer.length; k++) // reminder: resultBuffer.length = diff.size()
			{
				for (int l = 0; l < resultBuffer[k].getFoundCount(); l++) {
					...
					candidates.get(k).checkForBetterRating(groupedPeaks, 0, resultBuffer[k].getRow(l), diff.get(k),
							minRating);
				}
			}
	 ...
	 * @param maxMass
	 * @return will return ArrayList<PeakListRow> of all peaks within the range of
	 *         pL[parentIndex].mz -> pL[parentIndex].mz+maxMass
	 */
