...
public class GPLipidSearchTask extends AbstractTask {
  ...
  public GPLipidSearchTask(ParameterSet parameters, PeakList peakList) {
    ...
    minChainLength = parameters.getParameter(GPLipidSearchParameters.minChainLength).getValue();
    maxChainLength = parameters.getParameter(GPLipidSearchParameters.maxChainLength).getValue();
    maxDoubleBonds = parameters.getParameter(GPLipidSearchParameters.maxDoubleBonds).getValue();
    mzTolerance = parameters.getParameter(GPLipidSearchParameters.mzTolerance).getValue();
    selectedLipids = parameters.getParameter(GPLipidSearchParameters.lipidTypes).getValue();
    ionizationType = parameters.getParameter(GPLipidSearchParameters.ionizationMethod).getValue();
  ...
  public void run() {
    ...
    totalSteps = selectedLipids.length * (maxChainLength + 1) * (maxDoubleBonds + 1)
        * (maxChainLength + 1) * (maxDoubleBonds + 1);
    ...
    for (GPLipidType lipidType : selectedLipids) {
      for (int fattyAcid1Length = 0; fattyAcid1Length <= maxChainLength; fattyAcid1Length++) {
        for (int fattyAcid1DoubleBonds =
            0; fattyAcid1DoubleBonds <= maxDoubleBonds; fattyAcid1DoubleBonds++) {
          for (int fattyAcid2Length = 0; fattyAcid2Length <= maxChainLength; fattyAcid2Length++) {
            for (int fattyAcid2DoubleBonds =
                0; fattyAcid2DoubleBonds <= maxDoubleBonds; fattyAcid2DoubleBonds++) {
              ...
              if (((fattyAcid1DoubleBonds > 0) && (fattyAcid1DoubleBonds > fattyAcid1Length - 1))
                  || ((fattyAcid2DoubleBonds > 0)
                      && (fattyAcid2DoubleBonds > fattyAcid2Length - 1))) {
                finishedSteps++;
                continue;
              }

              // Prepare a lipid instance
              GPLipidIdentity lipid = new GPLipidIdentity(lipidType, fattyAcid1Length,
                  fattyAcid1DoubleBonds, fattyAcid2Length, fattyAcid2DoubleBonds);
