...
/**
 * This class represents a 3D boxes which are displayed when user shift+clicks on a peak in the 3D
 * view
 */
public class ThreeDPeakCells extends CellImpl {
   ...
   */
  public ThreeDPeakCells(final DisplayImpl display3D, final MathType pointType,
   ...
   */
  public void setPeaks(final Feature[] thePeaks) throws VisADException, RemoteException {
        ...
        if (references[index] == null) {
          ...
          display.addReference(reference,
              new ConstantMap[] {new ConstantMap(0.8, Display.Red),
                  new ConstantMap(0.2, Display.Green), new ConstantMap(0.2, Display.Blue),
  ...
  private Data createPeakBox(final Feature peak) throws VisADException {
    ...
    return GriddedSet.create(pointTupleType,
        new float[][] {
            {rtMin, rtMax, rtMin, rtMax, rtMin, rtMax, rtMin, rtMax, rtMin, rtMin, rtMin, rtMin,
                rtMax, rtMax, rtMax, rtMax,},
            {mzMin, mzMin, mzMin, mzMin, mzMax, mzMax, mzMax, mzMax, mzMin, mzMax, mzMin, mzMax,
                mzMin, mzMax, mzMin, mzMax},
            {heightMin, heightMin, heightMax, heightMax, heightMax, heightMax, heightMin, heightMin,
                heightMin, heightMin, heightMax, heightMax, heightMax, heightMax, heightMin,
                heightMin}},
        new int[] {2, 8});
  }
}
