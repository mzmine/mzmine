...
class ProductIonFilterDataSet extends AbstractXYDataset implements Task, XYToolTipGenerator {
	...
	ProductIonFilterDataSet(RawDataFile rawDataFile, Object xAxisType, Range<Double> rtRange, Range<Double> mzRange,
			ProductIonFilterVisualizerWindow visualizer, MZTolerance mzDifference, List<Double> targetedMZ_List, List<Double> targetedNF_List, Double basePeakPercent, File fileName) {
		...
		// mzDifference is maximum difference allowed between selected product m/z values and scan m/z value
	...
	public void run() {
		...
		//  dataList that will contain output m/z values, RT, and scan number for ID, ##for use in targeted peak detection
		...
		for (int scanNumber : scanNumbers) {
			...
			// topPeaks will contain indexes to mzValues in scan above a threshold defined as : 'scan basePeak Intensity' * percent of base Peak to include
			...
			/**
			 * Depending on filter conditions these if statements will filter based off of product m/z or neutral loss
			 *  or both within a scan. 
			 *  Pass becomes set to true if filter conditions are met and scan is added to output file and visual plot
			 */
			...
			if (targetedMZ_List.get(0) != 0 && targetedNF_List.get(0) != 0) {	
				...
				for (int h = 0; h < topPeaks.length; h++) {
					...
					for (int j = 0; j < targetedMZ_List.size(); j++) {
						if (mzDifference.getToleranceRange(targetedMZ_List.get(j)).contains(
								scanDataPoints[peakIndex].getMZ())==true) {
					...
					//scan for all neutral loss values if more than one, set pass to true if all netural loss values are found
					for (int j = 0; j < targetedNF_List.size(); j++) {
						if (mzDifference.getToleranceRange(targetedNF_List.get(j)).contains(
								neutralLoss)==true) {
			...
			if (pass == true) {
				...
				for (int i = 0; i < topPeaks.length; i++) {
					...
					ProductIonFilterDataPoint newPoint = new ProductIonFilterDataPoint(scanDataPoints[peakIndex].getMZ(),
							scan.getScanNumber(), scan.getPrecursorMZ(), scan.getPrecursorCharge(),
					...
					String temp = Double.toString(scan.getPrecursorMZ()) + "," + Double.toString(scanDataPoints[peakIndex].getMZ()) + "," +
		...
		try {
			...
			if (fileName.getPath().contains(namePattern)){
				...
				String cleanPlName = rawDataFile.getName()
                        .replaceAll("[^a-zA-Z0-9.-]", "_");
                // Substitute
                String newFilename = fileName.getPath()
                        .replaceAll(Pattern.quote(namePattern), cleanPlName);
		...
		// write output to csv file - for visual plotting in R. has product ion, precursor ion m/z, and retention times.
		try {
			String namePattern = "{}";
			File curFile = fileName;
	...
	public ProductIonFilterDataPoint getDataPoint(double xValue, double yValue) {
		Vector<ProductIonFilterDataPoint> dataCopy = new Vector<ProductIonFilterDataPoint>(dataSeries.get(RAW_LEVEL));
