 ...
 */
public class PointTwoDVisualizerWindow extends JFrame implements ActionListener {

    private static final long serialVersionUID = 1L;
    private PointTwoDToolBar toolBar;
    private PointTwoDPlot twoDPlot;
    private PointTwoDBottomPanel bottomPanel;
    private PointTwoDDataSet dataset;
    private RawDataFile dataFile;
    private boolean tooltipMode;
    private boolean logScale;

    public PointTwoDVisualizerWindow(RawDataFile dataFile, Scan scans[],
        ...
        dataset = new PointTwoDDataSet(dataFile, scans, rtRange, mzRange, this);

        toolBar = new PointTwoDToolBar(this);
        add(toolBar, BorderLayout.EAST);

        twoDPlot = new PointTwoDPlot(dataFile, this, dataset, rtRange, mzRange);
        add(twoDPlot, BorderLayout.CENTER);

        bottomPanel = new PointTwoDBottomPanel(this, dataFile, parameters);
        ...
        WindowSettingsParameter settings = paramSet
                .getParameter(PointTwoDVisualizerParameters.windowSettings);
    ...
    public void actionPerformed(ActionEvent event) {
        ...
        if (command.equals("SWITCH_PLOTMODE")) {
            ...
            if (twoDPlot.getPlotMode() == PointPlotMode.CENTROID) {
                toolBar.setCentroidButton(true);
                twoDPlot.setPlotMode(PointPlotMode.CONTINUOUS);
            } else {
                toolBar.setCentroidButton(false);
                twoDPlot.setPlotMode(PointPlotMode.CENTROID);
            }
        }
        ...
        if (command.equals("OUTPUT_THESE_DATA")) {
           twoDPlot.outputTheseData();
        }


    }

    PointTwoDPlot getPlot() {
        return twoDPlot;
    }
}
