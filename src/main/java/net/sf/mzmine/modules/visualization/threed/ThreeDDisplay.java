...
public class ThreeDDisplay extends DisplayImplJ3D {
  ...
  public ThreeDDisplay() throws RemoteException, VisADException {
    ...
    final RealTupleType intensityRange = new RealTupleType(intensityType,
        new LogCoordinateSystem(new RealTupleType(logIntensityType)), null);
    ...
    configureAxis(intensityMap.getAxisScale(), "Intensity",
        MZmineCore.getConfiguration().getIntensityFormat());

    // Set log axis properties.
    configureAxis(logIntensityMap.getAxisScale(), "Intensity",
        MZmineCore.getConfiguration().getIntensityFormat());
    ...
    projCont.setMatrix(
        MouseBehaviorJ3D.static_multiply_matrix(MOUSE_BEHAVIOUR_MATRIX, projCont.getMatrix()));
    ...
    // Peak area cells.
    cells = new ThreeDPeakCells(this, new RealTupleType(retentionTimeType, mzType, heightType),
        pickRenderer);
  }
   ...
   */
  public void setData(final float[][] intensityValues, final Set domainSet, final double rtMin,
  ...
  public void setPeaks(final PeakList peakList, final Feature[] peaks,
    ...
    for (int i = 0; i < peakCount; i++) {
      ...
      peakData[0] = new RealTuple(heightTupleType,
          new double[] {peaks[i].getRawDataPointsIntensityRange().upperEndpoint()
              + maxIntensity * PEAK_LABEL_OFFSET});
  ...
  public Color getAnnotationColor() {
    ...
    return new Color((float) peakColorMap[0].getConstant(), (float) peakColorMap[1].getConstant(),
  ...
  private void safeRemoveMap(final ScalarMap map) throws VisADException, RemoteException {
    ...
    for (Iterator<?> iterator = getMapVector().iterator(); !hasMap && iterator.hasNext();) {
