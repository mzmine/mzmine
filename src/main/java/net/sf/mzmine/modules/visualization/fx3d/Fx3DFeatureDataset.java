...
public class Fx3DFeatureDataset {
    ...
    Fx3DFeatureDataset(Feature feature, int rtResolution, int mzResolution,
        ...
        for (int x = minFeatureRtPoint; x < maxFeatureRtPoint; x++) {
            for (int z = minFeatureMzPoint; z < maxFeatureMzPoint; z++) {
                mesh.getPoints().addAll((float) x * factorX,
                        -maxIntensityValue * AMPLIFI, (float) z * factorZ);
            }
        }
        ...
        setFeatureColor(rtLength, mzLength);
    ...
    public void setFeatureColor(int width, int height) {
        WritableImage wr = new WritableImage(width, height);
        PixelWriter pw = wr.getPixelWriter();
        double opacity = 0.3;
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                Color color = Color.rgb(165, 42, 42, opacity);
