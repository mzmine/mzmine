...
public class SimpleFeature implements Feature {
	...
	this.rtRange = Range.closed(
                p.getRawDataPointsRTRange().lowerEndpoint(),
                p.getRawDataPointsRTRange().upperEndpoint());
	
        this.mzRange = Range.closed(
                p.getRawDataPointsMZRange().lowerEndpoint(),
                p.getRawDataPointsMZRange().upperEndpoint());
        
	this.intensityRange = Range.closed(
                p.getRawDataPointsIntensityRange().lowerEndpoint(), 
	...
	for (int i = 0; i < scanNumbers.length; i++) {
	    dataPointsPerScan[i] = new SimpleDataPoint(
                    p.getDataPoint(scanNumbers[i]));
	}
