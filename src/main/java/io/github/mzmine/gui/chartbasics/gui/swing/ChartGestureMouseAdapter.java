...
/**
 * Handles all MouseEvents (like a MouseAdapter) and transforms them into
 * {@link ChartGestureEvent}s which are then handled by a list of
 * {@link ChartGestureHandler}s.
 * <p>
...
public class ChartGestureMouseAdapter extends MouseAdapter
        implements GestureMouseAdapter {
    ...
    // listen for
    private HashMap<Event, Boolean> listensFor = new HashMap<Event, Boolean>(
            Event.values().length);
    ...
    /**
     * Call all handlers for a specific Gesture Also call all parent handlers
     * (AXIS is parent entity of RANGE_AXIS entity ALL is parent of all
     * events/entities)
     * 
     * @param e
     */
    private void handleEvent(final ChartGestureEvent e) {
        if (gestureHandlers != null)
            gestureHandlers.stream().filter(
                    handler -> handler.getGesture().filter(e.getGesture()))
    ...
    public void addDragGestureHandler(DragHandler[] handler, Key[] key,
        ...
        ChartGestureHandler h = ChartGestureHandler.createDragDiffHandler(
                handler, key, entity, button, orient, param);
        addGestureHandler(h);
    }
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        if (e.getComponent() instanceof ChartPanel) {
            ...
            ChartGesture.Entity gestureEntity = ChartGesture
                    .getGestureEntity(entity);
            ...
            if (!e.isConsumed()) {
                ...
                if (e.getClickCount() == 2) {
                    ...
                    handleEvent(new ChartGestureEvent(chartPanel, e, entity,
                            new ChartGesture(gestureEntity, Event.DOUBLE_CLICK,
                                    button)));
                } else if (e.getClickCount() == 1) {
                    // handle event
                    handleEvent(new ChartGestureEvent(chartPanel, e, entity,
                            new ChartGesture(gestureEntity, Event.CLICK,
                                    button)));
                }
            }
        }
    ...
    public void mouseDragged(MouseEvent e) {
        ...
        if (e.getComponent() instanceof ChartPanel) {
            ...
            ChartGesture.Entity gestureEntity = lastDragEvent.getGesture()
                    .getEntity();
    ...
    public void mouseWheelMoved(MouseWheelEvent e) {
        ...
        if (e.getComponent() instanceof ChartPanel) {
            ...
            // handle event
            handleEvent(new ChartGestureEvent(chartPanel, e, entity,
                    new ChartGesture(gestureEntity, Event.MOUSE_WHEEL,
                            button)));
        }
    }
