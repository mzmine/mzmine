...
/**
 * The {@link ChartGestureDragDiffHandler} consumes primary mouse events to
 * generate {@link ChartGestureDragDiffEvent}s. These events are then processed
 * by one or multiple {@link Consumer}s. Each Consumer has a specific
 * {@link Key} filter. Key and Consumer array have to be sorted accordingly.
 * 
...
public class ChartGestureDragDiffHandler extends ChartGestureHandler {
    ...
    public ChartGestureDragDiffHandler(ChartGesture.Entity entity,
    ...
    public ChartGestureDragDiffHandler(ChartGesture.Entity entity,
        ...
        super(new ChartGesture(entity,
                new Event[] { Event.RELEASED, Event.PRESSED, Event.DRAGGED },
                button, Key.ALL));
        /**
         * Handles PRESSED, DRAGGED, RELEASED Events Fires the correct
         * DragDiffHandlers for the Key filter
         */
    ...
    public Orientation getOrientation(ChartGestureEvent event) {
        ...
        if (ce instanceof AxisEntity) {
            ...
            if ((entity.equals(Entity.DOMAIN_AXIS)
                    && plotorient.equals(PlotOrientation.VERTICAL))
                    || (entity.equals(Entity.RANGE_AXIS)
                            && plotorient.equals(PlotOrientation.HORIZONTAL)))
        ...
        return new Consumer<ChartGestureEvent>() {
            ...
            public void accept(ChartGestureEvent event) {
                ...
                if (event.checkEvent(Event.RELEASED)) {
                    ...
                    if (last != null) {
                        // get data space coordinates
                        Point2D released = chartPanel.mouseXYToPlotXY(e.getX(),
                                e.getY());
                        if (released != null) {
                            double offset = 0;
                            double start = 0;
                            // scroll x
                            if (getOrientation(event)
                            ...
                            // new dragdiff event
                            ChartGestureDragDiffEvent dragEvent = new ChartGestureDragDiffEvent(
                                    startEvent, lastEvent, event, start, offset,
                                    orient);
                            ...
                            lastEvent = event;
                            // save updated last
                            last = chartPanel.mouseXYToPlotXY(e.getX(),
                                    e.getY());
                        }
                    }
                }
