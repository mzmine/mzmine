...
/**
 * A graphics export dialog with preview and a panel for
 * {@link GraphicsExportParameters} and {@link ChartThemeParameters} to set teh
 * chart theme previous to export
 * 
...
public class GraphicsExportDialog extends JFrame {
    ...
    // theme
    private EStandardChartTheme theme = ChartThemeFactory
            .createBlackNWhiteTheme();
    // chooser
    ...
    public static void main(String[] args) {
        try {
            ...
            JFreeChart chart = ChartFactory.createXYLineChart("XY", "time (s)",
                    "intensity", data);
    ...
    public GraphicsExportDialog() {
        ...
        String[] formats = parameters
                .getParameter(GraphicsExportParameters.exportFormat)
                .getChoices();
        chooser.addChoosableFileFilter(
                new FileTypeFilter(formats, "Export images"));
        ...
        {
            StringParameter p = parameters
                    .getParameter(GraphicsExportParameters.path);
        ...
        {
            StringParameter p = parameters
                    .getParameter(GraphicsExportParameters.filename);
        ...
        }
        {
            ...
            pnSettingsLeft.setLayout(new BorderLayout(0, 0));
            {
                ...
                GridBagPanel pn = new GridBagPanel();
                {
                    ...
                    JComponent comp;
                    // add unit
                    p = (UserParameter) parameters
                            .getParameter(GraphicsExportParameters.unit);
                    ...
                    JFontSpecs master = (JFontSpecs) parametersAndComponents
                            .get(chartParam
                                    .getParameter(
                                            ChartThemeParameters.masterFont)
                                    .getName());
    ...
    protected void renewPreview() {
        ...
        try {
            ...
            if (parameters.isUseOnlyWidth()) {
                double height = (ChartLogics.calcHeightToWidth(chartPanel,
                        parameters.getWidthPixel(), false));

                DoubleParameter p = parameters
                        .getParameter(GraphicsExportParameters.height)
                        .getEmbeddedParameter();
                DoubleComponent c = ((OptionalParameterComponent<DoubleComponent>) parametersAndComponents
                        .get(p.getName())).getEmbeddedComponent();
                p.setValueToComponent(c, height);
                p.setValueFromComponent(c);

                chartPanel.setSize((int) parameters.getWidthPixel(),
                        (int) parameters.getHeightPixel());
            ...
            LOG.log(Level.SEVERE,
                    "Error while renewing preview of graphics export dialog ",
                    ex);
        }
    }
    ...
    protected void choosePath() {
        ...
        if (chooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            if (file.isDirectory()) {
                ...
                // data file selected
                // get folder
                txtPath.setText(FileAndPathUtil.getFolderOfFile(file)
                        .getAbsolutePath());
                // get filename
                txtName.setText(FileAndPathUtil.getFileNameFromPath(file));
                // get format without .
                String f = FilenameUtils.getExtension(file.getName())
                        .toUpperCase();
    ...
    private void handleMasterFontChanged(FontSpecs font) {
        ...
        for (Parameter<?> p : chartParam.getParameters()) {
            ...
            JFontSpecs component = (JFontSpecs) parametersAndComponents
                    .get(p.getName());
