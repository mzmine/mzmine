...
/**
 * The handler processes {@link ChartGestureEvent}s in a {@link Consumer}.
 * Pre-defined handlers and drag-difference handlers can be generated. It also
 * provides a static list of standard gesture handler definitions.
 * 
...
public class ChartGestureHandler {
    ...
    public ChartGestureHandler(ChartGesture gesture,
            Consumer<ChartGestureEvent> handler) {
    ...
    /**
     * The drag diff handler listens for PRESSED, DRAGGED and RELEASED events
     * and is called for every range difference between two events. is a range
     * difference between two drag events
     * 
     * @param handler
     *            A list of DragHandlers (predefined consumers)
     * @param key
     *            A list of Key filters which are connected to the handler list
     * @param entity
     *            The Entity filter
     * @param button
     *            The Button filter
     * @param orient
     *            Orientation of drag events (Horizontal or Vertical)
     * @param param
     *            Parameters for specific handlers
     */
    public static ChartGestureHandler createDragDiffHandler(
            DragHandler[] handler, Key[] key, Entity entity, Button button,
        ...
        try {
            ...
            return new ChartGestureDragDiffHandler(entity, button, key,
                    consumer);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
     ...
     * 
     * @param handler
     *            The predefined consumer
     * @param param
     *            Parameters for specific handlers
     * @return
     * @throws Exception
     */
    public static Consumer<ChartGestureDragDiffEvent> createDragDiffConsumer(
            DragHandler handler, Object[] param) throws Exception {
        switch (handler) {
        case SCROLL_AXIS:
            return e -> {
        ...
        case ZOOM_AXIS_INCLUDE_ZERO:
            return de -> {
                ValueAxis axis = de.getAxis();
                if (axis != null) {
                    double diff = de.getDiff() / axis.getRange().getLength()
                            * 4;
     ...
     * @return
     */
    public static ChartGestureHandler createHandler(Handler handler,
            ChartGesture g) {
     ...
     * @param g
     * @param param
     *            Parameters for specific handlers <br>
     * @return
     */
    public static ChartGestureHandler createHandler(Handler handler,
            final ChartGesture g, Object[] param) {
            ...
            newHandler = e -> {
                ...
                if (h != null) {
                    ...
                    if (range != null && range.length > 0 && range[0] != null) {
                        ValueAxis dom = e.getChart().getXYPlot()
                                .getDomainAxis();
        ...
        case AUTO_ZOOM_OPPOSITE_AXIS:
            newHandler = e -> {
                ...
                } else if (e.getGesture().getEntity()
                        .equals(Entity.DOMAIN_AXIS))
                    e.getChartWrapper().autoRangeAxis();
                else
                    e.getChartWrapper().autoDomainAxis();
            };
     ...
     * @return
     */
    public static List<GestureHandlerFactory> initStandardGestures(
            boolean axisDrag, boolean axisWheel, boolean titleRemover,
        ...
        if (axisDrag) {
            ...
            standardGestures.add(new DragGestureHandlerDef(
                    new DragHandler[] { DragHandler.SCROLL_AXIS,
                            DragHandler.SCROLL_AXIS_AND_AUTO_ZOOM,
                            DragHandler.ZOOM_AXIS_CENTER,
                            DragHandler.ZOOM_AXIS_CENTER,
                            DragHandler.AUTO_ZOOM_OPPOSITE_AXIS },
                    new Key[] { Key.NONE, Key.SHIFT, Key.CTRL, Key.CTRL_SHIFT,
                            Key.CTRL_SHIFT },
                    Entity.DOMAIN_AXIS, Button.BUTTON1, null, null));

            // Zoom range axis (include zero): DRAG
            standardGestures.add(new DragGestureHandlerDef(
                    new DragHandler[] { DragHandler.ZOOM_AXIS_INCLUDE_ZERO },
                    new Key[] { Key.ALL }, Entity.RANGE_AXIS, Button.BUTTON1,
                    null, null));
        }
        if (axisWheel) {
            ...
            standardGestures.add(new GestureHandlerDef(Handler.SCROLL_AXIS,
                    Entity.DOMAIN_AXIS, new Event[] { Event.MOUSE_WHEEL }, null,
                    Key.NONE, null));
            standardGestures.add(new GestureHandlerDef(
                    Handler.SCROLL_AXIS_AND_AUTO_ZOOM, Entity.DOMAIN_AXIS,
                    new Event[] { Event.MOUSE_WHEEL }, null, Key.SHIFT, null));
            standardGestures.add(new GestureHandlerDef(Handler.ZOOM_AXIS_CENTER,
                    Entity.DOMAIN_AXIS, new Event[] { Event.MOUSE_WHEEL }, null,
                    Key.CTRL, null));
            standardGestures.add(new GestureHandlerDef(Handler.ZOOM_AXIS_CENTER,
                    Entity.DOMAIN_AXIS, new Event[] { Event.MOUSE_WHEEL }, null,
                    Key.CTRL_SHIFT, null));
            standardGestures.add(new GestureHandlerDef(
                    Handler.AUTO_ZOOM_OPPOSITE_AXIS, Entity.DOMAIN_AXIS,
                    new Event[] { Event.MOUSE_WHEEL }, null, Key.CTRL_SHIFT,
                    null));
            // Zoom range axis (include zero): MOUSE WHEEL
            standardGestures.add(new GestureHandlerDef(
                    Handler.ZOOM_AXIS_INCLUDE_ZERO, Entity.RANGE_AXIS,
        ...
        if (zoomHistory) {
            ...
            standardGestures
                    .add(new GestureHandlerDef(Handler.PREVIOUS_ZOOM_HISTORY,
                            Entity.PLOT, new Event[] { Event.DOUBLE_CLICK },
                            Button.BUTTON1, Key.NONE, null));
            standardGestures
                    .add(new GestureHandlerDef(Handler.NEXT_ZOOM_HISTORY,
                            Entity.PLOT, new Event[] { Event.DOUBLE_CLICK },
                            Button.BUTTON1, Key.CTRL, null));
        }
        if (titleRemover) {
            // Remove titles, legends: CTRL + CLICK on titles
            standardGestures.add(new GestureHandlerDef(Handler.TITLE_REMOVER,
                    Entity.TITLE, new Event[] { Event.CLICK }, Button.BUTTON1,
                    Key.CTRL, null));
        }
        if (axisAutoRange) {
            // Auto zoom axes: DOUBLE CLICK on axis
            standardGestures.add(new GestureHandlerDef(Handler.AUTO_ZOOM_AXIS,
                    Entity.DOMAIN_AXIS, new Event[] { Event.DOUBLE_CLICK },
                    Button.BUTTON1, null, null));
            standardGestures.add(new GestureHandlerDef(Handler.AUTO_ZOOM_AXIS,
                    Entity.RANGE_AXIS, new Event[] { Event.DOUBLE_CLICK },
                    Button.BUTTON1, null, null));
        }
        return standardGestures;
    }
