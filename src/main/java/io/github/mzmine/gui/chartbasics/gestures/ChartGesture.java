...
public class ChartGesture {
    // ########################################################################
    // statics
    public static final ChartGesture ALL = new ChartGesture(Entity.ALL,
            Event.ALL, Button.ALL);
    //
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof ChartGesture))
            ...
            return this.getEntity().equals(g.getEntity())
                    && this.getEvent().equals(g.getEvent())
                    && this.getButton().equals(g.getButton())
                    && this.getKey().equals(g.getKey());
        }
    }

    /**
     * True if the argument fits to this gesture. The entity and event have to
     * fit
     * 
     * @param g
     */
    public boolean filter(ChartGesture g) {
        return this.getEntity().filter(g.getEntity())
                && (this.getButton() == null
                        || this.getButton().filter(g.getButton()))
                &&
                // any element fits to any element?
                Stream.of(this.getEvent()).anyMatch(e1 -> Stream
                        .of(g.getEvent()).anyMatch(e2 -> e1.filter(e2)))
                &&
                // key match?
    ...
    public String toString() {
        return (button == null ? "" : (button.toString() + " "))
                ...
                + event[0].toString() + " " + entity.toString();
    }
