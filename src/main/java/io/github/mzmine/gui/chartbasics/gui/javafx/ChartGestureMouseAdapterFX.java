 ...
 */
public class ChartGestureMouseAdapterFX
        implements GestureMouseAdapter, MouseHandlerFX {
    ...
    // listen for
    private EnumMap<Event, Boolean> listensFor = new EnumMap<Event, Boolean>(
            Event.class);
    //
    private String id;
     ...
     * 
     * @param id
     *            the handler ID ({@code null} not permitted).
     * @param parent
     *            the chart viewer.
     */
    ...
    private ChartEntity findChartEntity(MouseEventWrapper e) {
        ...
        if (lastEntity != null && x == lastEntityX && y == lastEntityY)
            ...
            ChartRenderingInfo info = chartViewer.getCanvas()
                    .getRenderingInfo();
    ...
    public void handleMouseClicked(ChartCanvas chartPanel, MouseEvent eOrig) {
        ...
        if (!e.isConsumed()) {
            ...
            if (e.getClickCount() == 2) {
                ...
                // handle event
                handleEvent(
                        new ChartGestureEvent(cw, e, entity, new ChartGesture(
                                gestureEntity, Event.DOUBLE_CLICK, button)));
