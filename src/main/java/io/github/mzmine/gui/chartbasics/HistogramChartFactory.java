...
public class HistogramChartFactory {
    private static final Logger logger = LoggerFactory
            .getLogger(HistogramChartFactory.class);

    private static GaussianCurveFitter fitter = GaussianCurveFitter.create()
     ...
     *            the data
     * @param gMin
     *            lower bound of Gaussian fit
     * @param gMax
     *            upper bound of Gaussian fit
     * @param sigDigits
     *            number of significant digits
     * @return double[] {normFactor, mean, sigma} as a result of
     *         GaussianCurveFitter.create().fit(obs.toList())
     */
    public static double[] gaussianFit(List<DataPoint> data, double gMin,
            double gMax) {
        // gaussian fit
     ...
     *         GaussianCurveFitter.create().fit(obs.toList())
     */
    public static double[] gaussianFit(XYSeries series, double gMin,
            double gMax) {
        // gaussian fit
        WeightedObservedPoints obs = new WeightedObservedPoints();
     ...
     *         GaussianCurveFitter.create().fit(obs.toList())
     */
    public static double[] gaussianFit(XYDataset data, int series, double gMin,
            double gMax) {
        // gaussian fit
        WeightedObservedPoints obs = new WeightedObservedPoints();
     ...
     * @return
     */
    public static double[] addGaussianFit(XYPlot plot, XYSeries series,
     ...
     * @return
     */
    public static double[] addGaussianFit(XYPlot plot, XYDataset data,
            int series, double gMin, double gMax, int sigDigits,
            boolean annotations) {
        double[] fit = gaussianFit(data, series, gMin, gMax);
        double minval = data.getX(series, 0).doubleValue();
        double maxval = data.getX(series, data.getItemCount(series) - 1)
                .doubleValue();
        return addGaussianFit(plot, fit, minval, maxval, gMin, gMax, sigDigits,
                annotations);
    }
     ...
     * @return
     */
    public static double[] addGaussianFit(XYPlot plot, double[] fit,
            double drawStart, double drawEnd, int sigDigits,
            boolean annotations) {
        return addGaussianFit(plot, fit, drawStart, drawEnd, drawStart, drawEnd,
                sigDigits, annotations);
    }
     ...
     */
    public static double[] addGaussianFit(XYPlot plot, double[] fit,
            double drawStart, double drawEnd, double gMin, double gMax,
        ...
        XYSeries gs = new XYSeries("Gaussian: " + mean + " \u00B1 " + sigma
                + " [" + norm + "] (mean \u00B1 sigma [normalisation])");
     ...
     * @param plot
     * @param fit
     *            Gaussian fit {normalisation factor, mean, sigma}
     */
    public static void addGaussianFitAnnotations(XYPlot plot, double[] fit) {
        ...
        BasicStroke s = new BasicStroke(1, BasicStroke.CAP_BUTT,
                BasicStroke.JOIN_MITER, 1, new float[] { 5f, 2.5f }, 0);
    ...
    public static JFreeChart createHistogram(double[] data, double binwidth,
        ...
        return createHistogram(data, binwidth, yAxisLabel, r.getLowerBound(),
                r.getUpperBound(), null);
    }

    public static JFreeChart createHistogram(double[] data, double binwidth,
            String yAxisLabel, double min, double max,
        ...
        if (data != null && data.length > 0) {
            ...
            XYSeries series = createHistoSeries(data, binwidth, min, max,
                    function);
    ...
    public static JFreeChart createHistogram(XYSeries series, double barwidth,
            String yAxisLabel) {
        XYSeriesCollection xydata = new XYSeriesCollection(series);
        XYBarDataset dataset = new XYBarDataset(xydata, barwidth);
        JFreeChart chart = ChartFactory.createXYBarChart("", yAxisLabel, false,
                "n", dataset, PlotOrientation.VERTICAL, true, true, false);
     ...
     * @param data
     * @param binwidth
     * @return A histogram array with length = datawidth/binwidth +1 (datawidth
     *         = max-min)
     */
    public static XYSeries createHistoSeries(double[] data, double binwidth) {
        Range r = getBounds(data);
        return createHistoSeries(data, binwidth, r.getLowerBound(),
                r.getUpperBound(), null);
    }
     ...
     * @param binwidth
     * @param min
     *            real minimum of data
     * @param max
     *            real maximum of data
     * @param function
     *            function to transform data axis
     * @return A histogram array with length = datawidth/binwidth +1 (datawidth
     *         = max-min)
     */
    public static XYSeries createHistoSeries(double[] data, double binwidth,
        ...
        for (int i = 0; i < bins.length; i++) {
            ...
            if (bins[i] > 0) {
                ...
                if (!peakStarted && i > 0)
                    addDPToSeries(series, bins, i - 1, binwidth, min, max,
                            function);
     ...
     *         = max-min)
     */
    public static XYSeries createHistoSeries(DoubleArrayList data,
    ...
    private static void addDPToSeries(XYSeries series, int[] bins, int i,
            double binwidth, double min, double max,
            DoubleFunction<Double> function) {
     ...
     *         = max-min)
     */
    public static List<DataPoint> createHistoList(DoubleArrayList data,
            double binwidth, double min, double max,
            DoubleFunction<Double> function) {
        double datawidth = (max - min);
        int cbin = (int) Math.ceil(datawidth / binwidth);
        ...
        for (int i = 0; i < bins.length; i++) {
            ...
            if (bins[i] > 0) {
                // add previous zero once
                if (!peakStarted && i > 0)
                    addDPToList(result, bins, i - 1, binwidth, min, max,
                            function);
    ...
    private static void addDPToList(List<DataPoint> list, int[] bins, int i,
            double binwidth, double min, double max,
            DoubleFunction<Double> function) {
        // adds a data point to the series
        double x = min + (binwidth / 2.0) + i * binwidth;
    ...
    /**
     * Adds a value to a Histogram array. bins array should have length =
     * datawidth/binwidth +1 (datawidth = max-min)
     * 
     * @param bins
     * @param value
     * @param binwidth
     ...
     *            minimum of
     */
    public static void addValueToHistoArray(int[] bins, double value,
    ...
    public static JFreeChart createHistogramOld(double[] data, int bin,
            String yAxisLabel, double min, double max) {
        if (data != null && data.length > 0) {
            HistogramDataset dataset = new HistogramDataset();
            dataset.addSeries("histo", data, bin, min, max);

            JFreeChart chart = ChartFactory.createHistogram("", yAxisLabel, "n",
    ...
    public static JFreeChart createHistogram(double[] data, String yAxisLabel) {
        ...
        return createHistogram(data, yAxisLabel, range.getLowerBound(),
                range.getUpperBound());
    }
    ...
    public static JFreeChart createHistogram(double[] data, String yAxisLabel,
        ...
        Range r = getBounds(data);
        return createHistogram(data, r.getLength() / bin, yAxisLabel, min, max,
                function);
    }
     ...
     * @param yAxisLabel
     * @param width
     *            automatic width if parameter is <=0
     * @return
     */
    public static JFreeChart createHistogram(double[] data, String yAxisLabel,
            double width) {
        Range range = getBounds(data);
        return createHistogram(data, yAxisLabel, width, range.getLowerBound(),
                range.getUpperBound());
    }
    ...
    public static JFreeChart createHistogram(double[] data, String yAxisLabel,
        ...
        if (width <= 0)
            ...
            return createHistogram(data, width, yAxisLabel, min, max,
                    val -> val);
        }
    }
     ...
     *            automatic width if parameter is <=0
     * @param function
     *            transform the data axis after binning
     * @return
     */
    public static JFreeChart createHistogram(double[] data, String yAxisLabel,
            double width, double min, double max,
