 ...
 * ChartGestures (with a set of standard chart gestures), ZoomHistory,
 * AxesRangeChangeListener, data export, graphics export,
 * 
...
public class EChartViewer extends ChartViewer {
    ...
    /**
     * Enhanced ChartPanel with extra scrolling methods, zoom history, graphics
     * and data export<br>
     ...
     * 
     * @param chart
     * @param graphicsExportMenu
     *            adds graphics export menu
     * @param standardGestures
     *            adds the standard ChartGestureHandlers
     * @param dataExportMenu
     *            adds data export menu
     */
    public EChartViewer(JFreeChart chart, boolean graphicsExportMenu,
            boolean dataExportMenu, boolean standardGestures) {
        this(chart, graphicsExportMenu, dataExportMenu, standardGestures,
                false);
    }
    ...
    public EChartViewer(JFreeChart chart, boolean graphicsExportMenu,
            boolean dataExportMenu, boolean standardGestures,
            boolean addZoomHistory, boolean stickyZeroForRangeAxis) {
        super(null);
    ...
    protected void addMenuItem(Menu parent, String title,
            EventHandler<ActionEvent> al) {
        MenuItem pngItem = new MenuItem(title);
        pngItem.setOnAction(al);
        parent.getItems().add(pngItem);
    }

    protected void addMenuItem(ContextMenu parent, String title,
            EventHandler<ActionEvent> al) {
        MenuItem pngItem = new MenuItem(title);
        pngItem.setOnAction(al);
        parent.getItems().add(pngItem);
    ...
    /**
     * Init ChartPanel Mouse Listener For MouseDraggedOverAxis event For
     * scrolling X-Axis und zooming Y-Axis0
     */
    private void initChartPanel() {
        ...
        if (chartPanel.getChart().getPlot() instanceof XYPlot) {
            ...
            if (stickyZeroForRangeAxis) {
                ValueAxis rangeAxis = chartPanel.getChart().getXYPlot()
                        .getRangeAxis();
                        ...
                        public void axisRangeChanged(ChartViewWrapper chart,
                            ...
                            if (axesRangeListener != null)
                                for (AxesRangeChangedListener l : axesRangeListener)
                                    l.axesRangeChanged(chart, axis, lastR,
                                            newR);
                        }
                    });
                }
    ...
    /**
     * Adds all standard gestures defined in
     * {@link ChartGestureHandler#getStandardGestures()}
     */
    public void addStandardGestures() {
        ...
        if (m != null) {
            m.clearHandlers();
            for (GestureHandlerFactory f : ChartGestureHandler
                    .getStandardGestures())
                m.addGestureHandler(f.createHandler());

            logger.log(Level.FINEST, "Added standard gestures: "
                    + m.getGestureHandlers().size());
        }
    }
    ...
    protected void addExportMenu(boolean graphics, boolean data) {
        ...
        if (data) {
            ...
            // Excel XY
            MenuExportToExcel exportXY = new MenuExportToExcel(
                    new XSSFExcelWriterReader(), "to Excel", this);
            export.getItems().add(exportXY);
            // clip board
            MenuExportToClipboard exportXYClipboard = new MenuExportToClipboard(
                    "to Clipboard", this);
    ...
    public Object[][] getDataArrayForExport() {
        if (getChart().getPlot() instanceof XYPlot
                && getChart().getXYPlot() != null
                && getChart().getXYPlot().getDataset() != null) {
            try {
                List<Object[]> modelList = new ArrayList<>();

                for (int d = 0; d < getChart().getXYPlot()
                        .getDatasetCount(); d++) {
                    ...
                    if (data instanceof XYZDataset) {
                        ...
                        for (int s = 0; s < series; s++) {
                            ...
                            y[0] = "";
                            z[0] = "";
                            x[1] = getChart().getXYPlot().getDomainAxis()
                                    .getLabel();
                            y[1] = getChart().getXYPlot().getRangeAxis()
                                    .getLabel();
                            z[1] = "z-axis";
                            // write data
                ...
                logger.log(Level.WARNING, "Cannot retrieve data for export",
                        ex);
                return null;
            }
        }
    ...
    /**
     * Returns the {@link ChartGestureMouseAdapter} alternatively for other
     * ChartPanel classes use:
     * 
     * <pre>
     * this.getCanvas().addAuxiliaryMouseHandler(handler);
     * </pre>
