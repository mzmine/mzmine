...
import io.github.mzmine.modules.dataanalysis.rtmzplots.RTMZDataset;
...
public class LogratioDataset extends AbstractXYZDataset implements RTMZDataset {
    ...
    public LogratioDataset(PeakList alignedPeakList, ParameterSet parameters) {
        ...
        RawDataFile groupOneFiles[] = parameters
                .getParameter(LogratioParameters.groupOneFiles).getValue();
        RawDataFile groupTwoFiles[] = parameters
                .getParameter(LogratioParameters.groupTwoFiles).getValue();
        PeakMeasurementType measurementType = parameters
                .getParameter(LogratioParameters.measurementType).getValue();
        ...
        for (int rowIndex = 0; rowIndex < numOfRows; rowIndex++) {
            ...
            if ((groupOnePeakIntensities.size() > 0)
                ...
                if (groupTwoAvg != 0.0)
                    logratio = (double) (Math.log(groupOneAvg / groupTwoAvg)
                            / Math.log(2.0));
