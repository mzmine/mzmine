...
public class ProjectionPlotPanel extends EChartPanel {
    ...
    public ProjectionPlotPanel(ProjectionPlotWindow masterFrame,
        ...
        spotRenderer.setDefaultToolTipGenerator(
                new ProjectionPlotToolTipGenerator(parameters));
        plot.setRenderer(spotRenderer);

        // Setup legend
        if (createLegend) {
            LegendItemCollection legendItemsCollection = new LegendItemCollection();
            for (int groupNumber = 0; groupNumber < dataset
                    .getNumberOfGroups(); groupNumber++) {
                ...
                legendItemsCollection.add(
                        new LegendItem(paramValue.toString(), "-", null, null,
