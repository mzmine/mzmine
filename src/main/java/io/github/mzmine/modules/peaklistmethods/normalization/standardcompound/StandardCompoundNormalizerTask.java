...
package io.github.mzmine.modules.peaklistmethods.normalization.standardcompound;
...
import io.github.mzmine.modules.peaklistmethods.normalization.linear.LinearNormalizerParameters;
...
public class StandardCompoundNormalizerTask extends AbstractTask {
  ...
  public StandardCompoundNormalizerTask(MZmineProject project, PeakList peakList,
  ...
  public void run() {
    ...
    logger.finest("Starting standard compound normalization of " + originalPeakList + " using "
        + normalizationType + " (total " + standardRows.length + " standard peaks)");
    ...
    rowIteration: for (PeakListRow row : originalPeakList.getRows()) {
      ...
      for (RawDataFile file : originalPeakList.getRawDataFiles()) {
        ...
        if (normalizationType == StandardUsageType.Nearest) {
          ...
          logger.finest("Normalizing row #" + row.getID() + " using standard peak " + standardPeak
    ...
    normalizedPeakList.addDescriptionOfAppliedTask(
        new SimplePeakListAppliedMethod("Standard compound normalization", parameters));
