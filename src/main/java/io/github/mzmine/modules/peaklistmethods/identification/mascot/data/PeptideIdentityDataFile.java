...
import io.github.mzmine.util.MascotParserUtils;
...
public class PeptideIdentityDataFile {
   ...
   * 
   * @param RawDataFile rawFile
   */
  public void addRawDataFile(RawDataFile rawFile) {
    if (!CollectionUtils.arrayContains(rawDataFiles.toArray(new RawDataFile[0]), rawFile))
  ...
  /**
   * Returns a Vector<ModificationPeptide>, containing the information about the searched
  ...
  /**
   * Returns a map, containing the information about the protein hits and corresponding query
   * (identified peptide).
   */
  ...
  public SerieIonType[] getIonSeriesRules() {
    if (ionSeriesRules == null)
      ionSeriesRules =
          MascotParserUtils.parseFragmentationRules((String) this.getParameters().get("RULES"));
  ...
  public double getPeptideMassErrorTolerance() {
    if (peptideMassErrorTol == -1)
      peptideMassErrorTol = Double.parseDouble((String) this.getParameters().get("TOL"));
