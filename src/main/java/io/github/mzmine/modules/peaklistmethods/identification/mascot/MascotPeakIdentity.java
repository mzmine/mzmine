...
public class MascotPeakIdentity extends SimplePeakIdentity {
   ...
   * PROPERTY_MASS = "Mass (Mr)"; private static final String PROPERTY_DELTA = "Delta"; private
   ...
   * (Iterable<ProteinHit>) peptide.getProteinHits()) { if (name.length() > 0) { name.append(' '); }
   * name.append(p.getAccession()); }
   * 
   * setPropertyValue(PROPERTY_NAME, name.toString());
   * 
   * final HashMap<Integer, String> varMods = new HashMap<Integer, String>(); for (final
   * Modification mod : peptide.getModifications()) { if (mod != null) {
   * varMods.put(mod.getModificationID(), mod.getType() + " (" + mod.getLocation() + ')'); } }
   * 
   * final int[] modSequences = peptide.getVariableModificationsArray(); String modSeqString = "";
   * for (int i = 0; i < modSequences.length; i++) { if (varMods.containsKey(modSequences[i])) {
   * modSeqString += varMods.get(modSequences[i]) + " [" + i + "], "; } }
   * 
   * setPropertyValue(PROPERTY_METHOD, "MASCOT search"); setPropertyValue(PROPERTY_PEPTIDE,
   * peptide.getSequence()); setPropertyValue(PROPERTY_FORMULA, peptide.getSequence());
