...
package io.github.mzmine.modules.peaklistmethods.filtering.peakfilter;
  ...
  public void run() {
    ...
    try {
      ...
      if (!isCanceled()) {
        ...
        if (parameters.getParameter(PeakFilterParameters.AUTO_REMOVE).getValue()) {
      ...
      for (int i = 0; i < totalRawDataFiles; i++) {
        ...
        if (filterByDuration) {
          final Range<Double> durationRange = parameters
              .getParameter(PeakFilterParameters.PEAK_DURATION).getEmbeddedParameter().getValue();
        ...
        if (filterByHeight) {
          final Range<Double> heightRange = parameters
              .getParameter(PeakFilterParameters.PEAK_HEIGHT).getEmbeddedParameter().getValue();
        ...
        if (filterByTailingFactor) {
          final Range<Double> tailingRange =
              parameters.getParameter(PeakFilterParameters.PEAK_TAILINGFACTOR)
                  .getEmbeddedParameter().getValue();
        ...
        if (filterByAsymmetryFactor) {
          final Range<Double> asymmetryRange =
              parameters.getParameter(PeakFilterParameters.PEAK_ASYMMETRYFACTOR)
                  .getEmbeddedParameter().getValue();
