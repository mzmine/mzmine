...
package io.github.mzmine.modules.peaklistmethods.peakpicking.shapemodeler;
...
class ShapeModelerTask extends AbstractTask {
  ...
  public ShapeModelerTask(MZmineProject project, PeakList peakList, ParameterSet parameters) {

    this.project = project;
    this.originalPeakList = peakList;
    this.parameters = parameters;

    shapeModelerType = parameters.getParameter(ShapeModelerParameters.shapeModelerType).getValue();
    suffix = parameters.getParameter(ShapeModelerParameters.suffix).getValue();
    removeOriginal = parameters.getParameter(ShapeModelerParameters.autoRemove).getValue();
    resolution = parameters.getParameter(ShapeModelerParameters.massResolution).getValue();
  ...
  public void run() {
    ...
    for (PeakListRow row : originalPeakList.getRows()) {
      ...
      try {
        for (Feature peak : row.getPeaks()) {
          ...
          Feature shapePeak = (Feature) shapeModelConstruct.newInstance(peak, scanNumbers,
    ...
    newPeakList.addDescriptionOfAppliedTask(new SimplePeakListAppliedMethod(
        "Peaks shaped by " + shapeModelerType + " function", parameters));
