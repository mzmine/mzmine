package io.github.mzmine.modules.io.tdfimport.brukerexample;
...
// to run this example the program must be able to find and load native timsdata.dll (windows, 64bit)
// copy that dll form the lib folder to the bin folder
public class TDFBinExample {
	...
	public static void main(String[] args) throws UnsupportedEncodingException {
		...
		long handle = TDFLibrary.INSTANCE.tims_open(args[0], 0);
		...
		if (0 != handle) {
			...
			TDFBinExample.MsMsData msmsData = new TDFBinExample.MsMsData();
			...
			TDFBinExample.MsMsDataFrame msmsDataFrame = new TDFBinExample.MsMsDataFrame();
			error_stat = TDFLibrary.INSTANCE
			...
			for (Map.Entry<Long, TDFBinExample.MsMsDataFrame.MsMsSpectrum> msmsEntry : msmsDataFrame.msmsSpectra
					.entrySet()) {
