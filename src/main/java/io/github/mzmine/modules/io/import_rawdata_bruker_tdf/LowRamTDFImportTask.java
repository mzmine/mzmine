...
import io.github.mzmine.modules.io.import_rawdata_bruker_tdf.datamodel.sql.TDFSegmentTable;
 ...
 */
public class LowRamTDFImportTask extends AbstractTask {

  private static final Logger logger = Logger.getLogger(LowRamTDFImportTask.class.getName());
  ...
  private TDFSegmentTable tdfSegmentTable;
   ...
   */
  public LowRamTDFImportTask(MZmineProject project, File file, IMSRawDataFile newMZmineFile,
  ...
  public LowRamTDFImportTask(MZmineProject project, File file, IMSRawDataFile newMZmineFile,
    ...
    if (!(newMZmineFile instanceof TdfImsRawDataFileImpl)) {
      throw new IllegalArgumentException("Wrong ims raw data file type.");
    }
  ...
  public void run() {
    ...
    tdfSegmentTable = new TDFSegmentTable();
    isMaldi = false;
    ...
    Set<UndloadedTDFFrame> frames = new LinkedHashSet<>();
    try {
      for (int i = 0; i < numFrames; i++) {
        ...
        UndloadedTDFFrame frame = tdfUtils.extractCentroidScanForTimsFrameLowRam(newMZmineFile,
    ...
    identifySegments((TdfImsRawDataFileImpl) newMZmineFile, tdfUtils, handle);
...
//    appendScansFromTimsSegment(tdfUtils, handle, frameTable, frames);
  ...
  private void readMetadata() {
    ...
    synchronized (org.sqlite.JDBC.class) {
      ...
      try {
        ...
        tdfSegmentTable.executeQuery(connection);
  ...
  private void identifySegments(TdfImsRawDataFileImpl rawDataFile, TDFUtils utils, long handle) {
    for (int i = 0; i < tdfSegmentTable.getNumberOfSegments(); i++) {
      int start = Math.toIntExact(tdfSegmentTable.getFirstFrameColumn().get(i));
      int end = Math.toIntExact(tdfSegmentTable.getLastFrameColumn().get(i));
      final double[] mobilities = utils.convertScanNumsToOneOverK0(handle, start,
          TDFUtils.createPopulatedArray(
              rawDataFile.getFrame(start - 1).getNumberOfMobilityScans()));
      rawDataFile.addSegmentMobility(Range.closed(start, end), mobilities);
    }
  }
