...
public class ScanSelectPanel extends JPanel implements ActionListener {
     ...
     * Create the panel.
     */
    public ScanSelectPanel(PeakListRow row, ScanSortMode sort,
            double noiseLevel, int minNumberOfSignals, String massListName) {
    ...
    public ScanSelectPanel(Scan[] scansEntry, ScanSortMode sort,
            double noiseLevel, int minNumberOfSignals, String massListName) {
        this(sort, noiseLevel, minNumberOfSignals, massListName);
    ...
    public ScanSelectPanel(ScanSortMode sort, double noiseLevel,
        ...
        pnButtons.setLayout(new MigLayout("", "[40px]",
                "[grow][40px][40px][40px][40px][40px][grow]"));
        ...
        if (doc instanceof AbstractDocument)
            ((AbstractDocument) doc)
                    .setDocumentFilter(new DocumentSizeFilter(20));
        ...
        txtPrecursorMZ.setToolTipText(
                "Exact (ideally calculated) precursor m/z of this ion");
        pnData.add(txtPrecursorMZ, "cell 0 4,growx,aligny top");
        txtPrecursorMZ.setColumns(9);

        btnFromScan = new JButton("From scan");
        btnFromScan.setToolTipText(
                "Precursor m/z and charge from scan or feature");
        ...
        lbMassListError = new JLabel(
                "ERROR with masslist selection: Wrong name or no masslist");
    ...
    public void setMZandChargeFromScan() {
        ...
        if (scans != null && !scans.isEmpty()) {
            ...
            txtPrecursorMZ.setText(
                    MZmineCore.getConfiguration().getMZFormat().format(mz));
        }
    }

    /**
     * Show the exclude from export button (check button). This button is
     * usually hidden when only one ScanSelectPanel is shown in the
     * {@link MSMSLibrarySubmissionWindow}
     * 
     * @param state
     *            linked to the visibility of the exclude button
     */
    ...
    public boolean checkParameterValues(List<String> messages) {
        ...
        if (mz <= 0)
            messages.add("Precursor m/z is not set properly: "
                    + txtPrecursorMZ.getText());
    ...
    private void applySelectionState() {
        ...
        if (chart != null) {
            chart.getChart().getXYPlot()
                    .setBackgroundPaint(selected ? Color.WHITE : errorColor);
        }
    }
    ...
    public void setFilter(String massListName, double noiseLevel,
        ...
        try {
            if (row != null) {
                if (isFragmentScan) {
                    ...
                    scans = ScanUtils.listAllFragmentScans(row, massListName,
                            noiseLevel, minNumberOfSignals, sort);
                } else {
                    // get most representative MS 1 scans of all features
                    scans = ScanUtils.listAllMS1Scans(row, massListName,
                            noiseLevel, minNumberOfSignals, sort);
                }
            } else if (scansEntry != null) {
                scans = ScanUtils.listAllScans(scansEntry, massListName,
                        noiseLevel, minNumberOfSignals, sort);
            }
            selectedScanI = 0;
    ...
    public void createChart() {
        ...
        if (scans != null && !scans.isEmpty()) {
            ...
            DataPointsDataSet data = new DataPointsDataSet("Data",
            ...
            // add error label
            JLabel error = new JLabel(MessageFormat.format(
                    "NO MS2 SPECTRA: 0 of {0} match the minimum criteria",
                    getTotalScans()));
    ...
    private void analyzeScan(Scan scan) {
        ...
        if (massList != null) {
            ...
            lbTIC.setText(MZmineCore.getConfiguration().getIntensityFormat()
                    .format(tic));
    ...
    public DataPoint[] getFilteredDataPoints() {
        if (scans != null && !scans.isEmpty()) {
            Scan scan = scans.get(selectedScanI);
            MassList massList = ScanUtils.getMassListOrFirst(scan,
                    massListName);
            if (massList != null)
                return ScanUtils.getFiltered(massList.getDataPoints(),
                        noiseLevel);
        }
        return null;
    }
    ...
    public DataPoint[] getFilteredDataPointsRemoved() {
        if (scans != null && !scans.isEmpty()) {
            ...
            MassList massList = ScanUtils.getMassListOrFirst(scan,
                    massListName);
            if (massList != null)
                return ScanUtils.getBelowThreshold(massList.getDataPoints(),
                        noiseLevel);
        }
        return null;
    }
