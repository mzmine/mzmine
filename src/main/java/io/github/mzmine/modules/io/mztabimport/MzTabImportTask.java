...
class MzTabImportTask extends AbstractTask {
    ...
    MzTabImportTask(MZmineProject project, ParameterSet parameters,
        ...
        this.importRawFiles = parameters
                .getParameter(MzTabImportParameters.importrawfiles).getValue();
    }
    ...
    public void run() {
        ...
        try {
            ...
            SortedMap<Integer, RawDataFile> rawDataFiles = importRawDataFiles(
                    mzTabFile);
            ...
            RawDataFile rawDataArray[] = rawDataFiles.values()
    ...
    private SortedMap<Integer, RawDataFile> importRawDataFiles(
        ...
        if (importRawFiles) {
            ...
            for (Entry<Integer, MsRun> entry : msrun.entrySet()) {
                File fileToImport = new File(
                        entry.getValue().getLocation().getPath());
                ...
                if (fileToImport.exists() && fileToImport.canRead())
                    ...
                    // mzTab file
                    File checkFile = new File(inputFile.getParentFile(),
                    ...
                    if (checkFile.exists() && checkFile.canRead())
                        ...
                        checkFile = new File(inputFile.getParentFile(),
                                fileToImport.getName() + ".gz");
                        if (checkFile.exists() && checkFile.canRead())
                            filesToImport.add(checkFile);
                        else {
            ...
            if (underlyingTasks.size() > 0) {
                MZmineCore.getTaskController()
                        .addTasks(underlyingTasks.toArray(new Task[0]));
            }
            ...
            /*
             * // Sort raw data files based on order in mzTab file MainWindow
             * mainWindow = (MainWindow) MZmineCore.getDesktop(); ProjectTree
             * rawDataTree = mainWindow.getMainPanel() .getRawDataTree(); final
             * RawDataTreeModel treeModel = ((MZmineProjectImpl)
             * project).getRawDataTreeModel(); final DefaultMutableTreeNode
             * rootNode = treeModel.getRoot(); int[] selectedRows = new
             * int[rootNode.getChildCount()]; for (int i = 1; i <
             * rootNode.getChildCount() + 1; i++) { selectedRows[i - 1] = i; }
        ...
        for (Entry<Integer, MsRun> entry : msrun.entrySet()) {
            ...
            String rawFileName = new File(
                    entry.getValue().getLocation().getPath()).getName();
    ...
    private void importSmallMolecules(PeakList newPeakList, MZTabFile mzTabFile,
        ...
        double mzExp = 0, abundance = 0, peak_mz = 0, peak_rt = 0,
                peak_height = 0, rtValue = 0;
        // int charge = 0;
        int rowCounter = 0;
        ...
        for (SmallMolecule smallMolecule : smallMolecules) {
            ...
            for (Entry<Integer, RawDataFile> rawDataEntry : rawDataFiles
                ...
                if (smallMolecule
                        .getAbundanceColumnValue(dataFileAssay) != null) {
                ...
                if (smallMolecule.getOptionColumnValue(dataFileAssay,
                        "peak_height") != null) {
                    peak_height = Double.parseDouble(
                            smallMolecule.getOptionColumnValue(dataFileAssay,
                                    "peak_height"));
                } else {
                    peak_height = 0.0;
                }
                ...
                Feature peak = new SimpleFeature(rawData, peak_mz, peak_rt,
                        ...
                        status, representativeScan, fragmentScan,
                        allFragmentScans, finalRTRange, finalMZRange,
                        finalIntensityRange);
