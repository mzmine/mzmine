...
public class WaveletMassDetector implements MassDetector {
    ...
    public DataPoint[] getMassValues(DataPoint originalDataPoints[],
            ParameterSet parameters) {
        double noiseLevel = parameters
                .getParameter(WaveletMassDetectorParameters.noiseLevel)
                .getValue();
        int scaleLevel = parameters
                .getParameter(WaveletMassDetectorParameters.scaleLevel)
                .getValue();
        double waveletWindow = parameters
                .getParameter(WaveletMassDetectorParameters.waveletWindow)
                .getValue();
    ...
    private DataPoint[] getMzPeaks(double noiseLevel,
        ...
        for (int ind = 0; ind <= stopInd; ind++) {
            ...
            while ((ind <= stopInd)
                ...
                if (waveletDataPoints[ind]
                        .getIntensity() > waveletDataPoints[peakMaxInd]
                                .getIntensity()) {
                    peakMaxInd = ind;
                }
