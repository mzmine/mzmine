...
public class ImsExpanderModule2 implements MZmineProcessingModule {

  @Override
  public @NotNull String getName() {
    return "Ims expander2";
  }

  @Override
  public @Nullable Class<? extends ParameterSet> getParameterSetClass() {
    return ImsExpanderParameters2.class;
  }
  ...
  public @NotNull ExitCode runModule(@NotNull MZmineProject project,
    ...
    final ModularFeatureList[] featureLists = parameters.getParameter(
        ImsExpanderParameters2.featureLists).getValue().getMatchingFeatureLists();

    final int threadsPerFlist = Math.max(2, numThreads / Math.max(featureLists.length, 1));

    final MemoryMapStorage storage = MemoryMapStorage.forFeatureList();
    for (ModularFeatureList featureList : featureLists) {
      tasks.add(new ImsExpanderTask2(storage, parameters, featureList, project, threadsPerFlist,
          moduleCallDate));
    }
