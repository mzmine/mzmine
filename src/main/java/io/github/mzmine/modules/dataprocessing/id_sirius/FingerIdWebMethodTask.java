...
/**
 * Class FingerIdWebMethodTask Wrapper around FingerIdWebMethod - calculates the
 * result as a separate MZmine task On end of execution - updates result
 * containers (ResultWindow or PeakListRow)
 */
public class FingerIdWebMethodTask extends AbstractTask {
    private static final Logger logger = LoggerFactory
            .getLogger(FingerIdWebMethodTask.class);
     ...
     * 
     * @param annotation
     *            - SiriusIonAnnotation to process by FingerId
     * @param experiment
     *            - contains necessary information for a web-request
     * @param candidatesAmount
     *            - amount of candidates to return from this task
     * @param window
     *            - one of possible result containers
     * @param row
     *            - one of possible result containers
     */
    private FingerIdWebMethodTask(SiriusIonAnnotation annotation,
            Ms2Experiment experiment, Integer candidatesAmount,
            ResultWindow window, PeakListRow row) {
        if (window == null && row == null)
            throw new RuntimeException(
                    "Only one result container can be null at a time");
    ...
    /**
     * Constructor for FingerIdWebMethodTask, used by
     ...
     * @param candidatesAmount
     * @param window
     *            - Result container for SingleRowIdentificationTask
     */
    public FingerIdWebMethodTask(SiriusIonAnnotation annotation,
            Ms2Experiment experiment, Integer candidatesAmount,
            ResultWindow window) {
     ...
     * @param candidatesAmount
     * @param row
     *            - Result container for PeakListIdentificationTask
     */
    public FingerIdWebMethodTask(SiriusIonAnnotation annotation,
            Ms2Experiment experiment, Integer candidatesAmount,
            PeakListRow row) {
    ...
    public String getTaskDescription() {
        return String.format("Processing element %s by FingerIdWebMethod",
                formula);
    }
    ...
    public void run() {
        ...
        try {
            method = new FingerIdWebMethod(experiment, annotation,
                    candidatesAmount);
            fingerResults = method.execute();
            logger.debug("Successfully processed {} by FingerWebMethod",
                    formula);
        } catch (RuntimeException e) {
            e.printStackTrace();
            logger.error(
                    "Error during processing FingerIdWebMethod --- return initial compound");
            fingerResults = null;
        ...
        if (fingerResults == null || fingerResults.size() == 0) {
            logger.info(
                    "No results found by FingerId Web Method for {}, adding initial compound",
                    formula);
    ...
    private void submitResults(List<IonAnnotation> results) {
        ...
        if (row != null) { // Update PeakListRow - if called from
            ...
            int quantity = (candidatesAmount > results.size()) ? results.size()
                    : candidatesAmount;
            PeakListIdentificationTask.addSiriusCompounds(results, row,
                    quantity);
        }
    }
