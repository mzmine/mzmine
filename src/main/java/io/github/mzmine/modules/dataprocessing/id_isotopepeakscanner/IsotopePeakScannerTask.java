...
/**
 * This will scan a feature list for calculated isotope patterns. This class
 * loops through every peak and checks if there are peaks within a specified RT
 * and m/z window. The m/z window is calculated by the maximum mass shift caused
 * by the isotope pattern. Every peak that fits the m/z window is also checked
 * for RT if specified => groupedPeaks. The next step checks for peaks inside
 * the specified m/z window around the expected isotope-pattern m/zs (current
 * peak mass + (isotope[i]-isotope[0])). Every peak that fits the criteria is
 * added to resultBuffer. Furthermore, every peak in the resultBuffer is rated
 * by m/z and intensity, if the rating is better that the previous one the peak
 * will be added as a candidate (by Candidates.java). If the algorithm was able
 * to find a peak inside the feature list for every expected isotope peak the
...
public class IsotopePeakScannerTask extends AbstractTask {
     ...
     * @param peak
     */
    IsotopePeakScannerTask(MZmineProject project, PeakList peakList,
        ...
        rtTolerance = parameters
                ...
                .getValue();
        checkIntensity = parameters
                .getParameter(IsotopePeakScannerParameters.checkIntensity)
                .getValue();
        mergeWidth = parameters
                .getParameter(IsotopePeakScannerParameters.mergeWidth)
                .getValue();
        minPatternIntensity = parameters
                .getParameter(IsotopePeakScannerParameters.minPatternIntensity)
                .getValue();
        element = parameters.getParameter(IsotopePeakScannerParameters.element)
                .getValue();
        minRating = parameters
                .getParameter(IsotopePeakScannerParameters.minRating)
                .getValue();
        suffix = parameters.getParameter(IsotopePeakScannerParameters.suffix)
                .getValue();
        checkRT = parameters.getParameter(IsotopePeakScannerParameters.checkRT)
                .getValue();
        minHeight = parameters
                .getParameter(IsotopePeakScannerParameters.minHeight)
                .getValue();
        charge = parameters.getParameter(IsotopePeakScannerParameters.charge)
                .getValue();
        accurateAvgIntensity = parameters
                .getParameter(IsotopePeakScannerParameters.massList).getValue();
        massListName = parameters
                .getParameter(IsotopePeakScannerParameters.massList)
                .getEmbeddedParameter().getValue();
        ratingChoice = parameters
                .getParameter(IsotopePeakScannerParameters.ratingChoices)
                .getValue();

        autoCarbon = parameters
                .getParameter(IsotopePeakScannerParameters.autoCarbonOpt)
                .getValue();
        ParameterSet autoCarbonParameters = parameters
                .getParameter(IsotopePeakScannerParameters.autoCarbonOpt)
                .getEmbeddedParameters();
        autoCarbonMin = autoCarbonParameters
                .getParameter(AutoCarbonParameters.minCarbon).getValue();
        autoCarbonMax = autoCarbonParameters
                .getParameter(AutoCarbonParameters.maxCarbon).getValue();
        autoCarbonMinPatternSize = autoCarbonParameters
                .getParameter(AutoCarbonParameters.minPatternSize).getValue();
        ...
        polarityType = (charge > 0) ? PolarityType.POSITIVE
                : PolarityType.NEGATIVE;
        charge = (charge < 0) ? charge * -1 : charge;

        if (getPeakListPolarity(peakList) != polarityType)
            logger.warning(
                    "PeakList.polarityType does not match selected polarity. "
                            + getPeakListPolarity(peakList).toString() + "!="
                            + polarityType.toString());

        if (suffix.equals("auto")) {
            suffix = "";
            if (scanType == ScanType.AUTOCARBON)
                suffix = " autoCarbon";
            suffix += "_-Pat=" + element + "-RT=" + checkRT + "-INT="
                    + checkIntensity + "-minR=" + minRating + "-minH="
                    + minHeight + "_results";
        }
    ...
    public void run() {
        ...
        resultPeakList = new SimplePeakList(peakList.getName() + suffix,
        ...
        for (int i = 0; i < totalRows; i++) {
            ...
            // pL[index].mz ->
            // pL[index].mz+maxMass
            ArrayList<PeakListRow> groupedPeaks = groupPeaks(rows, i,
                    diff[maxPatternIndex][diff[maxPatternIndex].length - 1]);
                ...
                for (int p = 0; p < diff.length; p++) {
                    ...
                    { // this is necessary bc there might be more than one
                      ...
                      // p = pattern index for autoCarbon
                        if (mzTolerance.checkWithinTolerance(
                                groupedPeaks.get(0).getAverageMZ() + diff[p][k],
                            ...
                            resultBuffer[p][k].addRow(j); // row in
                                                          // groupedPeaks[]
                            resultBuffer[p][k]
                                    .addID(groupedPeaks.get(j).getID());
                        }
                    }
                }
            }
            ...
            for (int p = 0; p < diff.length; p++)
                candidates[p] = new Candidates(diff[p].length, minHeight,
                        mzTolerance, pattern[p], massListName, plh, ratingType);
            ...
            for (int p = 0; p < diff.length; p++) {
                ...
                for (int k = 0; k < resultBuffer[p].length; k++) // reminder:
                                                                 ...
                                                                 // diff.length
                {
                    for (int l = 0; l < resultBuffer[p][k]
                            .getFoundCount(); l++) {
                        // k represents index resultBuffer[k] and thereby the
                        // isotope number
                        // l represents the number of results in resultBuffer[k]
                        candidates[p].checkForBetterRating(k,
                                groupedPeaks.get(0),
                                groupedPeaks.get(resultBuffer[p][k].getRow(l)),
                                minRating, checkIntensity);
            ...
            for (int p = 0; p < diff.length; p++) {
                if (trueBuffers[p]
                        && checkIfAllTrue(candidates[p].getCandidates())) {
            ...
            for (int p = 0; p < diff.length; p++) {
                ...
                if (accurateAvgIntensity
                        && candidates[p].getAvgAccAvgRating() > bestRating) {
                    bestPatternIndex = p;
                    bestRating = candidates[p].getAvgAccAvgRating();
                } else if (!accurateAvgIntensity
                        && candidates[p].getSimpleAvgRating() > bestRating) {
                    bestPatternIndex = p;
            ...
            PeakListRow original = getRowFromCandidate(candidates,
                    bestPatternIndex, 0, plh);
            if (original == null)
                continue;
            ...
            if (resultMap.containsID(parent.getID())) // if we can assign this
                ...
                addComment(parent,
                        resultMap.getRowByID(parent.getID()).getComment());
            ...
            if (carbonRange != 1)
                addComment(parent, "BestPattern: "
                        + pattern[bestPatternIndex].getDescription());

            rowBuffer.add(parent);

            DataPoint[] dp = new DataPoint[pattern[bestPatternIndex]
            ...
            {
                PeakListRow originalChild = getRowFromCandidate(candidates,
                        bestPatternIndex, k, plh);
                ...
                if (accurateAvgIntensity) {
                    ...
                    dp[k] = new SimpleDataPoint(child.getAverageMZ(),
                            child.getAverageHeight());
                }

                String average = "";
                if (accurateAvgIntensity) {
                    average = " AvgRating: " + round(
                            candidates[bestPatternIndex].getAvgRating(k), 3);
                }

                addComment(parent, "Intensity ratios: " + getIntensityRatios(
                        pattern[bestPatternIndex],
                        pattern[bestPatternIndex].getHighestDataPointIndex()));
                if (accurateAvgIntensity)
                    addComment(parent, " Avg pattern rating: " + round(
                            candidates[bestPatternIndex].getAvgAccAvgRating(),
                            3));
                else
                    addComment(parent, " pattern rating: " + round(
                            candidates[bestPatternIndex].getSimpleAvgRating(),
                            3));

                addComment(child, (parent.getID() + "-Parent ID"
                        + " m/z-shift(ppm): "
                        + round(((child.getAverageMZ() - parent.getAverageMZ())
                                - diff[bestPatternIndex][k])
                                / child.getAverageMZ() * 1E6, 2)
                        + " I(c)/I(p): "
                        + round(child.getAverageHeight()
                                / plh.getRowByID(candidates[bestPatternIndex]
                                        .get(pattern[bestPatternIndex]
                                                .getHighestDataPointIndex())
                                        .getCandID()).getAverageHeight(),
                                2)
                        + " Identity: "
                        + pattern[bestPatternIndex].getIsotopeComposition(k)
                        + " Rating: "
                        + round(candidates[bestPatternIndex].get(k).getRating(),
                                3)
                        + average));
            ...
            IsotopePattern resultPattern = new SimpleIsotopePattern(dp,
     ...
     * 
     * @param candidates
     * @param bestPatternIndex
     *            The index of the isotope pattern that was found to be the best
     *            fit for the detected pattern
     * @param peakIndex
     *            the index of the candidate peak, the feature list row should
     *            be extracted for.
     * @param plh
     * @return null if no peak with the given parameters exists, the specified
     *         feature list row otherwise.
     */
    private @Nullable PeakListRow getRowFromCandidate(
            @Nonnull Candidates[] candidates, int bestPatternIndex,
            int peakIndex, @Nonnull PeakListHandler plh) {
    ...
    /**
     * This calculates the isotope pattern using ExtendedIsotopePattern and
     * creates an ArrayList<Double> that will contain the mass shift for every
     * expected isotope peak relative to the one with the lowest mass.
     * 
     * @return
     */
    private double[][] setUpDiffAutoCarbon() {
        ...
        if (scanType == ScanType.AUTOCARBON) {
            ...
            for (int p = 0; p < carbonRange; p++) {
                ...
                try {
                    patternBuffer[p] = (ExtendedIsotopePattern) IsotopePatternCalculator
                            .calculateIsotopePattern(strPattern[p], 0.001,
                                    mergeWidth, charge, polarityType, true);
                    patternBuffer[p] = (ExtendedIsotopePattern) IsotopePatternCalculator
                            .removeDataPointsBelowIntensity(patternBuffer[p],
                                    minPatternIntensity);
                } catch (Exception e) {
            ...
            for (int p = 0; p < carbonRange; p++) {
                if (patternBuffer[p]
                        .getNumberOfDataPoints() >= autoCarbonMinPatternSize) {
                    sizeCounter++;
                } else {
            ...
            logger.info(
                    "about to add " + sizeCounter + " patterns to the scan.");
            ...
            for (int p = 0; p < carbonRange; p++) {
                ...
                for (int i = 0; i < pattern[addCounter]
                        .getNumberOfDataPoints(); i++) {
            ...
            pattern[0] = (ExtendedIsotopePattern) IsotopePatternCalculator
                    .calculateIsotopePattern(element, 0.001, mergeWidth, charge,
                            polarityType, true);
            pattern[0] = (ExtendedIsotopePattern) IsotopePatternCalculator
                    .removeDataPointsBelowIntensity(pattern[0],
                            minPatternIntensity);
     ...
     * @param pL
     * @param parentIndex
     *            index of possible parent peak
     * @param maxMass
     * @return will return ArrayList<PeakListRow> of all peaks within the range
     *         of pL[parentIndex].mz -> pL[parentIndex].mz+maxMass
     */
    private ArrayList<PeakListRow> groupPeaks(PeakListRow[] pL, int parentIndex,
            double maxDiff) {
                                                          ...
                                                          // parent peak itself
        {
            ...
            if (!(pL[i].getAverageMZ() > mz && pL[i].getAverageMZ() <= (mz
                    + maxDiff + mzTolerance.getMzTolerance()))) {
            ...
            if (checkRT
                    && !rtTolerance.checkWithinTolerance(rt, r.getAverageRT()))
                continue;
     ...
     * 
     * @param pattern
     *            IsotopePattern to calculate intensity ratios of
     * @param index
     *            DataPoint index to normalize the intesitys to
    ...
    private static String getIntensityRatios(IsotopePattern pattern,
        ...
        for (int i = 0; i < dp.length; i++)
            ...
            ratios += round((dp[i].getIntensity() / dp[index].getIntensity()),
                    2) + ":";
        ratios = (ratios.length() > 0)
                ? ratios.substring(0, ratios.length() - 1)
                : ratios;
        return ratios;
    }
     ...
     * 
     * @param row
     *            PeakListRow to add the comment to
     * @param str
     *            comment to be added
     */
    ...
    /**
     * Add feature list to project, delete old if requested, add description to
     * result
     */
    public void addResultToProject() {
        ...
        resultPeakList.addDescriptionOfAppliedTask(
                new SimplePeakListAppliedMethod("IsotopePeakScanner",
                        parameters));
    }
    ...
    private boolean checkParameters() {
        ...
        if (accurateAvgIntensity || ratingType == RatingType.TEMPAVG) {
            ...
            if (peakList.getNumberOfRawDataFiles() > 1) {
                setErrorMessage(
                        "The number of raw data files of feature list \""
                                + peakList.getName()
            ...
            if (foundMassList == false) {
                setErrorMessage("Feature list \"" + peakList.getName()
                        + "\" does not contain a mass list by the name of "
                        + massListName + ".");
                setStatus(TaskStatus.ERROR);
                return false;
            }
