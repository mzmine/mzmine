...
public class SGFilter implements ScanFilter {
    ...
    static {
        ...
        int[] a19Ints = { 269, 264, 249, 224, 189, 144, 89, 24, -51, -136, 0, 0,
                0 };
    ...
    public Scan filterScan(Scan scan, ParameterSet parameters) {
        ...
        int numOfDataPoints = parameters
                .getParameter(SGFilterParameters.datapoints).getValue();
        ...
        for (int spectrumInd = marginSize; spectrumInd < (oldDataPoints.length
                - marginSize); spectrumInd++) {

            // zero intensity data points must be left unchanged
            if (oldDataPoints[spectrumInd].getIntensity() == 0) {
                newDataPoints[spectrumInd
                        - marginSize] = oldDataPoints[spectrumInd];
                continue;
            }
