...
public class PeakListBlankSubtractionSingleTask extends AbstractTask {
    ...
    private static Logger logger = Logger
            .getLogger(PeakListBlankSubtractionSingleTask.class.getName());
    ...
    public PeakListBlankSubtractionSingleTask(
            PeakListBlankSubtractionParameters parameters, RawDataFile thisRaw,
            PeakListRow[] rows) {
        ...
        this.checkFoldChange = parameters
                .getParameter(PeakListBlankSubtractionParameters.foldChange)
                .getValue();
        this.foldChange = parameters
                .getParameter(PeakListBlankSubtractionParameters.foldChange)
                .getEmbeddedParameter().getValue();
        this.blankRaws = parameters
                .getParameter(
                        PeakListBlankSubtractionParameters.blankRawDataFiles)
                .getValue().getMatchingRawDataFiles();
        this.alignedFeatureList = parameters
                .getParameter(
                        PeakListBlankSubtractionParameters.alignedPeakList)
                .getValue().getMatchingPeakLists()[0];
        this.minBlankDetections = parameters
                .getParameter(PeakListBlankSubtractionParameters.minBlanks)
                .getValue();
    ...
    public void run() {
        ...
        if (rows.isEmpty()) {
            logger.info("Raw data file " + thisRaw.getName()
                    + " did not have any features in "
        ...
        for (PeakListRow row : rows) {
            ...
            if (checkFoldChange && (getIntensityIncrease(row, thisRaw,
                    blankRaws) > foldChange)) {
                finishedRows++;
                continue;
            }
        ...
        logger.finest(thisRaw.getName() + " - Removed " + featuresRemoved
                + " features.");
        setStatus(TaskStatus.FINISHED);
    }

    /**
     * Calculates the intensity increase of a sample feature compared to a
     * blank/control sample in an aligned feature list row.
     * 
     * @param row
     *            The aligned feature list row.
     * @param thisRaw
     *            The raw data file of the sample.
     * @param blankRaws
     *            The raw data file of blanks/control samples.
     ...
     * @param aligned
     * @param mp
     *            Matching parameters to look at.
     * @return A score between 1 and 0. 1 = best, 0 = worst
     */
    /*
     * private double getSimpleRowVsAlignedRowScores(PeakListRow peak,
     * PeakListRow aligned, MatchingParameter[] mp) {
     * 
     * double score = 1; for (MatchingParameter p : mp) { if (p ==
     * MatchingParameter.MZ) { score *= compare(peak.getAverageMZ(),
     * aligned.getAverageMZ()); } else if (p == MatchingParameter.RT) { score *=
     * compare(peak.getAverageRT(), aligned.getAverageRT()); } else if (p ==
