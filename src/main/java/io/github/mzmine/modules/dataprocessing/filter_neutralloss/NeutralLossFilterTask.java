...
/**
 * This module will scan for neutral losses in a very similar way to
 * IsotopePeakScanner.
 *
 */
public class NeutralLossFilterTask extends AbstractTask {
    ...
    NeutralLossFilterTask(MZmineProject project, PeakList peakList,
        ...
        mzTolerance = parameters
                .getParameter(NeutralLossFilterParameters.mzTolerance)
                .getValue();
        rtTolerance = parameters
                .getParameter(NeutralLossFilterParameters.rtTolerance)
                .getValue();
        minHeight = parameters
                .getParameter(NeutralLossFilterParameters.minHeight).getValue();
        molecule = parameters.getParameter(NeutralLossFilterParameters.molecule)
                .getValue();
        dMassLoss = parameters
                .getParameter(NeutralLossFilterParameters.neutralLoss)
                .getValue();
        suffix = parameters.getParameter(NeutralLossFilterParameters.suffix)
                .getValue();
        checkRT = parameters.getParameter(NeutralLossFilterParameters.checkRT)
                .getValue();
        ...
        if (!molecule.isEmpty()) {
            ...
            if (formula != null) {
                ...
                logger.info(
                        "Mass of molecule: " + molecule + " = " + dMassLoss);
            }
        }
    ...
    public void run() {
        ...
        if (suffix.equals("auto")) {
            if (molecule.equals(""))
                suffix = " NL: " + dMassLoss + " RTtol: "
                        + rtTolerance.getTolerance() + "_results";
            else
                suffix = " NL (" + molecule + "): " + dMassLoss + " RTtol: "
                        + rtTolerance.getTolerance() + "_results";
        }
        ...
        for (int i = 0; i < totalRows; i++) {
                ...
                { // this is necessary bc there might be more than one possible
                  ...
                  // candidate for
                    if (mzTolerance.checkWithinTolerance(
                            groupedPeaks.get(0).getAverageMZ() + diff.get(k),
            ...
            Candidates candidates = new Candidates(diff.size(), minHeight,
                    mzTolerance, plh);
                                                          ...
                                                          // = diff.size()
            {
                for (int l = 0; l < resultBuffer[k].getFoundCount(); l++) {
                    ...
                    candidates.get(k).checkForBetterRating(groupedPeaks, 0,
                            resultBuffer[k].getRow(l), diff.get(k), minRating);
                }
            }
            ...
            for (int k = 1; k < candidates.size(); k++) // we skip k=0 because
                                                        ...
                                                        // added before
            {
                PeakListRow originalParent = getRowFromCandidate(candidates, 1,
                        plh);
                ...
                if (resultMap.containsID(parent.getID()))
                    comParent += resultMap.getRowByID(parent.getID())
                            .getComment();
                ...
                addComment(child, " m/z shift(ppm): "
                        + round(((parent.getAverageMZ() - child.getAverageMZ())
                                - diff.get(1)) / parent.getAverageMZ() * 1E6, 2)
                        + " ");
    ...
    public void addResultToProject() {
        ...
        resultPeakList.addDescriptionOfAppliedTask(
                new SimplePeakListAppliedMethod("NeutralLossFilter",
                        parameters));
    }
    ...
    private @Nullable PeakListRow getRowFromCandidate(
            @Nonnull Candidates candidates, int peakIndex,
