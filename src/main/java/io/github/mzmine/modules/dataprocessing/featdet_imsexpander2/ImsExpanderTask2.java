...
public class ImsExpanderTask2 extends AbstractTask {

  private static final Logger logger = Logger.getLogger(ImsExpanderTask2.class.getName());
  ...
  public ImsExpanderTask2(@Nullable final MemoryMapStorage storage,
    ...
    this.flist = flist;
    useMzToleranceRange = parameters.getParameter(ImsExpanderParameters2.mzTolerance).getValue();
    mzTolerance = parameters.getParameter(ImsExpanderParameters2.mzTolerance).getEmbeddedParameter()
        .getValue();
    maxNumTraces =
        parameters.getValue(ImsExpanderParameters2.maxNumTraces) ? parameters.getParameter(
            ImsExpanderParameters2.maxNumTraces).getEmbeddedParameter().getValue()
            : Integer.MAX_VALUE;
    binWidth = parameters.getParameter(ImsExpanderParameters2.mobilogramBinWidth).getValue()
        ? parameters.getParameter(ImsExpanderParameters2.mobilogramBinWidth).getEmbeddedParameter()
  ...
  public void run() {
    ...
    final List<ExpandingTrace2> expandingTraces = new ArrayList<>(rows.stream().map(
        row -> new ExpandingTrace2((ModularFeatureListRow) row,
    ...
    final List<List<ExpandingTrace2>> subLists = Lists.partition(expandingTraces, tracesPerList);

    for (final List<ExpandingTrace2> subList : subLists) {
      final Frame firstFrame = (Frame) subList.get(0).getRow().getBestFeature().getFeatureData()
          .getSpectrum(0);
      final ExpandingTrace2 lastFrameTrace = subList.stream().max(
      ...
      final ArrayList<ExpandingTrace2> traces = new ArrayList<>(subList);
      ...
      final BinningMobilogramDataAccess mobilogramDataAccess = EfficientDataAccess.of(imsFile,
          binWidth);
      tasks.add(
          new ImsExpanderSubTask2(getMemoryMapStorage(), parameters, framesSubList, flist, traces,
    ...
    newFlist.getAppliedMethods().add(
        new SimpleFeatureListAppliedMethod(ImsExpanderModule2.class, parameters,
            getModuleCallDate()));
    final OriginalFeatureListOption handleOriginal = parameters.getParameter(
        ImsExpanderParameters2.handleOriginal).getValue();
