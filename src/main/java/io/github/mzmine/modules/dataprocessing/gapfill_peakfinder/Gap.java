...
public class Gap {
     ...
     */
    public Gap(PeakListRow peakListRow, RawDataFile rawDataFile,
     ...
     * 
     * @param lock
     *            A lock for multi threading purpose. null if single threaded
     */
    public void noMoreOffers() {
        ...
        if (bestPeakDataPoints != null) {
            ...
            DataPoint finalDataPoint[] = new DataPoint[bestPeakDataPoints
                    .size()];
            Range<Double> finalRTRange = null, finalMZRange = null,
            ...
            for (int i = 0; i < bestPeakDataPoints.size(); i++) {
                ...
                if (i == 0) {
                    ...
                    finalIntensityRange = finalIntensityRange
            ...
            int[] allMS2FragmentScanNumbers = ScanUtils.findAllMS2FragmentScans(
                    rawDataFile, finalRTRange, finalMZRange);
            ...
            SimpleFeature newPeak = new SimpleFeature(rawDataFile, mz, rt,
                    ...
                    allMS2FragmentScanNumbers, finalRTRange, finalMZRange,
                    finalIntensityRange);
    ...
    private boolean checkRTShape(GapDataPoint dp) {
        ...
        if (dp.getRT() < rtRange.lowerEndpoint()) {
            double prevInt = currentPeakDataPoints
                    .get(currentPeakDataPoints.size() - 1).getIntensity();
    ...
    private void checkCurrentPeak() {
        ...
        for (int i = 1; i < currentPeakDataPoints.size() - 1; i++) {
            ...
            if (rtRange.contains(currentPeakDataPoints.get(i).getRT())) {
                ...
                if ((currentPeakDataPoints.get(i)
                        .getIntensity() >= currentPeakDataPoints.get(i + 1)
                                .getIntensity())
                        && (currentPeakDataPoints.get(i)
                                .getIntensity() >= currentPeakDataPoints
                                        .get(i - 1).getIntensity())) {

                    if (currentPeakDataPoints.get(i)
                            .getIntensity() > currentMaxHeight) {
