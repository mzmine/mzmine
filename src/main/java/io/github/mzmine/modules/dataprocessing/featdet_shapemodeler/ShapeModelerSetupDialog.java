...
import io.github.mzmine.modules.dataprocessing.featdet_chromatogramdeconvolution.ChromatogramTICDataSet;
import io.github.mzmine.modules.dataprocessing.featdet_chromatogramdeconvolution.PeakPreviewComboRenderer;
import io.github.mzmine.modules.visualization.tic.PeakDataSet;
import io.github.mzmine.modules.visualization.tic.TICPlot;
import io.github.mzmine.modules.visualization.tic.TICToolBar;
...
public class ShapeModelerSetupDialog extends ParameterSetupDialog
    ...
    public ShapeModelerSetupDialog(Window parent, boolean valueCheckRequired,
        ...
        for (Parameter<?> p : smParameters.getParameters()) {
            ...
            if ((p.getName().equals(ShapeModelerParameters.suffix.getName()))
                    || (p.getName().equals(
                            ShapeModelerParameters.autoRemove.getName())))
                continue;
    ...
    private void loadPreviewPeak() {
        ...
        JComboBox<?> component = (JComboBox<?>) getComponentForParameter(
                ShapeModelerParameters.shapeModelerType);
        ShapeModel model = (ShapeModel) component.getSelectedItem();

        JFormattedTextField resolutionField = (JFormattedTextField) getComponentForParameter(
                ShapeModelerParameters.massResolution);
