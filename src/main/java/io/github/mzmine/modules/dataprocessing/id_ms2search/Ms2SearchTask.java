...
class Ms2SearchResult {
    ...
    public Ms2SearchResult(double score, String searchType,
            List<DataPoint> matchedIons) {
        this.score = score;
    ...
    public String getMatchedIonsAsString() {
        ...
        for (int i = 0; i < this.matchedIons.size(); i++) {
            returnString = returnString
                    + String.format("%.4f", this.matchedIons.get(i).getMZ())
                    + "_";
        }
...
class Ms2SearchTask extends AbstractTask {
    ...
    public Ms2SearchTask(ParameterSet parameters, PeakList peakList1,
        ...
        scoreThreshold = parameters
                .getParameter(Ms2SearchParameters.scoreThreshold).getValue();

        intensityThreshold = parameters
                .getParameter(Ms2SearchParameters.intensityThreshold)
                .getValue();

        minimumIonsMatched = parameters
                .getParameter(Ms2SearchParameters.minimumIonsMatched)
                .getValue();
    ...
    public void run() {
        ...
        for (int i = 0; i < rows1Length; i++) {
            for (int j = 0; j < rows2Length; j++) {
                ...
                searchResult = simpleMS2similarity(scanA, scanB,
                ...
                if (searchResult != null
                    ...
                    this.addMS2Identity(rows1[i], featureA, featureB,
                            searchResult);
    ...
    private void addMS2Identity(PeakListRow row1, Feature featureA,
        ...
        Ms2Identity newIdentity = new Ms2Identity(featureA, featureB,
                searchResult);
