...
public class ScanSmoothingTask extends AbstractTask {
    ...
    public ScanSmoothingTask(MZmineProject project, RawDataFile dataFile,
        ...
        this.timeSpan = parameters
                .getParameter(ScanSmoothingParameters.timeSpan).getValue();
        this.scanSpan = parameters
                .getParameter(ScanSmoothingParameters.scanSpan).getValue();
        this.mzTol = parameters
                .getParameter(ScanSmoothingParameters.mzTolerance).getValue();
        this.mzPoints = parameters
                .getParameter(ScanSmoothingParameters.mzPoints).getValue();
        this.minimumHeight = parameters
                .getParameter(ScanSmoothingParameters.minimumHeight).getValue();
        this.suffix = parameters.getParameter(ScanSmoothingParameters.suffix)
                .getValue();
        this.removeOriginal = parameters
                .getParameter(ScanSmoothingParameters.removeOld).getValue();
        ...
        try {
                ...
                if (scan != null) {
                    ...
                    if (timeSpan > 0 || scanSpan > 0) {
                        ...
                        for (si = i; si > 1; si--) {
                            ...
                            if (scanS == null || scanS.getRetentionTime() < rt
                                    - timeSpan / 2) {
                                break;
                            }
                        }
                            ...
                            for (k = 0; k < mzValues[ii].length; k++) {
                                ...
                                if (dp.getIntensity() > 0) { // only process
                                    ...
                                    for (j = 0; j < mzValues.length; j++) {
                                        ...
                                        if (mzValues[j].length > k && Math
                                                .abs(mzValues[j][k].getMZ()
                                                        - mz) < timeMZtol) {
                                            f = k;
                                        } else {
                    ...
                    if ((mzTol > 0 || mzPoints > 0)) {
                        ...
                        for (k = 0; k < newDP.length; k++) {
                            ...
                            if (newDP[k].getIntensity() > 0) {
                                for (si = k; si > 0
                                        && (newDP[si].getMZ() + mzTol >= mz
                                                || k - si <= mzPoints); si--)
                                    ;
                                for (sj = k; sj < newDP.length - 1
                                        && (newDP[sj].getMZ() - mzTol <= mz
                                                || sj - k <= mzPoints); sj++)
                                    ;
                                double sum = 0;
                                for (j = si; j <= sj; j++) {
            ...
            if (!isCanceled()) {
                ...
                if (mzpassed + timepassed < totalScans / 2) {
                    logger.warning(
                            "It seems that parameters were not properly set. Scans processed : time="
