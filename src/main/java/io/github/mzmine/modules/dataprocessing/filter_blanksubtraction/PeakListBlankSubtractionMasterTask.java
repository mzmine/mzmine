...
public class PeakListBlankSubtractionMasterTask extends AbstractTask {
    ...
    private static Logger logger = Logger
            .getLogger("PeakListBlankSubstractionTask");
    ...
    public PeakListBlankSubtractionMasterTask(MZmineProject project,
        ...
        this.blankSelection = parameters
                .getParameter(
    ...
    public void run() {
        ...
        PeakListRow[] rows = PeakUtils
                .copyPeakRows(alignedFeatureList.getRows());
        ...
        for (RawDataFile raw : alignedFeatureList.getRawDataFiles()) {
            ...
            AbstractTask task = new PeakListBlankSubtractionSingleTask(
                    parameters, raw, rows);
        ...
        for (int i = 0; i < rows.length; i++) {
            ...
            if (blankRawsList
                    .containsAll(Arrays.asList(row.getRawDataFiles()))) {
                onlyBlankRows++;
                rows[i] = null;
            }
        ...
        logger.finest("Removed " + onlyBlankRows
                + " rows that only existed in blankfiles.");

        PeakList result = new SimplePeakList(
                alignedFeatureList.getName() + " sbtrctd",
        ...
        result.addDescriptionOfAppliedTask(new SimplePeakListAppliedMethod(
                PeakListBlankSubtractionModule.MODULE_NAME, parameters));
    ...
    private boolean checkBlankSelection(PeakList aligned,
        ...
        for (int i = 0; i < blankRaws.length; i++) {
            ...
            if (contained == false) {
                logger.info("Peak list " + aligned.getName()
                        + " does not contain raw data files "
                        + blankRaws[i].getName());
                return false;
            }
        }
        ...
        logger.info("Peak list " + aligned.getName()
                + " contains all selected blank raw data files.");
        return true;
    }
