...
/**
 * SiriusThread class Allows to process PeakListIdentificationTask faster by
 * subthreading it.
 */
public class SiriusThread implements Runnable {
    private static final Logger logger = LoggerFactory
            .getLogger(SiriusThread.class);
     ...
     * @param latch
     */
    public SiriusThread(PeakListRow peakListRow, ParameterSet parameters,
            Semaphore semaphore, CountDownLatch latch,
            PeakListIdentificationTask task) {
        ionType = parameters
                .getParameter(PeakListIdentificationParameters.ionizationType)
                .getValue();
        range = parameters
                .getParameter(PeakListIdentificationParameters.ELEMENTS)
                .getValue();
        siriusCandidates = parameters
                .getParameter(
                        PeakListIdentificationParameters.CANDIDATES_AMOUNT)
                .getValue();
        fingeridCandidates = parameters
                .getParameter(
                        PeakListIdentificationParameters.CANDIDATES_FINGERID)
                .getValue();
        siriusTimer = parameters
                .getParameter(PeakListIdentificationParameters.SIRIUS_TIMEOUT)
                .getValue();
        ...
        MZTolerance mzTolerance = parameters
                .getParameter(PeakListIdentificationParameters.MZ_TOLERANCE)
                .getValue();
    ...
    public void run() {
        ...
        try {
            ...
            releaseResources();
            task.remoteCancel("Scan does not have requested Mass List name ["
                    + massListName + "]");
            return;
        }
        ...
        /*
         * Code block below gives SiriusMethod specific amount of time to be
         * executed, if it expires -> log error and continue
         */
        try {
            final SiriusIdentificationMethod method = new SiriusIdentificationMethod(
                    ms1list, ms2list, peakListRow.getAverageMZ(), siriusIon,
                    siriusCandidates, constraints, deviationPpm);
            ...
            if (ms2list.isEmpty()) {
                ...
                addSiriusCompounds(siriusResults, peakListRow,
                        siriusCandidates);
            } else {
                ...
                for (int index = 0; index < siriusCandidates; index++) {
                    SiriusIonAnnotation annotation = (SiriusIonAnnotation) siriusResults
                            .get(index);
                    try {
                        FingerIdWebMethodTask task = new FingerIdWebMethodTask(
                                annotation, experiment, fingeridCandidates,
                                peakListRow);
                        ...
                        logger.error(
                                "Processing of FingerWebMethods were interrupted");
            ...
            logger.error("Timeout on Sirius method expired, abort. Row id = {}",
                    peakListRow.getID());
        } catch (ExecutionException ce) {
            logger.error("Concurrency error during Sirius method.  Row id = {}",
                    peakListRow.getID());
        } finally {
    ...
    /**
     * Method for dealing with multithread resources 1) Release semaphore 2)
     * Count down barrier
     */
