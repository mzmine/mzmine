...
package io.github.mzmine.modules.dataprocessing.featuredetection_shapemodeler.peakmodels;
...
import io.github.mzmine.modules.dataprocessing.featuredetection_chromatogramdeconvolution.savitzkygolay.SGDerivative;
...
public class EMGPeakModel implements Feature {
  ...
  public EMGPeakModel(Feature originalDetectedShape, int[] scanNumbers, double[] intensities,
      double[] retentionTimes, double resolution) {
    ...
    rawDataPointsIntensityRange = originalDetectedShape.getRawDataPointsIntensityRange();
    ...
    previousHeight = calculateEMGIntensity(H, M, Dp, Ap, C, allRetentionTimes[0]);
    ...
    for (int i = 0; i < allRetentionTimes.length; i++) {
      ...
      shapeHeight = calculateEMGIntensity(H, M, Dp, Ap, C, allRetentionTimes[i]);
  ...
  /**
   * This method calculates the width of the chromatographic peak at half intensity
   * 
   * @param listMzPeaks
   * @param height
   * @param RT
   ...
   * 
   * 
   *         This calculation is based on work of Foley J.P., Dorsey J.G. "Equations for calculation
   *         of chromatographic figures of Merit for Ideal and Skewed Peaks", Anal. Chem. 1983, 55,
   *         730-737.
   */

  private void initializEMGParameters(double[] intensities, double[] retentionTimes,
      double retentionTime, double maxIntensity) {

    // Add lateral zero values to intensities array. This allows us to
    // get better result from 2nd derivative.
    int LATERAL_OFFSET = 5;
    double[] intensitiesWithZeros = new double[intensities.length + (LATERAL_OFFSET * 2)];
    // Left side zeros
    ...
    logger.finest("Size of extended zeros = " + intensitiesWithZeros.length + " original "
        + intensities.length);
    ...
    double[] secondDerivative = SGDerivative.calculateDerivative(intensitiesWithZeros, false, 12);
    ...
    for (int i = 1; i < secondDerivative.length; i++) {
      ...
      if (((secondDerivative[i - 1] < 0.0f) && (secondDerivative[i] > 0.0f))
        ...
        if ((secondDerivative[i - 1] < 0.0f) && (secondDerivative[i] > 0.0f)) {
          ...
          if ((crossZero == 1) && (retentionTimes[index] > retentionTime)) {
    ...
    /*
     * The inflection point represents the tangent of the curve. We use the secant function to
     * calculate the pendient of the tangent line.
     * 
     * m = f(x+h) - f(x) / h
     * 
     * where m is the pendient, f(x) is the intensity in inflection point and h is the time between
     * two points defined by secant line. In our case, the identified peak's rt and height are x+h
     * and f(x+h).
     */
    ...
    /*
     * Then calculate peak's width at base (Wb), using tagent line. Also obtains the value of b and
     * a (b/a = asymmetry factor). Using rect line formula
     * 
     * x2 = [(y2 - Y1) / m] + x1
     * 
     * where x2 represents the retention time value at 10% of total peak's height (y2).
     */
    // First left side
    double a = ((maxIntensity * 0.10d) - intensities[inflectionPointLeft]) / mLeft;
    a += retentionTimes[inflectionPointLeft];
    // Second right side
    double b = (intensities[inflectionPointRight] - (maxIntensity * 0.10d)) / mRight;
     ...
     * 
     * where Wb is the peak width at base (10% of height), b is the right side of the peak and a the
     * left side.
     * 
     ...
     * 
     * where sigma is standard deviation and tau is the time constant of exponential function.
     */
    double variance = Math.pow(Wb, 2) / (1.76d * (Math.pow(Ap, 2)) - 11.15d * (Ap) + 28.0d);
    ...
    /*
     * The relationship between skewness/excess is directly related to the proportion between
     * tau/sigma. So excess can be calculated by
     * 
     * C = [sigma * (Ap)] / tau
     * 
    ...
    /*
     * From the location of peak maximum (peak's retention time) we can calculate the center of the
     * Gaussian part using
     * 
  ...
  /**
   * 
   * This method calculates the height of Exponential Modified Gaussian function, using the
   * mathematical model proposed by Zs. Papai and T. L. Pap "Determination of chromatographic peak
   * parameters by non-linear curve fitting using statistical moments", The Analyst,
   ...
   * @return intensity
   */
  private static double calculateEMGIntensity(double H, double M, double Dp, double Ap, double C,
      double t) {
    double shapeHeight;
