 ...
 * 02111-1307, USA.
 */
package io.github.mzmine.modules.dataprocessing.featdet_adap3decompositionV1_5;
...
public class ADAP3DecompositionV1_5SetupDialog extends ParameterSetupDialog {
    ...
    public ADAP3DecompositionV1_5SetupDialog(Window parent,
            boolean valueCheckRequired, final ParameterSet parameters,
            String message) {
    ...
    protected void addDialogComponents() {
        ...
        pnlLabelsFields = GUIUtils.makeTablePanel(2, 2,
                new JComponent[] { new JLabel("Feature list"), comboPeakList,
        ...
        super.mainPanel.add(pnlVisible, 0, super.getNumberOfParameters() + 3, 2,
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (source.equals(preview)) {
            ...
            final PeakList peakList = (PeakList) comboPeakList
                    .getSelectedItem();

            final List<Peak> peaks = ADAP3DecompositionV1_5Task.getPeaks(
                    peakList,
                    parameterSet.getParameter(
                            ADAP3DecompositionV1_5Parameters.EDGE_TO_HEIGHT_RATIO)
                            .getValue(),
                    parameterSet.getParameter(
                            ADAP3DecompositionV1_5Parameters.DELTA_TO_HEIGHT_RATIO)
                            .getValue());
            ...
            retTimeMZPlot.updateData(
                    ArrayUtils.toPrimitive(
                            retTimeValues.toArray(new Double[size])),
                    ArrayUtils.toPrimitive(mzValues.toArray(new Double[size])),
                    ArrayUtils.toPrimitive(
                            colorValues.toArray(new Double[size])));

            // ------------------------
            // Calculate shape clusters
            // ------------------------

            final ComboClustersItem item = (ComboClustersItem) comboClusters
                    .getSelectedItem();
            ...
            if (item != null) {
                ...
                retTimeIntensityPlot.updateData(shapeClusters, colors, texts,
                        null);
            }
        }
     ...
     *            list od ADAP peaks
     * @param retTimeValues
     *            output of retention times
     * @param mzValues
     *            output of m/z-values
     * @param colorValues
     *            output of colors
     */

    private void retTimeCluster(List<Peak> peaks, List<Double> retTimeValues,
            List<Double> mzValues, List<Double> colorValues) {
        Double minDistance = parameterSet
                .getParameter(
                        ADAP3DecompositionV1_5Parameters.MIN_CLUSTER_DISTANCE)
                .getValue();
        Integer minSize = parameterSet
                .getParameter(ADAP3DecompositionV1_5Parameters.MIN_CLUSTER_SIZE)
                .getValue();
        Double minIntensity = parameterSet
                .getParameter(
                        ADAP3DecompositionV1_5Parameters.MIN_CLUSTER_INTENSITY)
                .getValue();
    ...
    private void shapeCluster(List<Peak> peaks,
        ...
        Double edgeToHeightRatio = parameterSet
                .getParameter(
                        ADAP3DecompositionV1_5Parameters.EDGE_TO_HEIGHT_RATIO)
                .getValue();
        Double deltaToHeightRatio = parameterSet
                .getParameter(
                        ADAP3DecompositionV1_5Parameters.DELTA_TO_HEIGHT_RATIO)
                .getValue();
        Boolean useIsShared = parameterSet
                .getParameter(ADAP3DecompositionV1_5Parameters.USE_ISSHARED)
                .getValue();
        Double shapeSimThreshold = parameterSet
                .getParameter(
                        ADAP3DecompositionV1_5Parameters.SHAPE_SIM_THRESHOLD)
                .getValue();
        Double minModelPeakSharpness = parameterSet
                .getParameter(
                        ADAP3DecompositionV1_5Parameters.MIN_MODEL_SHARPNESS)
                .getValue();
        List<Range<Double>> deprecatedMZValues = parameterSet
                .getParameter(ADAP3DecompositionV1_5Parameters.MZ_VALUES)
                .getValue();

        if (edgeToHeightRatio == null || deltaToHeightRatio == null
                || useIsShared == null || shapeSimThreshold == null
                || minModelPeakSharpness == null || deprecatedMZValues == null)
            return;

        List<Peak> modelPeakCandidates = TwoStepDecomposition.filterPeaks(peaks,
                useIsShared, edgeToHeightRatio, deltaToHeightRatio,
                minModelPeakSharpness, deprecatedMZValues);

        if (modelPeakCandidates.isEmpty())
            return;

        List<List<Peak>> clusters = TwoStepDecomposition
        ...
        for (List<Peak> cluster : clusters) {
            List<NavigableMap<Double, Double>> c = new ArrayList<>(
                    cluster.size());
            ...
            for (Peak peak : cluster) {
                ...
                texts.add(peak.getInfo() + "\nSharpness: "
                        + numberFormat.format(FeatureTools
                                .sharpnessYang(peak.getChromatogram())));
            }
            outClusters.add(c);
            outText.add(texts);
    ...
    private byte compareParameters(Parameter[] newValues) {
        ...
        final Set<Integer> secondPhaseIndices = new HashSet<>(
                Arrays.asList(new Integer[] { 4, 5, 6, 7, 8, 10 }));
