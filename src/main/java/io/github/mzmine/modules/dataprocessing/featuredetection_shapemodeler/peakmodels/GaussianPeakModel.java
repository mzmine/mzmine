...
public class GaussianPeakModel implements Feature {
  ...
  public GaussianPeakModel(Feature originalDetectedShape, int[] scanNumbers, double[] intensities,
    ...
    FWHM = calculateWidth(intensities, retentionTimes, resolution, rt, mz, height);
   ...
   * @return FWHM
   */
  private static double calculateWidth(double[] intensities, double[] retentionTimes,
      double resolution, double retentionTime, double mass, double maxIntensity) {
    ...
    for (int i = 0; i < intensities.length - 1; i++) {
      ...
      if (retentionTimes[i] < retentionTime) {
        if ((intensity <= halfIntensity) && (intensityPlus >= halfIntensity)) {
