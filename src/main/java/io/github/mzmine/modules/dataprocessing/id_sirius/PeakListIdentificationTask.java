...
public class PeakListIdentificationTask extends AbstractTask {
    ...
    private static final org.slf4j.Logger logger = LoggerFactory
            .getLogger(PeakListIdentificationTask.class);
    ...
    PeakListIdentificationTask(final ParameterSet parameters,
        ...
        int threadsAmount = parameters
                .getParameter(PeakListIdentificationParameters.THREADS_AMOUNT)
                .getValue();
        ...
        siriusCandidates = parameters
                ...
                .getValue();
        fingerCandidates = parameters
                .getParameter(
                        PeakListIdentificationParameters.CANDIDATES_FINGERID)
                .getValue();

        String massListName = parameters
                .getParameter(PeakListIdentificationParameters.MASS_LIST)
                .getValue();
    ...
    public double getFinishedPercentage() {
        ...
        return numItems == 0 ? 0.0
                : (double) (numItems - latch.getCount()) / (double) numItems;
    }
            ...
            try {
                ...
                for (int index = 0; !isCanceled() && index < numItems;) {
                    try {
                        ...
                        Thread th = new Thread(new SiriusThread(rows[index++],
                                parameters, semaphore, latch, this));
                        th.setDaemon(true);
                        th.start();
     ...
     * 
     * @param annotations
     *            list of IonAnnotations
     * @param row
     *            to add identities
     * @param amount
     *            of identities to be added from list
     */
    public synchronized static void addSiriusCompounds(
            @Nonnull List<IonAnnotation> annotations, @Nonnull PeakListRow row,
            int amount) {
        for (int i = 0; i < amount; i++) {
            SiriusIonAnnotation annotation = (SiriusIonAnnotation) annotations
                    .get(i);
