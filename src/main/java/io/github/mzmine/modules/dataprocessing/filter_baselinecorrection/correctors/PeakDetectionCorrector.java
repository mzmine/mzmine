...
/**
 * @description Feature Detection baseline corrector. Feature detection is done
 *              in several steps sorting out real peaks through different
 *              criteria. Peaks are removed from spectra and minimums and
 *              medians are used to smooth the remaining parts of the spectra.
...
public class PeakDetectionCorrector extends BaselineCorrector {
    ...
    public double[] computeBaseline(final RSessionWrapper rSession,
        ...
        int left = parameters
                .getParameter(PeakDetectionCorrectorParameters.LEFT).getValue();
        int right = parameters
                .getParameter(PeakDetectionCorrectorParameters.RIGHT)
                .getValue();
        int lwin = parameters
                .getParameter(PeakDetectionCorrectorParameters.LWIN).getValue();
        int rwin = parameters
                .getParameter(PeakDetectionCorrectorParameters.RWIN).getValue();
        double snminimum = parameters
                .getParameter(PeakDetectionCorrectorParameters.SNMINIMUM)
                .getValue();
        double mono = parameters
                .getParameter(PeakDetectionCorrectorParameters.MONO).getValue();
        double multiplier = parameters
                .getParameter(PeakDetectionCorrectorParameters.MULTIPLIER)
                .getValue();
