...
/**
 * This class is used to manage objects of the Candidate class and to calculate
 * an average rating if specified, after all peaks have been assigned. This
 * methods use an instance of a PeakListHandler since its easier to handle row
 * ids than row indexes.
 * 
 *
...
public class Candidates {
    ...
    public Candidates(int size, double minHeight, MZTolerance mzTolerance,
            IsotopePattern pattern, String massListName, PeakListHandler plh,
            RatingType ratingType) {
    ...
    public Candidate get(int index) {
        if (index >= candidate.length)
            throw new MSDKRuntimeException(
                    "Candidates.get(index): index > length");
    ...
    /**
     * sets isotope pattern, should not be used when there is a different number
     * of data points in the new pattern.
     * 
     * @param pattern
     */
     ...
     * 
     * @param index
     * @return returns average intensity of a single peak. -1.0 if calculation
     *         failed.
     */
     ...
     * @param index
     * @param parent
     *            row of monoisotopic mass
     * @param cand
     *            row of candidate peak
     * @param minRating
     *            minimum rating
     * @param checkIntensity
     * @return true if better, false if worse
     */
    public boolean checkForBetterRating(int index, PeakListRow parent,
            PeakListRow cand, double minRating, boolean checkIntensity) {
        if (ratingType == RatingType.HIGHEST)
            return candidate[index].checkForBetterRating(parent, cand, pattern,
            ...
            return candidate[index].checkForBetterRating(dpParent,
                    candidateIntensity, cand, pattern, index, minRating,
                    checkIntensity);
        } else
    ...
    public double calcTemporaryAvgRating(int index) {
        ...
        return candidate[index].recalcRatingWithAvgIntensities(
                candidate[0].getMZ(), pattern, index, avg);
    }
    ...
    public double[] calcAvgRatings() {
        ...
        for (int i = 0; i < candidate.length; i++) {
            avgRating[i] = candidate[i].recalcRatingWithAvgIntensities(
                    candidate[0].getMZ(), pattern, i, avgHeight);
        }
        return avgRating;
    }
     ...
     * 
     * @param ID
     * @return avg heights of all with the ids, but only if they are contained
     *         in same scans and mass lists
     */
    private double[] getAvgPeakHeights(int[] ID) {
        ...
        for (RawDataFile raw : raws) {
            ...
            for (int i = 0; i < scanNums.length; i++) {
                ...
                if (list == null
                        || !massListContainsEveryMZ(list, mzs, minHeight))
                    continue;
                ...
                for (int j = 0; j < mzs.length; j++) {
                    ...
                    DataPoint dp = getClosestDataPoint(points,
                            rows[j].getAverageMZ(), minHeight);
     ...
     * @param mz
     * @param minHeight
     * @return closest data point to given mz above minimum intensity in a given
     *         set of data points; null if no DataPoint over given intensity
     */
    private DataPoint getClosestDataPoint(DataPoint[] dp, double mz,
            double minHeight) {
        ...
        for (DataPoint p : dp)
            if (Math.abs(p.getMZ() - mz) < Math.abs(mz - n.getMZ())
                    && p.getIntensity() >= minHeight)
                n = p;
     ...
     *            MassList to check
     * @param mz
     *            array of mzs that need to be contained
     * @param minHeight
     *            minimum peak intensity
     * @return true or false
     */
    private boolean massListContainsEveryMZ(MassList list, double[] mz,
            double minHeight) {
        ...
        for (int i = 0; i < mz.length; i++) {
            ...
            for (DataPoint p : dps) {
                ...
                if (p.getIntensity() >= minHeight
                        && mzTolerance.checkWithinTolerance(p.getMZ(), mz[i]))
                    aboveMinHeight = true;
            }
     ...
     */
    private DataPoint[] getMassListDataPointsByMass(MassList list,
