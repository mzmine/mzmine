...
public class TICSumDataSet extends AbstractXYZDataset implements Task {
    ...
    public TICSumDataSet(final RawDataFile[] files, final Range<Double> rangeRT,
        ...
        this(files, rangeRT, rangeMZ, window,
                ((window != null) ? window.getPlotType()
    ...
    private String rawFileString() {
        return Arrays.stream(dataFiles).map(RawDataFile::getName)
    ...
    private void calculateValues() {
        ...
        for (int r = 0; r < dataFiles.length; r++) {
            ...
            for (int index = 0; status != TaskStatus.CANCELED
                ...
                } else if (plotType == TICPlotType.BASEPEAK
                        && basePeak != null) {
                ...
                if (!data.isEmpty()
                        && rt < data.get(data.size() - 1).getRetentionTime()
                                + RT_BIN) {
                    for (int i = 0; i < data.size(); i++) {
                        ...
                        if (matchesRT(rt, dp.getRetentionTime())) {
                            ...
                            // insert
                            data.add(i, new SummedTICDataPoint(rt, intensity,
                                    mzBasePeak, intensityBasePeak));
                            break;
                        }
                    }
                } else {
                    // add new data point
                    data.add(new SummedTICDataPoint(rt, intensity, mzBasePeak,
                            intensityBasePeak));
                }
