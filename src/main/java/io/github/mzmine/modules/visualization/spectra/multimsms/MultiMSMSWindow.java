...
public class MultiMSMSWindow extends JFrame {
    ...
    private void addMenu() {
        ...
        setCol.addActionListener(e -> {
            try {
                col = Integer
                        .parseInt(JOptionPane.showInputDialog("Columns", col));
        ...
        //
        addCheckBox(settings, "auto columns", autoCol, e -> setAutoColumns(
                ((JCheckBoxMenuItem) e.getSource()).isSelected()));
        addCheckBox(settings, "show one axis only", onlyShowOneAxis,
                e -> setOnlyShowOneAxis(
                        ((JCheckBoxMenuItem) e.getSource()).isSelected()));
     ...
     * @param direction
     */
    public void setData(PeakListRow[] rows, RawDataFile[] allRaw,
            RawDataFile raw, boolean createMS1, SortingProperty sorting,
     ...
     */
    public void setData(PeakListRow[] rows, RawDataFile[] allRaw,
            RawDataFile raw, boolean createMS1) {
        this.rows = rows;
        this.allRaw = allRaw;
        ...
        if (raw != null && !rawContainsFragmentation(raw)) {
            ...
            raw = Arrays.stream(rows).map(PeakListRow::getBestFragmentation)
    ...
    public void updateAllCharts() {
        ...
        if (createMS1) {
            ...
            for (PeakListRow r : rows) {
                ...
                if (f != null
                        && (best == null || f.getHeight() > best.getHeight())) {
                    best = f;
                }
            }
            if (best != null) {
                scan = best.getDataFile()
                        .getScan(best.getRepresentativeScanNumber());
                EChartPanel cp = SpectrumChartFactory.createScanChartPanel(scan,
            ...
            EChartPanel cp = PseudoSpectrum.createChartPanel(rows, raw, false,
                    "pseudo");
            if (cp != null) {
        ...
        // MS2 of all rows
        for (PeakListRow row : rows) {
            EChartPanel c = SpectrumChartFactory.createMSMSChartPanel(row, raw,
                    showTitle, showLegend, alwaysShowBest,
                    useBestForMissingRaw);
    ...
    public void renewCharts(ChartGroup group) {
        ...
        if (group != null && group.size() > 0) {
            realCol = autoCol ? (int) Math.floor(Math.sqrt(group.size())) - 1
                    : col;
            if (realCol < 1)
                realCol = 1;
            ...
            for (ChartViewWrapper cp : group.getList()) {
                ...
                axis.setVisible(
                        !onlyShowOneAxis || i >= group.size() - realCol);
