...
public class SpectralMatchPanel extends JPanel {
    ...
    static final ImageIcon iconAll = IconUtil
            .loadIconFromResources("icons/exp_graph_all.png");
    static final ImageIcon iconPdf = IconUtil
            .loadIconFromResources("icons/exp_graph_pdf.png");
    static final ImageIcon iconEps = IconUtil
            .loadIconFromResources("icons/exp_graph_eps.png");
    static final ImageIcon iconEmf = IconUtil
            .loadIconFromResources("icons/exp_graph_emf.png");
    static final ImageIcon iconSvg = IconUtil
            .loadIconFromResources("icons/exp_graph_svg.png");
    ...
    public SpectralMatchPanel(SpectralDBPeakIdentity hit) {
        ...
        // add title
        MigLayout l = new MigLayout("aligny center, wrap, insets 0 10 0 30",
                "[grow][]", "[grow]");
        JPanel boxTitlePanel = new JPanel();
        boxTitlePanel.setLayout(l);

        double simScore = hit.getSimilarity().getScore();
        Color gradientCol = ColorScaleUtil.getColor(MIN_COS_COLOR,
                MAX_COS_COLOR, MIN_COS_COLOR_VALUE, MAX_COS_COLOR_VALUE,
                simScore);
        ...
        String name = hit.getEntry().getField(DBEntryField.NAME).orElse("N/A")
                .toString();
        ...
        score.setToolTipText(
                "Cosine similarity of raw data scan (top, blue) and database scan: "
        ...
        String inchiString = hit.getEntry().getField(DBEntryField.INCHI)
                .orElse("N/A").toString();
        String smilesString = hit.getEntry().getField(DBEntryField.SMILES)
                .orElse("N/A").toString();
        ...
        JPanel panelCompounds = extractMetaData("Compound information",
                hit.getEntry(), DBEntryField.COMPOUND_FIELDS);

        // instrument info
        JPanel panelInstrument = extractMetaData("Instrument information",
                hit.getEntry(), DBEntryField.INSTRUMENT_FIELDS);
        ...
        metaDataPanelScrollPane
                .setPreferredSize(new Dimension(META_WIDTH + 20, ENTRY_HEIGHT));
     ...
     * 
     * @param param
     *            {@link SpectraIdentificationResultsParameters}
     */
    private void addExportButtons(ParameterSet param) {
        JButton btnExport = null;

        if (param.getParameter(SpectraIdentificationResultsParameters.all)
                .getValue()) {
            btnExport = new JButton(IconUtil.scaled(iconAll, ICON_WIDTH));
            btnExport.setMaximumSize(
                    new Dimension(btnExport.getIcon().getIconWidth() + 6,
        ...
        if (param.getParameter(SpectraIdentificationResultsParameters.pdf)
                .getValue()) {
        ...
        if (param.getParameter(SpectraIdentificationResultsParameters.emf)
                .getValue()) {
        ...
        if (param.getParameter(SpectraIdentificationResultsParameters.eps)
                .getValue()) {
        ...
        if (param.getParameter(SpectraIdentificationResultsParameters.svg)
                .getValue()) {
    ...
    public void exportToGraphics(String format) {
        ...
        if (chooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            try {
                ...
                SwingExportUtil.writeToGraphics(this, f.getParentFile(),
                        f.getName(), format);
                // save path
                param.setValue(FileAndPathUtil.eraseFormat(f));
            } catch (Exception ex) {
                logger.log(Level.WARNING,
                        "Cannot export graphics of spectra match panel", ex);
            } finally {
    ...
    private void coupleZoomYListener() {
        CombinedDomainXYPlot domainPlot = (CombinedDomainXYPlot) mirrorChart
                .getChart().getXYPlot();
        ...
        queryPlot.getRangeAxis()
                .addChangeListener(new AxisRangeChangedListener(null) {
                    @Override
                    public void axisRangeChanged(ChartViewWrapper chart,
                            ValueAxis axis, Range lastR, Range newR) {
                        rangeHasChanged(newR);
                    }
                });
        libraryPlot.getRangeAxis()
                .addChangeListener(new AxisRangeChangedListener(null) {
                    @Override
                    public void axisRangeChanged(ChartViewWrapper chart,
                            ValueAxis axis, Range lastR, Range newR) {
    ...
    private JPanel extractMetaData(String title, SpectralDBEntry entry,
            DBEntryField[] other) {
        ...
        if (inchiString != "N/A") {
            try {
                ...
                InChIToStructure inchiToStructure = factory.getInChIToStructure(
                        inchiString, DefaultChemObjectBuilder.getInstance());
