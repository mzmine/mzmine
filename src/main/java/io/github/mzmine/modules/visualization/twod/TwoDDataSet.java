...
class TwoDDataSet extends AbstractXYDataset implements Task {
    ...
    public void run() {
        ...
        for (int index = 0; index < totalScans; index++) {
            ...
            basePeaks[index] = (scanBasePeak == null ? 0
                    : scanBasePeak.getIntensity());
    ...
    double upperEndpointIntensity(Range<Double> rtRange, Range<Double> mzRange,
        ...
        if (searchRetentionTimes[startScanIndex] > rtRange.upperEndpoint()) {
            ...
            if (diffPrev < diffNext)
                ...
                return upperEndpointIntensity(startScanIndex, mzRange,
                        plotMode);
        }

        for (int scanIndex = startScanIndex; ((scanIndex < searchRetentionTimes.length)
                && (searchRetentionTimes[scanIndex] <= rtRange
    ...
    ArrayList getCentroidedDataPointsInRTMZRange(Range<Double> rtRange,
        ...
        for (int scanIndex = startScanIndex; ((scanIndex < searchRetentionTimes.length)
            ...
            DataPoint searchMZ = new SimpleDataPoint(mzRange.lowerEndpoint(),
                    0);
            ...
            for (int mzIndex = startMZIndex; ((mzIndex < dataPoints.length)
                    && (dataPoints[mzIndex].getMZ() <= mzRange
                            .upperEndpoint())); mzIndex++) {
