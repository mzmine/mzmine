...
public class SpectrumChartFactory {
    ...
    public static Scan getMSMSScan(PeakListRow row, RawDataFile raw,
            boolean alwaysShowBest, boolean useBestForMissingRaw) {
        Scan scan = null;
    ...
    public static PseudoSpectrumDataSet createMSMSDataSet(Scan scan,
            String label) {
        if (scan != null) {
            return createMSMSDataSet(scan.getPrecursorMZ(),
                    scan.getRetentionTime(), scan.getDataPoints(), label);
        } else
            return null;
    }

    public static PseudoSpectrumDataSet createMSMSDataSet(double precursorMZ,
            double rt, DataPoint[] dps, String label) {
        ...
        PseudoSpectrumDataSet series = new PseudoSpectrumDataSet(true,
                MessageFormat.format("MSMS for m/z={0} RT={1}{2}",
     ...
     * @return
     */
    public static PseudoSpectrumDataSet createMirrorDataSet(Scan scan,
            Scan mirror) {
        NumberFormat mzForm = MZmineCore.getConfiguration().getMZFormat();
        NumberFormat rtForm = MZmineCore.getConfiguration().getRTFormat();
        ...
        if (scan != null && mirror != null) {
            String label1 = MessageFormat.format("MSMS for m/z={0} RT={1}",
                    mzForm.format(scan.getPrecursorMZ()),
                    rtForm.format(scan.getRetentionTime()));
            String label2 = MessageFormat.format("MSMS for m/z={0} RT={1}",
                    mzForm.format(mirror.getPrecursorMZ()),
                    rtForm.format(mirror.getRetentionTime()));
            // data
            PseudoSpectrumDataSet data = new PseudoSpectrumDataSet(true, label1,
                    label2);
            // for each row
     ...
     * @return
     */
    public static EChartPanel createMirrorChartPanel(Scan scan, Scan mirror,
            String labelA, String labelB, boolean showTitle,
            boolean showLegend) {
        if (scan == null || mirror == null)
            return null;

        return createMirrorChartPanel(labelA, scan.getPrecursorMZ(),
                scan.getRetentionTime(), scan.getDataPoints(), labelB,
                mirror.getPrecursorMZ(), mirror.getRetentionTime(),
                mirror.getDataPoints(), showTitle, showLegend);
    }

    public static EChartPanel createMirrorChartPanel(String labelA,
            double precursorMZA, double rtA, DataPoint[] dpsA, String labelB,
            double precursorMZB, double rtB, DataPoint[] dpsB,
            boolean showTitle, boolean showLegend) {
        PseudoSpectrumDataSet data = dpsA == null ? null
                : createMSMSDataSet(precursorMZA, rtA, dpsA, labelA);
        PseudoSpectrumDataSet dataMirror = dpsB == null ? null
                : createMSMSDataSet(precursorMZB, rtB, dpsB, labelB);
        ...
        PseudoSpectraRenderer renderer1 = new PseudoSpectraRenderer(Color.BLACK,
                false);
        PseudoSpectraRenderer renderer2 = new PseudoSpectraRenderer(Color.BLACK,
                false);
        ...
        final XYPlot subplot2 = new XYPlot(dataMirror, null, rangeAxis2,
                renderer2);
    ...
    public static EChartPanel createScanChartPanel(Scan scan, boolean showTitle,
            boolean showLegend) {
        if (scan == null)
            return null;
        PseudoSpectrumDataSet dataset = createMSMSDataSet(scan, "");
        JFreeChart chart = createChart(dataset, showTitle, showLegend,
                scan.getRetentionTime(), scan.getPrecursorMZ());
    ...
    public static EChartPanel createChartPanel(JFreeChart chart) {
        ...
        PseudoSpectraItemLabelGenerator labelGenerator = new PseudoSpectraItemLabelGenerator(
                pn);
     ...
     * @return
     */
    public static EChartPanel createMSMSChartPanel(PeakListRow row,
            RawDataFile raw, boolean showTitle, boolean showLegend,
            boolean alwaysShowBest, boolean useBestForMissingRaw) {
    ...
    public static JFreeChart createChart(PseudoSpectrumDataSet dataset,
            boolean showTitle, boolean showLegend, double rt,
            double precursorMZ) {
        //
        if (dataset == null)
            return null;
        ...
        if (precursorMZ == 0)
            ...
            title = MessageFormat.format("MSMS for m/z={0} RT={1}",
                    mzForm.format(precursorMZ), rtForm.format(rt));
        ...
        PseudoSpectraRenderer renderer = new PseudoSpectraRenderer(Color.BLACK,
                false);
     ...
     * @param scan
     */
    private static void addPrecursorMarker(JFreeChart chart,
            double precursorMZ) {
        chart.getXYPlot().addDomainMarker(new ValueMarker(precursorMZ,
                Color.ORANGE, new BasicStroke(1.5f)));
    }
}
