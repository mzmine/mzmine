 ...
 *
 */
public class EHistogramDialog extends HistogramDialog
    ...
    private void addKeyBindings() {
        ...
        GUIUtils.registerKeyHandler(pn, KeyStroke.getKeyStroke("LEFT"), this,
                "PREVIOUS_PEAK");
        GUIUtils.registerKeyHandler(pn, KeyStroke.getKeyStroke("RIGHT"), this,
                "NEXT_PEAK");
        ...
        btnPrevious.setToolTipText(
                "Jump to previous distribution (use left arrow");
        btnPrevious.addActionListener(e -> jumpToPrevPeak());
        pnJump.add(btnPrevious);

        JButton btnNext = new JButton(">");
        btnNext.setToolTipText(
                "Jump to previous distribution (use right arrow");
        ...
        for (int i = data.getItemCount(0) - 1; i >= 1; i--) {
            ...
            if (mz < mid) {
                ...
                if (!started) {
                    ...
                    // intensity drops?
                    if (data.getYValue(0, i - 1) != 0
                            && data.getYValue(0, i) >= 100 && data.getYValue(0,
                                    i - 1) < data.getYValue(0, i)) {
    ...
    private void setZoomAroundPeakAt(int i) {
        ...
        if (keepRange) {
            ...
            plot.getDomainAxis().setRangeAboutValue(data.getXValue(0, i),
                    length);
        } else {
