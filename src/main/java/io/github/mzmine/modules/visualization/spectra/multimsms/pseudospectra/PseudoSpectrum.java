...
public class PseudoSpectrum {
    ...
    public static PseudoSpectrumDataSet createDataSet(PeakListRow[] group,
            RawDataFile raw, boolean sum) {
        // data
        ...
        for (PeakListRow row : group) {
            ...
            // sum -> heighest peak
            if (sum)
                series.addDP(row.getAverageMZ(), row.getBestPeak().getHeight(),
                        annotation);
            else {
    ...
    public static EChartPanel createChartPanel(PeakListRow[] group,
            RawDataFile raw, boolean sum, String title) {
