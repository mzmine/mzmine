...
/**
 * There will be a single instance of this class, use getInst(). This class
 * keeps track of every DataPointProcessingController and manages their
 * assignment to the TaskController. Default settings are loaded as set in the
 * preferences.
 * 
...
public class DataPointProcessingManager implements MZmineModule {
    ...
    private static Logger logger = Logger
            .getLogger(DataPointProcessingManager.class.getName());
    ...
    public ParameterSet getParameters() {
        if (parameters == null) {
            parameters = MZmineCore.getConfiguration()
                    .getModuleParameters(DataPointProcessingManager.class);
            processingParameters = parameters
                    .getParameter(
                            DataPointProcessingParameters.processingParameters)
                    .getValue();
        }
        return parameters;
    }
    ...
    /**
     * Adds a controller to the end of the waiting list. Automatically tries to
     * start a controller after addition.
     * 
     * @param controller
     *            Controller to add.
     */
    public void addController(
        ...
        logger.finest("Controller added to waiting list. (size = "
                + waiting.size() + ")");
        startNextController();
    }

    /**
     * Removes a controller from the waiting list. Use this if the current plot
     * has changed.
     * 
     * @param controller
     */
    public boolean removeWaitingController(
    ...
    /**
     * Adds a controller to the running list. Don't use publicly. Is called by
     * startNextController() if running.size < MAX_RUNNING.
     * 
     * @param controller
     */
    private void addRunningController(
            @Nonnull DataPointProcessingController controller) {
    ...
    /**
     * Removes a controller from the running list. Don't use publicly. Is called
     * by the DPControllerStatusListener in the startNextController method, if
     * the task was canceled or finished.
     * 
     * @param controller
     */
    private boolean removeRunningController(
            DataPointProcessingController controller) {
        // if (!running.contains(controller))
        // return;
    ...
    /**
     * Tries to start the next controller from the waiting list and adds a
     * listener to automatically start the next one when finished. Every
     * SpectraPlot will call this method after adding its controller.
     */
    ...
    /**
     * Cancels the execution of a specific controller or removes it from waiting
     * list.
     * 
     * @param controller
     */
    ...
    /**
     * Cancels the execution of all running controllers. Keep in mind that
     * calling only this method will only cancel the running ones, the currently
     * waiting ones will be started afterwards. Consider calling
     * removeAllWaiting first or use cancelAndRemoveAll() instead.
     */
     ...
     * 
     * @param controller
     * @return true or false if the controller is still in the running list. If
     *         this is false and the controller wants to execute() something
     *         went wrong. Check the setStatus method of the controller and the
     *         tasks.
     */
    ...
    public boolean isEnabled() {
        ...
        return getParameters()
                .getParameter(DataPointProcessingParameters.enableProcessing)
                .getValue();
    }
    ...
    /**
     * @param mslevel
     *            the ms level of the queue Clears the list of processing steps
     */
     ...
     *            New processing list.
     */
    public void setProcessingQueue(MSLevel mslevel,
            @Nonnull DataPointProcessingQueue list) {
        if (list != null)
            processingParameters.setQueue(mslevel, list);
        else
            logger.warning("The processing list for " + mslevel.toString()
                    + " was about to be set to null.");
    }
