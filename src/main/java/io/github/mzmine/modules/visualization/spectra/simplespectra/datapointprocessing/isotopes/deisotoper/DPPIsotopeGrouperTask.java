...
public class DPPIsotopeGrouperTask extends DataPointProcessingTask {
    ...
    private static Logger logger = Logger
            .getLogger(DPPIsotopeGrouperTask.class.getName());
    ...
    public DPPIsotopeGrouperTask(DataPoint[] dataPoints, SpectraPlot plot,
        ...
        mzTolerance = parameterSet
                .getParameter(DPPIsotopeGrouperParameters.mzTolerance)
                .getValue();
        monotonicShape = parameterSet
                .getParameter(DPPIsotopeGrouperParameters.monotonicShape)
                .getValue();
        maximumCharge = parameterSet
                .getParameter(DPPIsotopeGrouperParameters.maximumCharge)
                .getValue();
        // element =
        // parameterSet.getParameter(DPPIsotopeGrouperParameters.element).getValue();
        autoRemove = parameterSet
                .getParameter(DPPIsotopeGrouperParameters.autoRemove)
                .getValue();
        setDisplayResults(parameterSet
                .getParameter(DPPIsotopeGrouperParameters.displayResults)
                .getValue());
        setColor(parameterSet
                .getParameter(DPPIsotopeGrouperParameters.datasetColor)
                .getValue());
    }
    ...
    public void run() {
        ...
        IsotopePattern pattern = IsotopePatternCalculator
                .calculateIsotopePattern(element, 0.01, 1,
                        PolarityType.POSITIVE);
        double isotopeDistance = pattern.getDataPoints()[1].getMZ()
        ...
        for (int i = 0; i < sortedDataPoints.length; i++) {
            ...
            for (int charge : charges) {
                ...
                fitPattern(fittedPeaks, aPeak, charge, sortedDataPoints,
                        isotopeDistance);
            ...
            SimpleIsotopePattern newPattern = new SimpleIsotopePattern(
                    originalPeaks, IsotopePatternStatus.DETECTED,
                    aPeak.toString());

            sortedDataPoints[i].addResult(
                    new DPPIsotopePatternResult(newPattern, bestFitCharge));
            deisotopedDataPoints.add(sortedDataPoints[i]);
     ...
     */
    private void fitPattern(Vector<DataPoint> fittedPeaks, DataPoint p,
            int charge, DataPoint[] sortedPeaks, double isotopeDistance) {
     ...
     */
    private void fitHalfPattern(DataPoint p, int charge, int direction,
            Vector<DataPoint> fittedPeaks, DataPoint[] sortedPeaks,
    ...
    /*
     * @Override public String getTaskDescription() { return
     * "Deisotoping of Scan #" +
    ...
    public void displayResults() {
        if (isDisplayResults() || getController().isLastTaskRunning()) {
            getTargetPlot().addDataSet(new DPPResultsDataSet(
                    "Isotopes (" + getResults().length + ")", getResults()),
                    getColor(), false);
        }
    }

    /**
     * This method generates a single IsotopesDataSet from all detected isotope
     * patterns in the results.
     * 
     * @param dataPoints
     * @return
     */
    private IsotopesDataSet compressIsotopeDataSets(
            ProcessedDataPoint[] dataPoints) {
        ...
        for (ProcessedDataPoint dp : dataPoints) {
            if (dp.resultTypeExists(ResultType.ISOTOPEPATTERN)) {
                list.add(((DPPIsotopePatternResult) dp
        ...
        IsotopePattern full = new SimpleIsotopePattern(
                dpList.toArray(new DataPoint[0]), IsotopePatternStatus.DETECTED,
                "Isotope patterns");
