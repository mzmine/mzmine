...
class MsMsDataSet extends AbstractXYDataset implements Task {
    ...
    private int allScanNumbers[], msmsScanNumbers[], totalScans, totalmsmsScans,
            processedScans, allProcessedScans, processedColors, totalEntries,
            lastMSIndex;
    ...
    public void run() {
        ...
        for (int index = 0; index < totalScans; index++) {
            ...
            if (scan.getMSLevel() == 1) {
                ...
                if (intensityType == IntensityType.MS) {
                    ...
                    Range<Double> precursorMZRange = Range.closed(
        ...
        for (int index = 0; index < processedScans - 1; index++) {
            ...
            if (normalizationType == NormalizationType.all) {
                ...
                Double mzTolerance = precursorMZ * 10 / 1000000;
                Range<Double> precursorMZRange = Range.closed(
                        precursorMZ - mzTolerance, precursorMZ + mzTolerance);
     ...
     * 
     */
    public void highlightSpectra(double mz, MZTolerance searchMZTolerance,
            double minIntensity, boolean neutralLoss, Color c) {
        // mzRange
        searchMZTolerance.getToleranceRange(mz);
        Range<Double> precursorMZRange = searchMZTolerance
                    ...
                    if (ions > 1) {
                        ...
                        for (int x = 0; x < ions; x++) {
                            for (int y = 0; y < ions; y++) {
                                ionDiff[x][y] = Math
                                        .abs(selectedIons[x] - selectedIons[y]);
                            }
                        }
