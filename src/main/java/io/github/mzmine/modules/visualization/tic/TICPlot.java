...
public class TICPlot extends EChartPanel implements MouseWheelListener {
    ...
    public TICPlot(final ActionListener listener) {
        ...
        // Y-axis label.
        final String yAxisLabel = (this.plotType == TICPlotType.BASEPEAK)
                ? "Base peak intensity"
                : "Total ion intensity";
        ...
        if (listener instanceof TICVisualizerWindow) {
            ...
            RemoveFilePopupMenu = popupMenu.add(
                    new RemoveFilePopupMenu((TICVisualizerWindow) listener));
            popupMenu.addSeparator();
        ...
        if (event.getButton() == MouseEvent.BUTTON1) {
            ...
            if (event.getX() < 70) { // User clicked on Y-axis
                if (event.getClickCount() == 2) { // Reset zoom on Y-axis
                    ...
                    .getY() > this.getChartRenderingInfo().getPlotInfo()
    ...
    public synchronized void addPeakDataset(final XYDataset dataSet) {
        ...
        renderer.setSeriesPaint(0,
                PEAK_COLORS[numOfPeaks % PEAK_COLORS.length]);
        addDataSetRenderer(dataSet, renderer);
        numOfPeaks++;
    }
    ...
    public void setPlotType(final TICPlotType plotType) {
        ...
        if (this.plotType != plotType) {
            ...
            // Y-axis label.
            String yAxisLabel = (this.plotType == TICPlotType.BASEPEAK)
                    ? "Base peak intensity"
                    : "Total ion intensity";
