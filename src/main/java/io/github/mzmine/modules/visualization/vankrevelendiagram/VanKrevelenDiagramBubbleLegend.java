...
public class VanKrevelenDiagramBubbleLegend extends BorderPane {
  ...
  private void init() {
    double min = Arrays.stream(vanKrevelenDiagramXYZDataset.getBubbleSizeValues()).min()
        .orElse(0.0);
    double max = Arrays.stream(vanKrevelenDiagramXYZDataset.getBubbleSizeValues()).max()
        .orElse(0.0);
    ...
    Text valueType = new Text(
        vanKrevelenDiagramXYZDataset.getBubbleVanKrevelenDataType().getName());
    ...
    NumberFormat numberFormat = identifyNumberFormat(
        vanKrevelenDiagramXYZDataset.getBubbleVanKrevelenDataType());
  ...
  private NumberFormat identifyNumberFormat(
      VanKrevelenDiagramDataTypes bubbleVanKrevelenDiagramDataTypes) {
