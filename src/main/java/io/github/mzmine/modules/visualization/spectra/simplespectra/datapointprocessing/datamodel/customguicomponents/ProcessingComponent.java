...
import io.github.mzmine.modules.visualization.spectra.simplespectra.datapointprocessing.datamodel.customguicomponents.DPPMSLevelTreeNode;
import io.github.mzmine.modules.visualization.spectra.simplespectra.datapointprocessing.datamodel.customguicomponents.DPPModuleCategoryTreeNode;
import io.github.mzmine.modules.visualization.spectra.simplespectra.datapointprocessing.datamodel.customguicomponents.DPPModuleTreeNode;
import io.github.mzmine.modules.visualization.spectra.simplespectra.datapointprocessing.datamodel.customguicomponents.DisableableTreeCellRenderer;
...
public class ProcessingComponent extends JPanel implements ActionListener {
    ...
    private static final Logger logger = Logger
            .getLogger(ProcessingComponent.class.getName());
    ...
    public ProcessingComponent() {
        ...
        cbDiffMSn = GUIUtils.addCheckbox(this,
                "Use different settings for " + MSLevel.MSONE.toString()
                        + " and " + MSLevel.MSMS.toString(),
                this, "CBX_DIFFMSN",
                "If enabled, MS^1 and MS^n processing will use different parameters. The currently used settings are highlighted in green.");
        cbDiffMSn.setSelected(DataPointProcessingManager.getInst()
                .getProcessingParameters().isDifferentiateMSn());

        add(cbDiffMSn, BorderLayout.NORTH);

        setupTreeViews();
        split = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, tvProcessing,
                tvAllModules);
        initTreeListeners();
        ...
        GUIUtils.addButton(buttonPanel, "Set parameters", null, this,
                "BTN_SET_PARAMETERS");
        ...
        chooser.addChoosableFileFilter(
                new FileNameExtensionFilter("XML files", XML_EXTENSION));
    ...
    public void actionPerformed(ActionEvent e) {
        if (e.getActionCommand().equals("BTN_ADD")) {
            ...
            msLevelNodes[MSLevel.MSMS.ordinal()]
                    .setEnabled(cbDiffMSn.isSelected());
    ...
    private void setupTreeViews() {
        ...
        // on.
        DPPModuleCategoryTreeNode[] moduleCategories = new DPPModuleCategoryTreeNode[ModuleSubCategory
                .values().length];
        for (int i = 0; i < moduleCategories.length; i++) {
            moduleCategories[i] = new DPPModuleCategoryTreeNode(
                    ModuleSubCategory.values()[i]);
        ...
        for (MZmineModule module : moduleList) {
            if (module instanceof DataPointProcessingModule) {
                ...
                for (DPPModuleCategoryTreeNode catItem : moduleCategories) {
                    if (dppm.getModuleSubCategory()
                            .equals(catItem.getCategory())) {
    ...
    private void addSelectedModule() {
        ...
        if (selected instanceof DPPModuleTreeNode) {
            ...
            logger.finest("Cannot add item " + selected.toString() + " to "
                    + this.getName() + ".");
        }
    }
    ...
    private void addModule(@Nonnull DPPModuleTreeNode node,
            @Nonnull DPPMSLevelTreeNode target) {

        if (nodeContains(target, node)) {
            logger.finest("Cannot add module "
                    + ((DPPModuleTreeNode) node).getModule().getName()
        ...
        if (!moduleFitsMSLevel(node.getModule(), target)) {
            logger.warning("The use of module \"" + node.getModule().getName()
                    ...
                    + target.getMSLevel().toString()
                    + "\". This might lead to unexpected results.");
        }

        DefaultMutableTreeNode root = (DefaultMutableTreeNode) tvProcessing
                .getModel().getRoot();
        ((DefaultTreeModel) tvProcessing.getModel()).insertNodeInto(node,
                target, target.getChildCount());
    ...
    private void removeModule() {
        ...
        if (selected instanceof DPPModuleTreeNode) {
            DefaultMutableTreeNode parent = (DefaultMutableTreeNode) selected
                    .getParent();
            if (parent instanceof DPPMSLevelTreeNode
                    && ((DPPMSLevelTreeNode) parent).isEnabled()) {
                parent.remove(selected);
                logger.finest("Removed module "
                        + ((DPPModuleTreeNode) selected).getModule().getName()
                        + " from processing list.");
            }
        } else {
            logger.finest("Cannot remove item " + selected.toString()
                    + " from processing list.");
        }
        ((DefaultTreeModel) tvProcessing.getModel()).reload();
        expandAllNodes(tvProcessing);
    ...
    public @Nonnull DPPParameterValueWrapper getValueFromComponent() {
        ...
        for (MSLevel mslevel : MSLevel.cropValues())
            value.setQueue(mslevel,
                    getProcessingQueueFromNode(getNodeByMSLevel(mslevel)));

        return value;
    }

    /**
     * Creates DataPointProcessingQueues from the items currently in the tree
     * view.
     * 
    ...
    public @Nonnull DataPointProcessingQueue getProcessingQueueFromNode(
        ...
        do {
            DefaultMutableTreeNode item = (DefaultMutableTreeNode) nodes
                    .nextElement();
    ...
    /**
     * Creates a MZmineProcessingStep<DataPointProcessingModule> from an
     * DPPModuleTreeItem.
     * 
     * @param item
     *            Tree item.
    ...
    private @Nonnull MZmineProcessingStep<DataPointProcessingModule> createProcessingStep(
        ...
        return new MZmineProcessingStepImpl<>(item.getModule(),
                item.getParameters());
    }
    ...
    private void sendValueWrapper() {
        ...
        DataPointProcessingManager manager = DataPointProcessingManager
                .getInst();
    ...
    /**
     * Creates a collection of DPPModuleTreeItem from a queue. Can be used after
     * loading a queue from a file.
     * 
     * @param queue
     *            The queue.
    ...
    /**
     * Convenience method to publicly set the items of the processing list from
     * the tree view. Used to set the default queue, if set, loaded by the
     * manager's constructor.
     * 
     * @param queue
     */
    public void setTreeViewProcessingItemsFromQueue(
            @Nullable DataPointProcessingQueue queue, MSLevel level) {
        logger.info("Loading queue into tvProcessing...");
        DPPMSLevelTreeNode targetNode = getNodeByMSLevel(level);

        targetNode.removeAllChildren();
        Collection<DPPModuleTreeNode> moduleNodes = createTreeItemsFromQueue(
                queue);
     ...
     * @param valueWrapper
     */
    public void setValueFromValueWrapper(
            DPPParameterValueWrapper valueWrapper) {

        cbDiffMSn.setSelected(valueWrapper.isDifferentiateMSn());

        for (MSLevel mslevel : MSLevel.cropValues())
            setTreeViewProcessingItemsFromQueue(valueWrapper.getQueue(mslevel),
                    mslevel);
    ...
    /**
     * Convenience method to check if the module's ms level is applicable for
     * this component.
     * 
     * @param module
     * @return
    ...
    private @Nullable DefaultMutableTreeNode getSelectedItem(
            @Nonnull JTree tree) {
