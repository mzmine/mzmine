...
/**
 * This class will control the tasks to process the DataPoints in a
 * SpectraWindow. Every SpectraPlot is meant to have an instance of this class
 * associated with it.
 * 
...
public class DataPointProcessingController {
    ...
    private Logger logger = Logger
            .getLogger(DataPointProcessingController.class.getName());
    ...
    /**
     * This is used to cancel the execution of this controller. It is set to
     * NORMAL in the constructor.
     */
    ...
    /**
     * 
     * @return Results of this task. Might be null, make sure to check the
     *         status first!
     */
    ...
    /**
     * Convenience method to cancel the execution of this controller. The
     * manager will listen to this change by its DPControllerStatusListener. The
     * ControllerStatus is changed in the execute() method of this controller.
     */
    ...
    /**
     * This will execute the modules associated with the plot. It will start
     * with the first one and execute the following ones afterwards
     * automatically. This method is called by the public method execute(). The
     * status listener in this method starts the next task recursively after the
     ...
     * @param plot
     */
    private void execute(DataPoint[] dp,
            MZmineProcessingStep<DataPointProcessingModule> step,
            SpectraPlot plot) {
        ...
        if (!step.getParameterSet().checkParameterValues(err)) {
            ...
            setStatus(ControllerStatus.CANCELED);
            logger.warning(
                    step.getModule().getName() + " - Not all parameters set."
        ...
        if (step.getModule() instanceof DataPointProcessingModule) {
            ...
            Task t = ((DataPointProcessingModule) inst).createTask(dp,
                    parameters, plot, this, new TaskStatusListener() {
                        @Override
                        public void taskStatusChanged(Task task,
                            ...
                            if (!(task instanceof DataPointProcessingTask)) {
                                ...
                                logger.warning(
                                        "This should have been a DataPointProcessingTask.");
                                return;
                            }
                            ...
                            case FINISHED:
                                if (queue.hasNextStep(step)) {
                                    if (DataPointProcessingManager.getInst()
                                            .isRunning(
                                                    ((DataPointProcessingTask) task)
                                                            .getController())) {

                                        MZmineProcessingStep<DataPointProcessingModule> next = queue
                                                .getNextStep(step);

                                        // pass results to next task and start
                                        // recursively
                                        ProcessedDataPoint[] result = ((DataPointProcessingTask) task)
                                                .getResults();
                                        ((DataPointProcessingTask) task)
                                                .displayResults();
                                    ...
                                    setResults(((DataPointProcessingTask) task)
                                            .getResults());
                                    ((DataPointProcessingTask) task)
                                            .displayResults();
                                    setStatus(ControllerStatus.FINISHED);
    ...
    /**
     * Executes the modules in the PlotModuleCombo to the plot with the given
     * DataPoints. This will be called by the DataPointProcessingManager. This
     * starts the first module, which recursively starts the following ones
     * after finishing.
     */
    public void execute() {
        if (queue == null || queue.isEmpty() || plot == null) {
        ...
        MZmineProcessingStep<DataPointProcessingModule> first = queue
                .getFirstStep();
        if (first == null) {
            setStatus(ControllerStatus.ERROR);
            return;
    ...
    public boolean addControllerStatusListener(
            DPPControllerStatusListener list) {
        if (listener == null)
            listener = new ArrayList<DPPControllerStatusListener>();
        return listener.add(list);
    }

    public boolean removeControllerStatusListener(
            DPPControllerStatusListener list) {
        if (listener != null)
