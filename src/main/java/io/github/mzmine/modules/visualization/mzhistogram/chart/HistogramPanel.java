...
public class HistogramPanel extends JPanel {
    ...
    public HistogramPanel() {
        ...
        }
        {
            ...
            center1.setLayout(new BorderLayout(0, 0));
            {
                ...
                boxSettings.setLayout(
                        new BoxLayout(boxSettings, BoxLayout.Y_AXIS));
                {
                ...
                }
                {
                    ...
                    {
                        cbExcludeSmallerNoise = new JCheckBox(
                                "exclude smallest");
                ...
                }
                {
                    ...
                    }
                    {
                        ...
                        btnUpdateGaussian
                                .addActionListener(e -> updateGaussian());
                    ...
                    }
                    {
                        ...
                        txtGaussianLower.setToolTipText(
                                "The lower bound (domain axis) for the Gaussian fit");
                    ...
                    }
                    {
                        ...
                        txtGaussianUpper.setToolTipText(
                                "The upper bound (domain axis, x) for the Gaussian fit");
                    ...
                    {
                        JLabel lblSignificantFigures = new JLabel(
                                "significant figures");
                        secondGaussian.add(lblSignificantFigures);
                    }
                    {
                        txtPrecision = new JTextField();
                        txtPrecision.setToolTipText(
                                "Change number of significant figures and press update");
                ...
                }
                {
                    ...
                    }
                    {
                        ...
                        third.add(txtRangeX);
                        txtRangeX.setToolTipText(
                                "Set the x-range for both histograms");
                        txtRangeX.setText("0");
                    ...
                    }
                    {
                        ...
                        }
                        {
                            ...
                            panel.add(txtRangeY);
                            txtRangeY.setText("0");
                            txtRangeY.setToolTipText(
                                    "Set the y-range for both histograms");
     ...
     * @param data
     * @param binWidth
     *            zero (0) for auto detection, -1 to keep last binWidth
     */
    ...
    private void addListener() {
        ...
        // ranges
        DelayedDocumentListener ddlx = new DelayedDocumentListener(
                e -> applyXRange());
        DelayedDocumentListener ddly = new DelayedDocumentListener(
                e -> applyYRange());
        ...
        txtBinWidth.getDocument().addDocumentListener(
                new DelayedDocumentListener(e -> updateHistograms()));
        txtBinShift.getDocument().addDocumentListener(
                new DelayedDocumentListener(e -> updateHistograms()));
    ...
    private void applyXRange() {
        try {
            ...
            if (x < xe) {
                if (pnHisto != null)
                    pnHisto.getChart().getXYPlot().getDomainAxis().setRange(x,
                            xe);
            }
        } catch (Exception e2) {
            logger.error("", e2);
    ...
    private void applyYRange() {
        try {
            ...
            if (y < ye) {
                if (pnHisto != null)
                    pnHisto.getChart().getXYPlot().getRangeAxis().setRange(y,
                            ye);
            }
        } catch (Exception e2) {
            logger.error("", e2);
                    ...
                    new SwingWorker<JFreeChart, Void>() {
                        ...
                        protected JFreeChart doInBackground() throws Exception {
                            ...
                            if (cbExcludeSmallerNoise.isSelected()) {
                                ...
                                dat = DoubleStream.of(dat)
                                        .filter(d -> d > noise).toArray();
                            }

                            Range r = HistogramChartFactory.getBounds(dat);

                            DoubleFunction<Double> f = cbThirdSQRT.isSelected()
                                    ? val -> Math.cbrt(val)
                                    : val -> val;

                            JFreeChart chart = HistogramChartFactory
                                    .createHistogram(dat, xLabel, binwidth,
                                            r.getLowerBound() - binShift,
                                            r.getUpperBound(), f);
                            // add gaussian?
                        ...
                        protected void done() {
                            ...
                            try {
                                Range x = null, y = null;
                                if (pnHisto != null) {
                                    x = pnHisto.getChart().getXYPlot()
                                            .getDomainAxis().getRange();
                                    y = pnHisto.getChart().getXYPlot()
                                            .getRangeAxis().getRange();
                                }
                                histo = get();

                                if (histo != null) {
                                    if (x != null)
                                        histo.getXYPlot().getDomainAxis()
                                                .setRange(x);
                                    if (y != null)
                                        histo.getXYPlot().getRangeAxis()
                                                .setRange(y);
                                    pnHisto = new EChartPanel(histo, true, true,
                                            true, true, true);
    ...
    protected void addGaussianCurve(XYPlot p) {
        try {
            ...
            HistogramChartFactory.addGaussianFit(p, data, 0, gMin, gMax,
                    sigDigits, true);
        } catch (Exception ex) {
            logger.error("", ex);
        }
