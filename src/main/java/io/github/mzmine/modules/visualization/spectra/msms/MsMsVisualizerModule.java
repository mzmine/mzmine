...
public class MsMsVisualizerModule implements MZmineRunnableModule {
    ...
    public ExitCode runModule(@Nonnull MZmineProject project,
        ...
        Range<Double> rtRange = parameters
                .getParameter(MsMsParameters.retentionTimeRange).getValue();
        Range<Double> mzRange = parameters.getParameter(MsMsParameters.mzRange)
                .getValue();
        final IntensityType intensityType = parameters
                .getParameter(MsMsParameters.intensityType).getValue();
        final NormalizationType normalizationType = parameters
                .getParameter(MsMsParameters.normalizationType).getValue();
    ...
    public static void showIDAVisualizerSetupDialog(RawDataFile dataFile,
        ...
        if (rtRange != null)
            parameters.getParameter(MsMsParameters.retentionTimeRange)
                    .setValue(rtRange);
        if (mzRange != null)
            parameters.getParameter(MsMsParameters.mzRange).setValue(mzRange);
        if (intensityType != null)
            parameters.getParameter(MsMsParameters.intensityType)
                    .setValue(intensityType);
        if (normalizationType != null)
            parameters.getParameter(MsMsParameters.normalizationType)
                    .setValue(normalizationType);
        if (!Double.isNaN(minPeakInt))
            parameters.getParameter(MsMsParameters.minPeakInt)
                    .setValue(minPeakInt);
        ...
        intensityType = parameters.getParameter(MsMsParameters.intensityType)
                .getValue();
        normalizationType = parameters
                .getParameter(MsMsParameters.normalizationType).getValue();
        minPeakInt = parameters.getParameter(MsMsParameters.minPeakInt)
                .getValue();
