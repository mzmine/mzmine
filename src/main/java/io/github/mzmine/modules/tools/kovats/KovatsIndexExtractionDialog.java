...
import io.github.mzmine.modules.visualization.tic.TICSumDataSet;
...
public class KovatsIndexExtractionDialog extends ParameterSetupDialog {
    ...
    protected void addDialogComponents() {
        ...
        DelayedDocumentListener ddlUpdateChart = new DelayedDocumentListener(
                e -> updateChart());

        newMainPanel = new JPanel(
                new MigLayout("fill", "[right][grow,fill]", ""));
        ...
        // add min max
        JPanel pnKovatsParam = new JPanel(
                new FlowLayout(FlowLayout.CENTER, 5, 5));
        west.add(pnKovatsParam, BorderLayout.NORTH);
        minc = (IntegerComponent) getComponentForParameter(
                KovatsIndexExtractionParameters.minKovats);
        maxc = (IntegerComponent) getComponentForParameter(
                KovatsIndexExtractionParameters.maxKovats);
        ...
        comboKovats = (MultiChoiceComponent) getComponentForParameter(
                KovatsIndexExtractionParameters.kovats);
        ...
        MZRangeComponent mzc = (MZRangeComponent) getComponentForParameter(
                KovatsIndexExtractionParameters.mzRange);
        RTRangeComponent rtc = (RTRangeComponent) getComponentForParameter(
                KovatsIndexExtractionParameters.rtRange);
        DoubleComponent noisec = (DoubleComponent) getComponentForParameter(
                KovatsIndexExtractionParameters.noiseLevel);
        DoubleComponent edgeRatioC = (DoubleComponent) getComponentForParameter(
                KovatsIndexExtractionParameters.ratioTopEdge);

        valuesComponent.addDocumentListener(
                new DelayedDocumentListener(e -> kovatsValuesChanged()));
        ...
        btnCombineFiles.setToolTipText(
                "Select multiple Kovats index files to be combined into one");
        ...
        cbSecondRaw.addItemListener(
                e -> useSecondDataFile(cbSecondRaw.isSelected()));
        ...
        pnPeakPick.add(mzc);
        pnPeakPick.add(
                new JLabel(KovatsIndexExtractionParameters.rtRange.getName()),
                "cell 0 5");
        pnPeakPick.add(rtc);
        pnPeakPick.add(
                new JLabel(
                        KovatsIndexExtractionParameters.noiseLevel.getName()),
                "cell 0 6");
        pnPeakPick.add(noisec);
        pnPeakPick.add(
                new JLabel(
                        KovatsIndexExtractionParameters.ratioTopEdge.getName()),
                "cell 0 7");
    ...
    private void setMzRangeByAlkane(int diff) {
        ...
        String formula = currentAlkane
                .getFormula(cbCurrentAlkaneSubH.isSelected());
        lbCurrentAlkane.setText(formula);
        // set mz
        Range<Double> mzRange = MzRangeFormulaCalculatorModule
                .getMzRangeFromFormula(formula, IonizationType.POSITIVE,
                        new MZTolerance(0.75, 0), 1);
        ((MZRangeComponent) getComponentForParameter(
                KovatsIndexExtractionParameters.mzRange)).setValue(mzRange);
    }
    ...
    private void initRawDataFileSelection() {
        ...
        RawDataFilesSelection select = parameterSet
                .getParameter(KovatsIndexExtractionParameters.dataFiles)
                .getValue();
        ...
        if (select != null && select.getMatchingRawDataFiles().length > 0) {
            RawDataFile[] exists = Arrays
                    .stream(select.getMatchingRawDataFiles())
                    .filter(r -> Arrays.stream(dataFiles)
                            .anyMatch(d -> r.getName().equals(d.getName())))
        ...
        if (raw == null) {
            ...
            RawDataFile[] kovats = Arrays.stream(dataFiles)
                    .filter(d -> d.getName().toLowerCase().contains("kovats"))
                    .toArray(RawDataFile[]::new);
            RawDataFile[] dro = Arrays.stream(dataFiles)
                    .filter(d -> d.getName().toLowerCase().contains("dro"))
                    .toArray(RawDataFile[]::new);
    ...
    private void setLastFile(File f) {
        ((FileNameComponent) getComponentForParameter(
                KovatsIndexExtractionParameters.lastSavedFile)).setValue(f);
        parameterSet.getParameter(KovatsIndexExtractionParameters.lastSavedFile)
                .setValue(f);
    }
        ...
        try {
            ...
            for (String s : entries) {
                if (s.isEmpty())
                    continue;
                String[] e = s.split(":");
                parsedValues.put(KovatsIndex.getByShortName(e[0]),
     ...
     * 
     * @param values
     * @param checkMinMaxSelectedChange
     *            check min max and selected Kovats for Kovats Index list
     */
    private void setValues(TreeMap<KovatsIndex, Double> values,
            boolean checkMinMaxSelectedChange) {
        if (values.size() < 2)
            return;
        ...
        for (Entry<KovatsIndex, Double> e : values.entrySet()) {
            s.append(e.getKey().getShortName() + ":"
                    + rtFormat.format(e.getValue()));
            s.append(",");
    ...
    private void updateChart() {
        ...
        try {
            ...
            org.jfree.data.Range domainZoom = chart == null ? null
                    : chart.getChart().getXYPlot().getDomainAxis().getRange();
            org.jfree.data.Range rangeZoom = chart == null ? null
                    : chart.getChart().getXYPlot().getRangeAxis().getRange();

            Range<Double> rangeMZ = parameterSet
                    .getParameter(KovatsIndexExtractionParameters.mzRange)
                    .getValue();
            Range<Double> rangeRT = parameterSet
                    .getParameter(KovatsIndexExtractionParameters.rtRange)
                    .getValue();
            if (rangeMZ == null) {
            ...
            if (rangeRT == null) {
                ...
                ((RTRangeComponent) getComponentForParameter(
                        KovatsIndexExtractionParameters.rtRange))
                                .setValue(rangeRT);
            }

            // create dataset
            TICSumDataSet data = new TICSumDataSet(selectedDataFile, rangeRT,
                    rangeMZ, null, TICPlotType.BASEPEAK);
            ...
            chart.getGestureAdapter()
                    .addGestureHandler(new ChartGestureDragDiffHandler(
                            Entity.PLOT, Button.BUTTON1, new Key[] { Key.NONE },
                            e -> handleMarkerDrag(e)));
            chart.getGestureAdapter().addGestureHandler(new ChartGestureHandler(
                    new ChartGesture(Entity.PLOT, Event.RELEASED), e -> {
                        if (chart != null)
                            chart.setMouseZoomable(true);
                        if (currentlyDraggedMarker != null) {
                            // set value of current marker
                            logger.info("Marker dragging ended at "
                                    + currentlyDraggedMarker.getValue());
    ...
    private void handleMarkerDrag(ChartGestureDragDiffEvent e) {
        ...
        if (currentlyDraggedMarker != null) {
            ...
            currentlyDraggedMarker
                    .setValue(e.getLatestEvent().getCoordinates().getX());
        }
    }
    ...
    protected void updateParameterSetFromComponents() {
        ...
        selectedKovats = parameterSet
                .getParameter(KovatsIndexExtractionParameters.kovats)
                .getValue();
        pickedValuesString = parameterSet
                .getParameter(
                        KovatsIndexExtractionParameters.pickedKovatsValues)
                .getValue();
        noiseLevel = parameterSet
                .getParameter(KovatsIndexExtractionParameters.noiseLevel)
                .getValue() != null
                        ? parameterSet.getParameter(
                                KovatsIndexExtractionParameters.noiseLevel)
                                .getValue()
                        : 0;
        ratioEdge = parameterSet
                .getParameter(KovatsIndexExtractionParameters.ratioTopEdge)
                .getValue() != null
                        ? parameterSet.getParameter(
                                KovatsIndexExtractionParameters.ratioTopEdge)
                                .getValue()
        ...
        if (cbSecondRaw.isSelected()
                || comboDataFileName2.getSelectedItem() == null)
            selectedDataFile = new RawDataFile[] {
                    (RawDataFile) comboDataFileName.getSelectedItem(),
                    (RawDataFile) comboDataFileName2.getSelectedItem() };
        else
            selectedDataFile = new RawDataFile[] {
                    (RawDataFile) comboDataFileName.getSelectedItem() };

        if (selectedDataFile != null && selectedDataFile.length > 0
                && selectedDataFile[0] != null) {
    ...
    private void updateKovatsList() {
        updateParameterSetFromComponents();
        try {
            int min = parameterSet
                    .getParameter(KovatsIndexExtractionParameters.minKovats)
                    .getValue();
            int max = parameterSet
                    .getParameter(KovatsIndexExtractionParameters.maxKovats)
                    .getValue();
            KovatsIndex[] newValues = KovatsIndex.getRange(min, max);
            KovatsIndex[] newSelected = Stream.of(newValues)
                    .filter(k -> ArrayUtils.contains(selectedKovats, k))
                    .toArray(KovatsIndex[]::new);

            parameterSet.getParameter(KovatsIndexExtractionParameters.kovats)
                    .setChoices(newValues);
            parameterSet.getParameter(KovatsIndexExtractionParameters.kovats)
                    .setValue(newSelected);
            MultiChoiceComponent kovatsc = (MultiChoiceComponent) getComponentForParameter(
                    KovatsIndexExtractionParameters.kovats);
    ...
    private int loadFile() {
        File lastFile = parameterSet
                .getParameter(KovatsIndexExtractionParameters.lastSavedFile)
                .getValue();
        ...
        if (chooser.showOpenDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            for (File cf : f) {
                try {
                    List<String> lines = Files.readLines(cf,
                    ...
                    logger.log(Level.WARNING,
                            "Cannot read lines of " + cf.getAbsolutePath(), e);
                }
            }
    ...
    private synchronized void saveToFile() {
        ...
        if (chooser.showSaveDialog(this) == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                logger.log(Level.WARNING, "Error while saving Kovats file to "
                        + f.getAbsolutePath(), e);
            }
        }
    }
