...
public class IsotopePatternPreviewDialog extends ParameterSetupDialog {
    ...
    String[][] columns = {
            { "Exact mass / Da", "Intensity", "Isotope composition" },
    ...
    public IsotopePatternPreviewDialog(Window parent,
    ...
    protected void addDialogComponents() {
        ...
        pFormula = parameterSet
                .getParameter(IsotopePatternPreviewParameters.formula);
        pMinIntensity = parameterSet
                .getParameter(IsotopePatternPreviewParameters.minIntensity);
        pMergeWidth = parameterSet
                .getParameter(IsotopePatternPreviewParameters.mergeWidth);
        pCharge = parameterSet
                .getParameter(IsotopePatternPreviewParameters.charge);

        Dimension screenSize = Toolkit.getDefaultToolkit().getScreenSize();

        cmpMinIntensity = (PercentComponent) getComponentForParameter(
                IsotopePatternPreviewParameters.minIntensity);
        cmpMergeWidth = (DoubleComponent) getComponentForParameter(
                IsotopePatternPreviewParameters.mergeWidth);
        cmpCharge = (IntegerComponent) getComponentForParameter(
                IsotopePatternPreviewParameters.charge);
        cmpFormula = (StringComponent) getComponentForParameter(
                IsotopePatternPreviewParameters.formula);
        ...
        pnText.setHorizontalScrollBarPolicy(
                JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
        pnText.setVerticalScrollBarPolicy(
    ...
    public void updateWindow() {
        if (!updateParameters()) {
            logger.warning(
                    "updateWindow() failed. Could not update parameters or parameters are invalid."
        ...
        if (FormulaUtils.getFormulaSize(formula) > 5E3
            ...
            logger.finest("Big formula " + formula + " size: "
                    + FormulaUtils.getFormulaSize(formula)
                    + " or last calculation recent: "
                    + (System.nanoTime() - lastCalc) / 1E6 + " ms");
        }
    ...
    protected void updateChart(ExtendedIsotopePattern pattern) {
        ...
        if (pol == PolarityType.NEUTRAL)
            chart = ChartFactory.createXYBarChart("Isotope pattern preview",
                    "Exact mass / Da", false, "Abundance", dataset);
        else
            chart = ChartFactory.createXYBarChart("Isotope pattern preview",
                    "m/z", false, "Abundance", dataset);
    ...
    private boolean checkParameters() {
        if (pFormula.getValue() == null || pFormula.getValue().equals("")
            ...
            logger.fine(
                    "Invalid input or Element == \"\" or invalid elements.");
            return false;
        }
