...
package io.github.mzmine.modules.rawdatamethods.peakpicking.gridmass;
...
import io.github.mzmine.modules.masslistmethods.chromatogrambuilder.Chromatogram;
...
public class GridMassTask extends AbstractTask {
  ...
  public GridMassTask(MZmineProject project, RawDataFile dataFile, ParameterSet parameters) {
    ...
    this.minimumTimeSpan =
        parameters.getParameter(GridMassParameters.timeSpan).getValue().lowerEndpoint();
    this.maximumTimeSpan =
        parameters.getParameter(GridMassParameters.timeSpan).getValue().upperEndpoint();
    ...
    this.intensitySimilarity =
        parameters.getParameter(GridMassParameters.intensitySimilarity).getValue();
  ...
  public void run() {
    ...
    logger.info("Creating Grid of probes on " + dataFile + " every " + mzFormat.format(byMZ)
    ...
    logger.info("Finding local maxima for each probe on " + dataFile + " radius: scans=" + scanR
        + ", m/z=" + mzR);
    int okProbes = 0;
    for (i = 0; i < idata; i++) {
      if (i % 100 == 0 && isCanceled())
              ...
              if (debug > 2)
                System.out.println(
                    "Joining s1 id " + s1.spotId + "=" + mzFormat.format(s1.center.mzCenter)
                        ...
                        + " ~ " + mzFormat.format(s2.maxMZ) + "] time="
                        + timeFormat.format(retentiontime[s2.center.scanCenter]) + " int="
    ...
    for (i = 0; i < spots.size() - 1; i++) {
      ...
      if (s1.center != null && s1.size() > 0) {
        ...
        if (totalScans * rtPerScan > maximumTimeSpan) {
          if (debug > 2)
            System.out.println("Removing " + toRemove.size() + " masses around "
                + mzFormat.format(s1.center.mzCenter) + " m/z (" + s1.spotId + "), time "
                + timeFormat.format(retentiontime[s1.center.scanCenter]) + ", intensity "
