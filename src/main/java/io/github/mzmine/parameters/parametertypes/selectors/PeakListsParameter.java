...
public class PeakListsParameter
        implements UserParameter<PeakListsSelection, PeakListsComponent> {
    ...
    public PeakListsParameter cloneParameter() {
        PeakListsParameter copy = new PeakListsParameter(name, minCount,
                maxCount);
        if (value != null)
    ...
    public boolean checkValue(Collection<String> errorMessages) {
        ...
        if (matchingPeakLists.length < minCount) {
            errorMessages.add("At least " + minCount
                    + " feature lists  must be selected");
            return false;
        }
        if (matchingPeakLists.length > maxCount) {
            errorMessages.add(
                    "Maximum " + maxCount + " feature lists may be selected");
            return false;
        }
        return true;
    }
    ...
    public void loadValueFromXML(Element xmlElement) {
        ...
        if (Strings.isNullOrEmpty(attrValue))
            ...
            selectionType = PeakListsSelectionType
