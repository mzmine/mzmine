    ...
    private void updateParametersToTable(){
        ...
        for(RawDataFile rawFile:fileList){
            ...
            for(UserParameter<?,?> parameter:parameterList){
                fileParametersValue.add(new SimpleStringProperty((String) currentProject.getParameterValue(parameter,rawFile)));
            }
...
//                        String oldParaVal = event.getOldValue();
    ...
    public void removePara(ActionEvent actionEvent) {
        ...
        if(result.isPresent() && result.get() == ButtonType.OK){
            UserParameter<?,?> tbdParameter = currentProject.getParameterByName(parameterName);//ToBeDeletedParameter
