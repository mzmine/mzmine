...
public enum Weighting {
     ...
     * @return
     */
    public double transform(double weight, Double noiseLevel,
            Double maxWeight) {
        // e.g., lOG(weight)-LOG(noise)
        double real = 0;
        if (noiseLevel != null)
     ...
     * @return
     */
    public double[] transform(double[] weights, Double noiseLevel,
            Double maxWeight) {
        return DoubleStream.of(weights)
                .map(v -> transform(v, noiseLevel, maxWeight)).toArray();
    }
