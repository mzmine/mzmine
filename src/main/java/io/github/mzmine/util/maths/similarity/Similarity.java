...
/**
 * Different similarity measures such as COSINE SIMILARITY, PEARSON,
 * SPEARMAN,...
 * 
...
public abstract class Similarity {
    ...
    public static final Similarity COSINE = new Similarity() {
        ...
        public double calc(double[][] data) {
            return dot(data)
                    / (Math.sqrt(norm(data, 0)) * Math.sqrt(norm(data, 1)));
        }
    };
    ...
    public static double maxFoldChange(double[][] data, final int i) {
        double min = Arrays.stream(data).mapToDouble(d -> d[i]).min()
                .getAsDouble();
        double max = Arrays.stream(data).mapToDouble(d -> d[i]).max()
                .getAsDouble();
        return max / min;
    }
