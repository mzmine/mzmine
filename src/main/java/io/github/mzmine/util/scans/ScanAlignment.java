...
public class ScanAlignment {
    ...
    public static final DataPointSorter sorter = new DataPointSorter(
    ...
    /**
     * Aligned data points within mzTolerance. Sort by intensity and match every
     * signal only once
     * 
     * @param a
     * @param b
     ...
     */
    public static List<DataPoint[]> align(MZTolerance mzTol, DataPoint[] a,
            DataPoint[] b) {
        // sort by intensity
     ...
     * @return
     */
    public static Range<Double> getOverlapMZ(MZTolerance mzTol, DataPoint[] a,
            DataPoint[] b) {
     ...
     */
    public static DataPoint[][] cropToOverlap(MZTolerance mzTol, DataPoint[] a,
            DataPoint[] b) {
        Range<Double> overlap = getOverlapMZ(mzTol, a, b);
        DataPoint[] newa = Arrays.stream(a)
                .filter(d -> overlap.contains(d.getMZ()))
                .toArray(DataPoint[]::new);
        DataPoint[] newb = Arrays.stream(b)
                .filter(d -> overlap.contains(d.getMZ()))
                .toArray(DataPoint[]::new);
     ...
     * @return
     */
    private static DataPoint findMatch(MZTolerance mzTol, DataPoint dpa,
            List<DataPoint> bsorted) {
    ...
    public static DataPoint[][] removeUnaligned(DataPoint[][] list) {
        ...
        for (int d = 0; d < list[0].length; d++) {
            ...
            if (noneNull)
                aligned.add(Arrays.stream(list).map(dp -> dp[dd])
                        .toArray(DataPoint[]::new));
        }
        // to array
    ...
    /**
     * Might want to remove unaligned before. Missing data points are replaced
     * by 0. <br>
     ...
     * @return array of [data points][intensity in scans]
     */
    public static double[][] toIntensityMatrixWeighted(
            List<DataPoint[]> diffAligned, double weightIntensity,
            double weightMZ) {
        double[][] data = new double[diffAligned.size()][];
        for (int i = 0; i < data.length; i++) {
            DataPoint[] dps = diffAligned.get(i);
            ...
            for (int d = 0; d < dps.length; d++) {
                DataPoint dp = dps[d];
                if (dp != null)
                    data[i][d] = Math.pow(dp.getIntensity(), weightIntensity)
                            * Math.pow(dp.getMZ(), weightMZ);
                else
                    data[i][d] = 0;
            }
        }
     ...
     * 
     * @param aligned
     *            aligned list(DataPoint)[dimension]
     * @return
     */
    public static DataPoint[][] convertBackToMassLists(
            List<DataPoint[]> aligned) {
        int dpCount = aligned.size();
        int maxDimensions = aligned.stream().mapToInt(dps -> dps.length).max()
                .orElse(0);
