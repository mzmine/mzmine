...
public class ScanUtils {
    ...
    private static final Logger logger = Logger
            .getLogger(ScanUtils.class.getName());
    ...
    public static @Nonnull String scanToString(@Nonnull Scan scan,
            @Nonnull Boolean includeFileName) {
     ...
     * @param binRange
     *            x coordinates of the left and right edge of the first bin
     * @param numberOfBins
     *            Number of bins
     * @param interpolate
    ...
    /**
     * sort the data points by their m/z value. This method should be called
     * before using other search methods to do binary search in logarithmic
     * time.
     * 
     * @param dataPoints
     *            spectrum that should be sorted
     */
    ...
    /**
     * Returns the index of the datapoint with lowest m/z within the given
     * datapoints which is within the given mass range
     * 
     * @param dataPoints
     *            sorted(!) list of datapoints
     * @param mzRange
     *            m/z range to search in
     * @return index of datapoint or -1, if no datapoint is in range
     */
    public static int findFirstPeakWithin(DataPoint[] dataPoints,
            Range<Double> mzRange) {
        ...
        if (insertionPoint < 0) {
            ...
            if (k < dataPoints.length
                    && mzRange.contains(dataPoints[k].getMZ()))
                return k;
            else
                return -1;
        } else {
    ...
    /**
     * Returns the index of the datapoint with largest m/z within the given
     * datapoints which is within the given mass range
     * 
     * @param dataPoints
     *            sorted(!) list of datapoints
     ...
     * @return index of datapoint or -1, if no datapoint is in range
     */
    public static int findLastPeakWithin(DataPoint[] dataPoints,
            Range<Double> mzRange) {
        final int insertionPoint = Arrays.binarySearch(dataPoints,
    ...
    /**
     * Returns the index of the datapoint with highest intensity within the
     * given datapoints which is within the given mass range
     * 
     * @param dataPoints
     *            sorted(!) list of datapoints
     * @param mzRange
     ...
     * @return index of datapoint or -1, if no datapoint is in range
     */
    public static int findMostIntensePeakWithin(DataPoint[] dataPoints,
            Range<Double> mzRange) {
        ...
        for (; k < dataPoints.length; ++k) {
            if (!mzRange.contains(dataPoints[k].getMZ()))
                break;
            if (dataPoints[k].getIntensity() > dataPoints[mostIntense]
                    .getIntensity()) {
                mostIntense = k;
            }
        }
    ...
    /**
     * Finds all MS/MS scans on MS2 level within given retention time range and
     * with precursor m/z within given m/z range
     */
    public static int[] findAllMS2FragmentScans(RawDataFile dataFile,
            Range<Double> rtRange, Range<Double> mzRange) {
        ...
        if (resultScans.length > 0) {
            resultScans = fragmentScanNumbersInMZRange.stream().mapToInt(i -> i)
                    .toArray();
        } else {
            resultScans = new int[] {};
        }
    ...
    public static char[] encodeDataPointsBase64(DataPoint dataPoints[]) {
        ...
        char encodedData[] = Base64.getEncoder().encodeToString(peakBytes)
                .toCharArray();
        return encodedData;
    }
    ...
    /**
     * Sorted list (best first) of all MS2 fragmentation scans with n signals >=
     * noiseLevel in the specified or first massList, if none was specified
     * 
     * @param row
     *            all MS2 scans of all features in this row
     * @param massListName
     *            the name or null/empty to always use the first masslist
     * @param noiseLevel
     * @param minNumberOfSignals
     * @param sort
     *            the sorting property (best first, index=0)
     * @return
     */
    @Nonnull
    public static List<Scan> listAllFragmentScans(PeakListRow row,
            @Nullable String massListName, double noiseLevel,
            int minNumberOfSignals, ScanSortMode sort)
            throws MissingMassListException {
        List<Scan> scans = listAllFragmentScans(row, massListName, noiseLevel,
                minNumberOfSignals);
        // first entry is the best scan
        scans.sort(Collections
                .reverseOrder(new ScanSorter(massListName, noiseLevel, sort)));
        return scans;
    }

    /**
     * List of all MS2 fragmentation scans with n signals >= noiseLevel in the
    ...
    public static List<Scan> listAllFragmentScans(PeakListRow row,
        ...
        return listAllScans(scans, massListName, noiseLevel,
                minNumberOfSignals);
    }

    /**
     * Sorted list of all MS1 {@link Feature#getRepresentativeScan()} of all
     * features. scans with n signals >= noiseLevel in the specified or first
     * massList, if none was specified
     * 
     * @param row
     *            all representative MS1 scans of all features in this row
     * @param massListName
     *            the name or null/empty to always use the first masslist
     * @param noiseLevel
     * @param minNumberOfSignals
     ...
     */
    @Nonnull
    public static List<Scan> listAllMS1Scans(PeakListRow row,
            @Nullable String massListName, double noiseLevel,
            int minNumberOfSignals, ScanSortMode sort)
            throws MissingMassListException {
        List<Scan> scans = listAllMS1Scans(row, massListName, noiseLevel,
                minNumberOfSignals);
        // first entry is the best scan
        scans.sort(Collections
                .reverseOrder(new ScanSorter(massListName, noiseLevel, sort)));
    ...
    /**
     * List of all MS1 {@link Feature#getRepresentativeScan()} of all features.
     * scans with n signals >= noiseLevel in the specified or first massList, if
     * none was specified
     * 
     * @param row
     * @param massListName
    ...
    /**
     * List of all scans with n signals >= noiseLevel in the specified or first
     * massList, if none was specified
     * 
     * @param massListName
     *            the name or null/empty to always use the first masslist
     ...
     */
    @Nonnull
    public static List<Scan> listAllScans(Scan[] scans,
            @Nullable String massListName, double noiseLevel,
            int minNumberOfSignals, ScanSortMode sort)
            throws MissingMassListException {
        List<Scan> filtered = listAllScans(scans, massListName, noiseLevel,
                minNumberOfSignals);
        // first entry is the best scan
        filtered.sort(Collections
                .reverseOrder(new ScanSorter(massListName, noiseLevel, sort)));
        return filtered;
    }
     ...
     * @param scan
     * @param massListName
     * @return null if no masslist with this name or if name was not specified
     *         and this scan has zero masslists
     * @throws MissingMassListException
     */
    ...
    public static double getTIC(DataPoint[] data, double noiseLevel) {
        return Stream.of(data).mapToDouble(DataPoint::getIntensity)
                .filter(i -> i >= noiseLevel).sum();
    }
    ...
    public static DataPoint[] getFiltered(DataPoint[] data, double noiseLevel) {
        return Stream.of(data).filter(dp -> dp.getIntensity() >= noiseLevel)
     ...
     * @return
     */
    public static DataPoint[] getBelowThreshold(DataPoint[] data,
            double noiseLevel) {
        return Stream.of(data).filter(dp -> dp.getIntensity() < noiseLevel)
                .toArray(DataPoint[]::new);
    }
    ...
    /**
     * Finds the first MS1 scan preceding the given MS2 scan. If no such scan
     * exists, returns null.
     */
    ...
    /**
     * Finds the first MS1 scan succeeding the given MS2 scan. If no such scan
     * exists, returns null.
     */
    public static @Nullable Scan findSucceedingPrecursorScan(
            @Nonnull Scan scan) {
     ...
     */
    public static @Nonnull Collection<Scan> selectBestMS2Scans(
            @Nonnull PeakListRow row, @Nonnull String massListName,
            @Nonnull Integer topN) throws MissingMassListException {

        @SuppressWarnings("null")
        final @Nonnull List<Scan> allMS2Scans = Arrays
                .asList(row.getAllMS2Fragmentations());
     ...
     */
    public static @Nonnull Collection<Scan> selectBestMS2Scans(
            @Nonnull Collection<Scan> scans, @Nonnull String massListName,
            @Nonnull Integer topN) throws MissingMassListException {
        ...
        TreeSet<Scan> sortedScans = new TreeSet<>(Collections.reverseOrder(
                new ScanSorter(massListName, 0, ScanSortMode.MAX_TIC)));
     ...
     * numberOfPeaksPerBin most intense peaks within the window. This is a very
     * simple and robust method to remove most noise in the spectrum without
     * having to estimate any noise intensity parameter.
     * 
     * @param dataPoints
     *            spectrum
     * @param binRange
     *            sliding mass window. Is shifted in each step by its width.
     * @param numberOfPeaksPerBin
     *            number of peaks to keep within the sliding mass window
     * @return
     */
    public static DataPoint[] extractMostIntensePeaksAcrossMassRange(
            DataPoint[] dataPoints, Range<Double> binRange,
            int numberOfPeaksPerBin) {
        double offset = binRange.lowerEndpoint();
        final double width = binRange.upperEndpoint()
                - binRange.lowerEndpoint();
        ...
        for (Integer bin : bins.keySet()) {
            ...
            Collections.sort(list, (u, v) -> Double.compare(v.getIntensity(),
                    u.getIntensity()));
    ...
    /**
     * Generalization of the cosine similarity for high resolution. See
     * Algorithmic Mass Spectrometry by Sebastian Böcker, chapter 4.2 While the
     * cosine similarity transforms the spectrum into a finite dimensional
     * vector, the probability product transforms it into a mixture of
     * continuous gaussians.
     *
     * As for cosine similarity it is recommended to first take the square root
     * of all peak intensities, before calling this method.
     *
     * @param scanLeft
     *            the first spectrum
     * @param scanRight
     *            the second spectrum
     * @param expectedMassDeviationInPPM
     *            the width of the gaussians (corresponds to the expected mass
     *            deviation). Rather use a larger than a small value! Value is
     *            given in ppm and Dalton.
     * @param noiseLevel
     *            the lowest intensity for a peak to be considered
     * @param mzRange
     *            the m/z range in which the peaks are compared. use null for
     *            the whole spectrum
     *
     */
    public static double probabilityProduct(DataPoint[] scanLeft,
            DataPoint[] scanRight, MZTolerance expectedMassDeviationInPPM,
            double noiseLevel, @Nullable Range<Double> mzRange) {
        double d = probabilityProductUnnormalized(scanLeft, scanRight,
                expectedMassDeviationInPPM, noiseLevel, mzRange);
        double l = probabilityProductUnnormalized(scanLeft, scanLeft,
                expectedMassDeviationInPPM, noiseLevel, mzRange);
        double r = probabilityProductUnnormalized(scanRight, scanRight,
                expectedMassDeviationInPPM, noiseLevel, mzRange);
    ...
    /**
     * Calculates the probability product without normalization. Usually, this
     * method is only useful if you plan to normalize the spectra (or value)
     * yourself.
     * 
     * @see #probabilityProduct(DataPoint[], DataPoint[], MZTolerance, double,
     *      Range)
     */
    public static double probabilityProductUnnormalized(DataPoint[] scanLeft,
            DataPoint[] scanRight, MZTolerance expectedMassDeviationInPPM,
            double noiseLevel, @Nullable Range<Double> mzRange) {
        int i, j;
        double score = 0d;
        ...
        // computation
        final double allowedDifference = expectedMassDeviationInPPM
                .getMzToleranceForMass(1000d) * 5;
        while (i < nl && j < nr) {
    ...
    /**
     * Calculates the product of the integrals of two gaussians centered in lp
     * and rp with given variance
     */
    private static double probabilityProductScore(DataPoint lp, DataPoint rp,
            double variance) {
        final double mzDiff = Math.abs(lp.getMZ() - rp.getMZ());
        final double constTerm = 1.0 / (Math.PI * variance * 4);
        final double propOverlap = constTerm
                * Math.exp(-(mzDiff * mzDiff) / (4 * variance));
