...
public class PeakUtils {
    ...
    private static final PeakListRowSorter ascMzRowSorter = new PeakListRowSorter(
    ...
    /**
     * Compares identities of two feature list rows. 1) if preferred identities
     * are available, they must be same 2) if no identities are available on
     * both rows, return true 3) otherwise all identities on both rows must be
     * same
     * 
     * @return True if identities match between rows
     * 
     */
    public static boolean compareIdentities(PeakListRow row1,
            PeakListRow row2) {
        ...
        if ((row1PreferredIdentity != null)
                && (row2PreferredIdentity != null)) {
            if (row1PreferredIdentity.getName()
                    .equals(row2PreferredIdentity.getName()))
                return true;
            else
                return false;
        }
     ...
     */
    public static boolean compareChargeState(PeakListRow row1,
            PeakListRow row2) {
    ...
    /**
     * Returns true if feature list row contains a compound identity matching to
     * id
     * 
     */
     ...
     */
    public static double integrateOverMzRtRange(RawDataFile dataFile,
     ...
     * @param row
     *            The row.
     * @return The average retention time range of all features contained in
     *         this peak list row across all raw data files. Empty range (0,0)
     *         if the row is null or has no feature assigned to it.
     */
    public @Nonnull static Range<Double> getPeakListRowAvgRtRange(
            PeakListRow row) {
