...
public class PeakSummaryComponent extends JPanel implements ActionListener {
    ...
    public PeakSummaryComponent(PeakListRow row, RawDataFile[] rawDataFiles,
            ...
            boolean tableVisible, boolean buttonsVisible,
            ...
            for (int r = 0; r < numRows; r++) {
                ...
                if (r == 0) {
                    comp = headerRenderer.getTableCellRendererComponent(
                            peaksInfoList, model.getColumnName(c), false, false,
                            r, c);
        ...
        if (command.equals("SHOW")) {
            ...
            for (int i = 0; i < indexesRow.length; i++) {
                ...
                if ((rtRange == null) || (mzRange == null)) {
                    ...
                    mzRange = mzRange
                            .span(selectedPeaks[i].getRawDataPointsMZRange());
                }
            }
            ...
            if (visualizerType.equals("Chromatogram")) {
                ...
                for (int i = 0; i < selectedPeaks.length; ++i) {
                    ...
                    if (ip != null) {
                        ...
                        DataPoint[] newDataPoints = Arrays
                                .stream(ip.getDataPoints())
                                .map(p -> new SimpleDataPoint(p.getMZ(),
                                        -p.getIntensity()))
                        ...
                        if (identity != null) {
                            String spectrum = identity.getPropertyValue(
                                    PeakIdentity.PROPERTY_SPECTRUM);
                        ...
                        if (identityDataPoints.isEmpty()) // Plot raw spectrum
                            ...
                            SpectraVisualizerModule.showNewSpectrumWindow(
                                    ...
                                    null, null,
                                    new SimpleIsotopePattern(newDataPoints,
                                            ip.getStatus(),
                                            ip.getDescription()));
                            ...
                            SpectraVisualizerModule.showNewSpectrumWindow(
                                    ...
                                    new SimpleIsotopePattern(
                                            identityDataPoints.toArray(
                                                    new DataPoint[identityDataPoints
                                                            .size()]),
                                            IsotopePatternStatus.DETECTED,
                                            identity.getPropertyValue(
                ...
                for (int i = 0; i < selectedPeaks.length; i++) {
                    ...
                    Range<Double> localMZRange = Range.closed(
                            Math.max(0, peakMZRange.lowerEndpoint() - mzLen),
                            peakMZRange.upperEndpoint() + mzLen);
                    Fx3DVisualizerModule.setupNew3DVisualizer(dataFiles[i],
                ...
                for (int i = 0; i < selectedPeaks.length; i++) {
                    ...
                    SpectraVisualizerModule.showNewSpectrumWindow(dataFiles[i],
                            selectedPeaks[i].getMostIntenseFragmentScanNumber(),
                            ip);
