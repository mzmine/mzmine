...
public class StartRserve {
    ...
    public static boolean installRserve(String Rcmd, String http_proxy,
        ...
        boolean ok = doInR((http_proxy != null
     ...
     */
    public static boolean doInR(String todo, String Rcmd, String rargs,
            StringBuffer out, StringBuffer err) {
        try {
            Process p;
     ...
     */
    public static boolean launchRserve(String cmd,
            /* String libloc, */ String rargs, String rsrvargs, boolean debug) {
        ...
        // rsrvargs + "')", cmd,
        // rargs, null, null);
        boolean startRserve = doInR("library(Rserve);Rserve("
                + (debug ? "TRUE" : "FALSE") + ",args='" + rsrvargs + "')", cmd,
                rargs, null, null);
        if (startRserve) {
            System.err
        ...
        int attempts = 5; /*
                           ...
                           * conservative here, because at this point the
                           * process execution itself was successful and the
                           * start up is usually asynchronous
                           */
        while (attempts > 0) {
            try {
                RConnection c = null;
                ...
                if (rsrvargs.contains("--RS-port")) {
                    String rsport = rsrvargs.split("--RS-port")[1].trim()
                            .split(" ")[0];
