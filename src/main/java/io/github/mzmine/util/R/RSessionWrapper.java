...
public class RSessionWrapper {
     ...
     */
    public static final double NA_DOUBLE = Double
     ...
     * Constructor.
     */
    public RSessionWrapper(REngineType rEngineType, String callerFeatureName,
            String[] reqPackages, String[] reqPackagesVersions) {
                ...
                if (this.rEngineType == REngineType.RSERVE) {
                    ...
                    try {
                        ...
                        synchronized (RSessionWrapper.R_SESSION_SEMAPHORE) {
                            ...
                            final String rLocation = RLocationDetection
                                    .getRExecutablePath();
                        ...
                        synchronized (rSemaphore) {
                            ...
                            if (isWindows) {
                                ...
                                int port = RserverConf.getNewAvailablePort();
                                conf = new RserverConf("localhost", port,
                                        RS_LOGIN, RS_DYN_PWD, null); // props);
                            } else {
                            ...
                            if (isWindows) {
                                ...
                                // every
                                // time.
                                this.session = Rsession.newInstanceTry(
                    ...
                    Globals.RScript_Linux = RLocationDetection
                    ...
                    ((RCaller) this.rEngine).getRCode()
                            .addRCode("r_version <- R.version.string");
                    LOG.log(logLvl, ((String[]) this.collect(
                            "r_version"/*
        ...
        if (this.rEngineType == REngineType.RSERVE) {
            ...
            if (this.session != null && !this.userCanceled) {
                ...
                try {
                    ...
                    LOG.log(logLvl,
                            "Loaded package KO: '" + e.getMessage() + "'.");
    ...
    public <T> void assign(String objName, T object)
        ...
        if (this.rEngineType == REngineType.RSERVE) {
            ...
            if (this.session != null && !this.userCanceled) {
                String msg = "Rserve error: couldn't assign R object '"
                        + objName + "' (instance '" + this.getPID() + "').";
                try {
            ...
            try {
                ...
                if (object instanceof double[]) {
                    ((RCaller) this.rEngine).getRCode().addDoubleArray(objName,
                            (double[]) object);
                } else if (object instanceof int[]) {
                    ((RCaller) this.rEngine).getRCode().addIntArray(objName,
                            (int[]) object);
                } else if (object instanceof boolean[]) {
                    ((RCaller) this.rEngine).getRCode().addLogicalArray(objName,
                            (boolean[]) object);
                } else if (object instanceof String[]) {
                    ((RCaller) this.rEngine).getRCode().addStringArray(objName,
                            (String[]) object);
                } else if (object instanceof String) {
                    ((RCaller) this.rEngine).getRCode().addString(objName,
                            (String) object);
                } else if (object instanceof Integer) {
                    ((RCaller) this.rEngine).getRCode().addInt(objName,
                            (Integer) object);
                } else if (object instanceof Double) {
                    ((RCaller) this.rEngine).getRCode().addDouble(objName,
                            (Double) object);
                } else {
                    String msg = "RCaller error: couldn't assign R object '"
                            + objName + "' (Unhandled type for object '"
                            + object + "').";
                    throw new RSessionWrapperException(msg);
                }
    ...
    public Object collect(String objOrExp, boolean stopOnError)
            throws RSessionWrapperException {
    ...
    public Object collect(String objOrExp, boolean tryEval, boolean stopOnError)
        ...
        if (this.rEngineType == REngineType.RSERVE) {
            ...
            try {
                ...
                ((RCaller) this.rEngine).getRCode()
                        .addRCode(obj_type_var + " <- typeof(" + obj + ")");
                String obj_ismatrix_var = obj + "_ismatrix";
                ((RCaller) this.rEngine).getRCode().addRCode(obj_ismatrix_var
                        + " <- inherits(" + obj + ", \"matrix\")");
                //
                String code = "obj_lst <- list(" + obj + "=" + obj + ","
                ...
                try {
                    ...
                    String obj_type_value = ((RCaller) this.rEngine).getParser()
                            .getAsStringArray(obj_type_var)[0];
                    boolean obj_ismatrix_value = ((RCaller) this.rEngine)
                            .getParser().getAsLogicalArray(obj_ismatrix_var)[0];
                    RCallerResultType type = getRCallerResultType(obj,
                            obj_type_value,
                            obj_ismatrix_value/* , obj_isarray_value */);

                    // Do get the objects
                    if (type == RCallerResultType.DOUBLE_ARRAY) {
                        return ((RCaller) this.rEngine).getParser()
                                .getAsDoubleArray(obj);
                    } else if (type == RCallerResultType.DOUBLE_MATRIX) {
                        return ((RCaller) this.rEngine).getParser()
                                .getAsDoubleMatrix(obj);
                    } else if (type == RCallerResultType.INT_ARRAY) {
                        return ((RCaller) this.rEngine).getParser()
                                .getAsIntArray(obj);
                    } else if (type == RCallerResultType.BOOL_ARRAY) {
                        return ((RCaller) this.rEngine).getParser()
                                .getAsLogicalArray(obj);
                    } else if (type == RCallerResultType.STRING_ARRAY) {
                        return ((RCaller) this.rEngine).getParser()
                                .getAsStringArray(obj);
                    } else {
                        String msg = this.rEngineType + ": Wrong type passed: '"
                                + type + "' for object: '" + obj + "'!";
                        throw new RSessionWrapperException(msg);
                    }
