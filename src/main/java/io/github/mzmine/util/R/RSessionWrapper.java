...
import io.github.mzmine.util.LoggerStream;
import io.github.mzmine.util.TextUtils;
import io.github.mzmine.util.R.Rcaller.RCallerResultType;
import io.github.mzmine.util.R.Rsession.RserverConf;
...
public class RSessionWrapper {
  ...
  /**
   * NA real value as defined in R. Note: it can NOT be used in comparisons, you must use
   * {@link #isNA(double)} instead.
   */
  public static final double NA_DOUBLE = Double.longBitsToDouble(0x7ff00000000007a2L);
  /**
   * NA integer value as defined in R. Unlike its real equivalent this one can be used in
   * comparisons, although {@link #isNA(int) } is provided for consistency.
   */
    ...
    try {
      ...
      final String globalFailureMsg =
          "Could not start Rserve ( R> install.packages(c('Rserve')) ). "
              ...
              + RLocationDetection.R_HOME_ENV_KEY + "' environment variable can also be used).";
      ...
      final String r_homeFailureMsg = "Correct path to the R installation directory could not be "
          ...
          + "the 'R executable path' in the project's preferences. " + "(Note: alternatively, the '"
          + RLocationDetection.R_HOME_ENV_KEY + "' environment variable can also be used).";
      ...
      if (this.rEngine == null) {
        ...
        if (this.rEngineType == REngineType.RSERVE) {
          ...
          try {
            ...
            Object rSemaphore =
                (isWindows) ? RSessionWrapper.R_SESSION_SEMAPHORE : this.R_DUMMY_SEMAPHORE;
          ...
          LOG.log(logLvl, ((String[]) this.collect("r_version"/*
  ...
  public <T> void assign(String objName, T object) throws RSessionWrapperException {
    ...
    if (this.rEngineType == REngineType.RSERVE) {
      ...
      if (this.session != null && !this.userCanceled) {
        String msg = "Rserve error: couldn't assign R object '" + objName + "' (instance '"
            + this.getPID() + "').";
        try {
  ...
  private void checkConnectivity(RConnection con) throws RSessionWrapperException {
  ...
  //
  private Object collectRserve(String obj, boolean stopOnError) throws RSessionWrapperException {
    ...
    if (this.session != null && !this.userCanceled) {
      ...
      String msg = "Rserve error: couldn't collect result for R expression '" + obj
  ...
  //
  public Object collect(String objOrExp, boolean tryEval, boolean stopOnError)
      throws RSessionWrapperException {
    ...
    if (this.rEngineType == REngineType.RSERVE) {
      ...
      try {
        ...
        // here...
        {
          ...
          if (tryEval) {
            objOrExp = "try(eval(parse(text='" + objOrExp.replace("'", "\\'") + "')),silent=FALSE)";
          }
        ...
        ((RCaller) this.rEngine).getRCode()
            .addRCode(obj_ismatrix_var + " <- inherits(" + obj + ", \"matrix\")");
        //
        String code = "obj_lst <- list(" + obj + "=" + obj + "," + obj_type_var + "=" + obj_type_var
        ...
        try {
          ...
          String obj_type_value =
              ((RCaller) this.rEngine).getParser().getAsStringArray(obj_type_var)[0];
          boolean obj_ismatrix_value =
              ((RCaller) this.rEngine).getParser().getAsLogicalArray(obj_ismatrix_var)[0];
          RCallerResultType type = getRCallerResultType(obj, obj_type_value,
              obj_ismatrix_value/* , obj_isarray_value */);
          ...
          if (type == RCallerResultType.DOUBLE_ARRAY) {
            ...
            String msg =
                this.rEngineType + ": Wrong type passed: '" + type + "' for object: '" + obj + "'!";
            throw new RSessionWrapperException(msg);
          }
  ...
  private RCallerResultType getRCallerResultType(String var, String typeofvar,
      boolean ismatrix/* , boolean isarray */) {
  ...
  /**
   * Keep logging clean. Turn off standard outputs, since 'Rsession' library is way far too
   * talkative on them.
   */
    ...
    if (rSession != null && rSession.getPID() != -1) {
      ...
      if (RSessionWrapper.isWindows()) {
        try {
          ...
          Process proc =
              new ProcessBuilder("TASKKILL", "/PID", "" + rSession.getPID(), "/F").start();
          StreamGobbler errorGobbler = new StreamGobbler(proc.getErrorStream(), "Error", os_err); // ,
          // fos_err);
          ...
          if (exitVal != 0)
            throw new RSessionWrapperException("Killing Rserve instance of PID '"
                + rSession.getPID() + "'" + " failed. \n" + os_err.toString());
