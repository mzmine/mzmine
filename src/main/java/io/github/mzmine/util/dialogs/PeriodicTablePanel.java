...
public class PeriodicTablePanel extends JPanel {
    ...
    public class ElementMouseAction implements MouseListener {
        ...
        public void mouseExited(MouseEvent e) {
            ((ElementButton) e.getSource())
                    .setBackground(buttoncolors.get(e.getSource()));
        }
    ...
    class ElementButton extends JButton {
        ...
        public ElementButton(String element, MouseListener e,
            ...
            setToolTipText(
                    elementTranslator(PeriodicTable.getAtomicNumber(element)));
            addMouseListener(e);
        }
    ...
    private JPanel CreateLabelProperties(String elementSymbol) {
        ...
        if (elementSymbol != null) {
            ...
            label = new JLabel("<html><FONT SIZE=+2>"
                    ...
                    + " (" + elementSymbol + ")</FONT><br> "
                    + GT.get("Atomic number") + " "
                    + PeriodicTable.getAtomicNumber(elementSymbol)
                    + (group != null ? ", " + GT.get("Group") + " " + group
                            : "")
                    + ", " + GT.get("Period") + " "
                    + PeriodicTable.getPeriod(elementSymbol) + "</html>");
            label.setBorder(BorderFactory.createEmptyBorder(5, 5, 5, 5));
            pan.add(label, BorderLayout.NORTH);

            label = new JLabel("<html><FONT> " + GT.get("CAS RN:") + " "
                    + PeriodicTable.getCASId(elementSymbol) + "<br> "
                    + GT.get("Element Category:") + " "
                    + serieTranslator(
                            PeriodicTable.getChemicalSeries(elementSymbol))
                    + "<br> " + GT.get("State:") + " "
                    + phaseTranslator(PeriodicTable.getPhase(elementSymbol))
                    + "<br> " + GT.get("Electronegativity:") + " "
                    + (PeriodicTable
                            .getPaulingElectronegativity(elementSymbol) == null
                                    ? GT.get("undefined")
                                    : PeriodicTable.getPaulingElectronegativity(
                                            elementSymbol))
                    + "<br>" + "</FONT></html>");
