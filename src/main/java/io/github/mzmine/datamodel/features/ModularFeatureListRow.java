...
/**
 * Map of all feature related features.
 *
...
public class ModularFeatureListRow implements FeatureListRow, ModularDataModel {
  ...
  /**
   * Constructor for row with only one raw features file.
   *
   * @param flist
   * @param id
   * @param raw
  ...
  public void addFeature(RawDataFile raw, Feature feature) {
    ...
    if (Objects.equals(modularFeature.getFeatureList(), getFeatureList())) {
      // features are final - replace all values for all features types
      // keep old feature
  ...
  /**
   * nonnull if this feature list contains this raw features file. Even if there is no feature in this
   * raw features file
   *
   * @param raw
   * @return
