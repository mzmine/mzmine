...
public class IonType extends NeutralMolecule implements Comparable<IonType> {
    ...
    public IonType createModified(final @Nonnull IonModification... newMod) {
        ...
        IonModification nm = new CombinedIonModification(
                allMod.toArray(new IonModification[allMod.size()]));
    ...
    /**
     * is a modification of parameter adduct? only if all adducts are the same,
     * mass difference must be different ONLY if this is a mod of parameter
     * adduct
     * 
     * @param adduct
     * @return
    ...
    public boolean isModificationOf(IonType ion) {
        if (!hasMods() || !(ion.getModCount() < getModCount()
                && mass != ion.mass && adduct.equals(ion.adduct)
                && molecules == ion.molecules && charge == ion.charge))
            return false;
        else if (!ion.hasMods())
            return true;
        else {
    ...
    public IonType getModifiedOnly() {
        return new IonType(1,
                IonModification.getUndefinedforCharge(this.charge), mod);
    }
    ...
    public double getMass(double mz) {
        return ((mz * this.getAbsCharge()) - this.getMassDifference())
                / this.getMolecules();
    }
    ...
    public double getMZ(double neutralmass) {
        return (neutralmass * getMolecules() + getMassDifference())
                / getAbsCharge();
    }
    ...
    @Override
    public int hashCode() {
        return Objects.hash(adduct, mod == null ? "" : mod, charge, molecules,
                mass, name);
    }
    ...
    /**
     * Has modifications and the adduct type is undefined (this should be target
     * to refinement)
     * 
     * @return
     */
    ...
    /**
     * Is adding or removing all sub adducts / modifications from the molecular
     * formula
     * 
     * @param formula
     * @return
    ...
    public IMolecularFormula addToFormula(IMolecularFormula formula)
        ...
        // add
        Arrays.stream(adduct.getAdducts())
                .filter(m -> m.getMass() >= 0 && m.getCDKFormula() != null)
                .forEach(m -> FormulaUtils.addFormula(result,
                        m.getCDKFormula()));
        if (mod != null)
            Arrays.stream(mod.getAdducts())
                    .filter(m -> m.getMass() >= 0 && m.getCDKFormula() != null)
                    .forEach(m -> FormulaUtils.addFormula(result,
                            m.getCDKFormula()));
        ...
        Arrays.stream(adduct.getAdducts())
                .filter(m -> m.getMass() < 0 && m.getCDKFormula() != null)
                .forEach(m -> FormulaUtils.subtractFormula(result,
                        m.getCDKFormula()));
        if (mod != null)
            Arrays.stream(mod.getAdducts())
                    .filter(m -> m.getMass() < 0 && m.getCDKFormula() != null)
