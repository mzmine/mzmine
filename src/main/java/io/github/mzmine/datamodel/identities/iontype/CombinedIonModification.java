...
public class CombinedIonModification extends IonModification {
    ...
    public IonModification createOpposite() {
        IonModification[] mod = Arrays.stream(adducts)
                .map(IonModification::createOpposite)
    ...
    public IonModification remove(IonModification type) {
        ...
        if (newList.isEmpty())
            ...
            return new CombinedIonModification(
                    newList.toArray(new IonModification[newList.size()]));
    }
    ...
    public boolean hasModificationLimit() {
        return Arrays.stream(adducts)
                .anyMatch(IonModification::hasModificationLimit);
    }

    @Override
    public int getModificationLimit() {
        return Arrays.stream(adducts)
                .mapToInt(IonModification::getModificationLimit)
